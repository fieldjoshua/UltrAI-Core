...
class DocumentCache:
    ...
    def __init__(self, cache_dir: str = ".cache", memory_cache_size: int = 100):
        ...
        for directory in [self.text_cache_dir, self.chunks_cache_dir, self.embeddings_cache_dir]:
    ...
    def invalidate(self, file_path: str, operation: Optional[str] = None) -> None:
        ...
        file_hash = hashlib.md5(file_path.encode()).hexdigest()
        for cache_dir in [self.text_cache_dir, self.chunks_cache_dir, self.embeddings_cache_dir, self.cache_dir]:
...
class UltraDocumentsOptimized:
    def __init__(
        ...
        if self.cache_enabled:
            ...
            self.logger.info(f"Document cache initialized with {memory_cache_size} memory slots")
    ...
    def _load_embedding_model(self):
        ...
        try:
            ...
            self.logger.info(f"Document embedding model loaded: {self.embedding_model_name}")
        except Exception as e:
    ...
    def _process_text(self, file_path: Union[str, Path]) -> str:
        ...
        try:
            ...
            if self.cache_enabled:
                ...
                if cached_text is not None:
                    self.logger.debug(f"Retrieved text file content from cache: {file_path}")
                    return cached_text
            ...
            try:
                ...
                if text is None:
                    self.logger.error(f"Could not decode text file {file_path} with any encoding")
    ...
    def chunk_document(self, file_path: Union[str, Path]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Check cache first
            if self.cache_enabled:
                cached_chunks = self.cache.get(str(file_path), f"chunks_{self.chunk_size}_{self.chunk_overlap}")
            ...
            # Cache chunks
            if self.cache_enabled:
                self.cache.set(str(file_path), f"chunks_{self.chunk_size}_{self.chunk_overlap}", chunks)
    ...
    def chunk_documents(self, file_paths: List[Union[str, Path]]) -> List[Dict[str, Any]]:
    ...
    async def get_relevant_chunks(self, query: str, chunks: List[Dict[str, Any]], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        if self.embedding_model is None:
            ...
            self.logger.warning("Embedding model not available, falling back to keyword matching")
    ...
    def _keyword_match_chunks(self, query: str, chunks: List[Dict[str, Any]], top_k: int = 5) -> List[Dict[str, Any]]:
