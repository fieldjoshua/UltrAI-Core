document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadDataForCurrentStep() {
        switch (currentStep) {
            ...
            case 4: // Analysis Types
                if (document.getElementById('analysis-types-container').children.length <= 1) {
                    fetchAnalysisTypes();
                }
                break;
            case 5: // A La Carte Options
                if (document.getElementById('alacarte-options-container').children.length <= 1) {
                    fetchAlacarteOptions();
                }
                break;
            case 6: // Output Formats
                if (document.getElementById('output-formats-container').children.length <= 1) {
                    fetchOutputFormats();
                }
                break;
        }
    ...
    async function fetchModels() {
        try {
            ...
            document.getElementById('models-loading').textContent = 'Error loading models. Try again later.';
        }
    }
    ...
    async function fetchAnalysisTypes() {
        try {
            ...
            document.getElementById('analysis-types-loading').textContent = 'Error loading analysis types. Try again later.';
        }
    }
    ...
    async function fetchAlacarteOptions() {
        try {
            ...
            document.getElementById('alacarte-options-loading').textContent = 'Error loading options. Try again later.';
        }
    }
    ...
    async function fetchOutputFormats() {
        try {
            ...
            document.getElementById('output-formats-loading').textContent = 'Error loading output formats. Try again later.';
        }
    }
