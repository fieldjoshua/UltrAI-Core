from sympy import Matrix, Symbol, cos, simplify, sin, symbols, zeros
from sympy.physics.mechanics import Body, inertia
from sympy.physics.vector import Dyadic, Point, ReferenceFrame, dynamicsymbols
...
def test_particle_body():
    ...
    with warns_deprecated_sympy():
        particle_body = Body(
            "particle_body", particle_masscenter, particle_mass, particle_frame
        )
...
def test_body_add_force():
    ...
    point = rigid_body.masscenter.locatenew(
        "rigidbody_body_point0", l * rigid_body.frame.x
    )
...
def test_body_dcm():
    ...
    assert A.dcm(B) == Matrix(
        [[cos(10), sin(10), 0], [-sin(10), cos(10), 0], [0, 0, 1]]
    )
    assert A.dcm(B.frame) == Matrix(
        [[cos(10), sin(10), 0], [-sin(10), cos(10), 0], [0, 0, 1]]
    )
...
def test_apply_force():
    ...
    B1.apply_force(
        force, P1, B2, P2
    )  # applying equal and opposite force on moving points
    ...
    B1.apply_force(
        force2, reaction_body=B2
    )  # Applying time varying force on masscenter
...
def test_parallel_axis():
    ...
    Ip_expected = inertia(
        N, Ix + m * b**2, Iy + m * a**2, Iz + m * (a**2 + b**2), ixy=-m * a * b
    )
    ...
    Ip_expected = inertia(
        N, m * b**2, m * a**2, m * (a**2 + b**2), ixy=-m * a * b
    )
