...
def test_besselj_series():
    ...
    assert besselj(0, x ** (1.1)).series(x) == 1 + x**4.4 / 64 - x**2.2 / 4 + O(
        x**6
    )
...
def test_besseli_series():
    ...
    assert besseli(0, x ** (1.1)).series(x) == 1 + x**4.4 / 64 + x**2.2 / 4 + O(
        x**6
    )
...
def test_jn():
    ...
    assert mjn(3, z) == (15 / z**4 - 6 / z**2) * sin(z) + (
        1 / z - 15 / z**3
    ) * cos(z)
...
def test_issue_26134():
    ...
    assert (
        ...
        .dummy_eq(
            Integral(
                x**2 * exp(-(x**2) / 2 - Rational(9, 2)) * besseli(1, 3 * x),
                (x, 4, oo),
            )
            / 3
        )
