...
def test_lll():
    ...
    for basis_dm, reduced_dm in normal_test_data:
        ...
        reduced, transform = _ddm_lll(
            basis_dm.rep.to_ddm(), delta=delta, return_transform=True
        )
        assert reduced == reduced_dm.rep.to_ddm()
...
def test_lll_linear_dependent():
    ...
    for not_basis in linear_dependent_test_data:
        ...
        raises(
            DMRankError, lambda: _ddm_lll(not_basis.rep.to_ddm(), return_transform=True)
        )
...
def test_lll_wrong_delta():
    ...
    for wrong_delta in [QQ(-1, 4), QQ(0, 1), QQ(1, 4), QQ(1, 1), QQ(100, 1)]:
        ...
        raises(
            DMValueError,
            lambda: _ddm_lll(
                dummy_matrix.rep, delta=wrong_delta, return_transform=True
            ),
        )
        raises(
            DMValueError, lambda: ddm_lll_transform(dummy_matrix.rep, delta=wrong_delta)
        )
        raises(DMValueError, lambda: dummy_matrix.rep.lll_transform(delta=wrong_delta))
        raises(
            DMValueError,
            lambda: dummy_matrix.rep.to_sdm().lll_transform(delta=wrong_delta),
        )
...
def test_lll_wrong_shape():
    ...
    raises(
        DMShapeError, lambda: _ddm_lll(wrong_shape_matrix.rep, return_transform=True)
    )
...
def test_lll_wrong_domain():
    ...
    raises(
        DMDomainError, lambda: _ddm_lll(wrong_domain_matrix.rep, return_transform=True)
    )
