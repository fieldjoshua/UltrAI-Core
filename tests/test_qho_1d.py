from sympy.abc import m, omega, x
...
from sympy.physics.qho_1d import E_n, coherent_state, psi_n
...
def test_wavefunction():
    Psi = {
        ...
        2: (nu / pi) ** Rational(1, 4)
        * (2 * nu * x**2 - 1)
        / sqrt(2)
        * exp(-nu * x**2 / 2),
        3: (nu / pi) ** Rational(1, 4)
        * sqrt(nu / 3)
        * (2 * nu * x**3 - 3 * x)
        * exp(-nu * x**2 / 2),
    }
...
def test_coherent_state(n=10):
    ...
    alpha = Symbol("alpha")
    for i in range(n + 1):
        assert simplify(sqrt(n + 1) * coherent_state(n + 1, alpha)) == simplify(
