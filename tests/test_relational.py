...
def test_univariate_relational_as_set():
    ...
    assert Ne(x, 0).as_set() == Interval(-oo, 0, True, True) + Interval(
        0, oo, True, True
    )
...
def test_multivariate_relational_as_set():
    assert (x * y >= 0).as_set() == Interval(0, oo) * Interval(0, oo) + Interval(
        -oo, 0
    ) * Interval(-oo, 0)
...
def test_issue_18188():
    ...
    assert result2.as_set() == ConditionSet(
...
def test_nothing_happens_to_Eq_condition_during_simplify():
    ...
    assert Eq(2 * sign(r + 3) / (5 * Abs(r + 3) ** Rational(3, 5)), 0).simplify() == Eq(
        Piecewise(
            (0, Eq(r, -3)), ((r + 3) / (5 * Abs((r + 3) ** Rational(8, 5))) * 2, True)
        ),
        0,
    )
...
def test_polynomial_relation_simplification():
    assert Ge(3 * x * (x + 1) + 4, 3 * x).simplify() in [
        Ge(x**2, -Rational(4, 3)),
        Le(-(x**2), Rational(4, 3)),
    ]
    assert Le(-(3 * x * (x + 1) + 4), -3 * x).simplify() in [
        Ge(x**2, -Rational(4, 3)),
        Le(-(x**2), Rational(4, 3)),
    ]
...
def test_is_eq():
    ...
    assert (
        is_eq(
            x,
            y,
            Q.infinite(x) & Q.infinite(y) & Q.extended_real(x) & ~Q.extended_real(y),
        )
        is False
    )
    assert (
    ...
    def _eval_is_eq(lhs, rhs):
        if type(lhs) == PowTest and type(rhs) == PowTest:
            return fuzzy_and(
                [is_eq(lhs.args[0], rhs.args[0]), is_eq(lhs.args[1], rhs.args[1])]
            )
...
def test_is_ge_le():
    ...
    def _eval_is_ge(lhs, rhs):
        if type(lhs) == PowTest and type(rhs) == PowTest:
            return fuzzy_and(
                [is_ge(lhs.args[0], rhs.args[0]), is_ge(lhs.args[1], rhs.args[1])]
            )
