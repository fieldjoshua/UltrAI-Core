...
from sympy.core.function import ArgumentIndexError, Lambda, PoleError, diff
...
def test_sin_series():
    assert sin(x).series(x, 0, 9) == x - x**3 / 6 + x**5 / 120 - x**7 / 5040 + O(
        x**9
    )
...
def test_atan_rewrite():
    ...
    assert (
        atan(x).rewrite(asin)
        == (-asin(1 / sqrt(x**2 + 1)) + pi / 2) * sqrt(x**2) / x
    )
    ...
    assert (
        atan(x).rewrite(acsc) == (-acsc(sqrt(x**2 + 1)) + pi / 2) * sqrt(x**2) / x
    )
...
def test_acot_rewrite():
    ...
    assert acot(x).rewrite(asec) == x * sqrt(x ** (-2)) * asec(
        sqrt((x**2 + 1) / x**2)
    )
...
def test_acsc():
    ...
    assert (
        acsc(x).rewrite(atan) == (-atan(sqrt(x**2 - 1)) + pi / 2) * sqrt(x**2) / x
    )
    assert (
        acsc(x).rewrite(acot)
        == (-acot(1 / sqrt(x**2 - 1)) + pi / 2) * sqrt(x**2) / x
    )
