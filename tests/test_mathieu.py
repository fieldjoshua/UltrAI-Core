...
def test_mathieus():
    ...
    assert conjugate(mathieus(a, q, z)) == mathieus(
        conjugate(a), conjugate(q), conjugate(z)
    )
...
def test_mathieusprime():
    ...
    assert diff(mathieusprime(a, q, z), z) == (-a + 2 * q * cos(2 * z)) * mathieus(
        a, q, z
    )
...
def test_mathieucprime():
    ...
    assert diff(mathieucprime(a, q, z), z) == (-a + 2 * q * cos(2 * z)) * mathieuc(
        a, q, z
    )
