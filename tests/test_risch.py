...
def test_gcdex_diophantine():
    assert gcdex_diophantine(
        Poly(x**4 - 2 * x**3 - 6 * x**2 + 12 * x + 15),
        Poly(x**3 + x**2 - 4 * x - 4),
        Poly(x**2 - 1),
    ) == (
        Poly((-(x**2) + 4 * x - 3) / 5),
        Poly((x**3 - 7 * x**2 + 16 * x - 10) / 5),
    )
    assert gcdex_diophantine(
        Poly(x**3 + 6 * x + 7), Poly(x**2 + 3 * x + 2), Poly(x + 1)
...
def test_frac_in():
    ...
    assert frac_in(
        Poly((2 + 2 * x + x * (1 + x)) / (1 + x) ** 2, t), x, cancel=True
...
def test_derivation():
    ...
    assert derivation(
        Poly(t**2 + 1 / x * t + (1 - 2 * x) / (4 * x**2), t), DE
    ) == Poly(
        -2 * t**3
        - 4 / x * t**2
        - (5 - 2 * x) / (2 * x**2) * t
    ...
    DE = DifferentialExtension(
        extension={"D": [Poly(1, x), Poly(1 / x, t1), Poly(t, t)]}
    )
    assert derivation(Poly(x * t * t1, t), DE) == Poly(t * t1 + x * t * t1 + t, t)
    assert derivation(Poly(x * t * t1, t), DE, coefficientD=True) == Poly(
        (1 + t1) * t, t
    )
    ...
    assert derivation((t + 1) / (t - 1), DE, basic=True) == -2 * t / (
        1 - 2 * t + t**2
    )
...
def test_canonical_representation():
    ...
    assert canonical_representation(
        Poly(t**5 + t**3 + x**2 * t + 1, t), Poly((t**2 + 1) ** 3, t), DE
    ) == (
        Poly(0, t, domain="ZZ[x]"),
        (
...
def test_hermite_reduce():
    ...
    assert hermite_reduce(
        ...
        (
            Poly(-(x**2) - 4, t, domain="ZZ(x,nu)"),
            Poly(4 * t**2 + 2 * x**2 + 4, t, domain="ZZ(x,nu)"),
        ),
        (
            Poly(
                (-2 * nu**2 - x**4) * t - (2 * x**3 + 2 * x), t, domain="ZZ(x,nu)"
            ),
    ...
    assert hermite_reduce(a, d, DE) == (
        (
            ...
            Poly(
                3 * t**4 - 9 * x * t**3 + 9 * x**2 * t**2 - 3 * x**3 * t,
                t,
                domain="ZZ(x)",
            ),
        ),
    ...
    assert hermite_reduce(
        ...
        Poly(-x * t**2 + 2 * x * t - x, t, domain="ZZ(x)"),
        DE,
    ) == (
    ...
    assert hermite_reduce(
        ...
        (
            Poly(x**3 * t + x**4 + 1, t, domain="ZZ(x)"),
            Poly(x**3 * t**3 - x, t, domain="ZZ(x)"),
        ),
        (Poly(0, t, domain="ZZ(x)"), Poly(1, t, domain="ZZ(x)")),
...
def test_residue_reduce():
    ...
    assert residue_reduce(
        ...
        [
            (
                Poly(z**2 - 1, z, domain="QQ"),
                Poly(-2 * z * t / x - 2 / x, t, domain="ZZ(z,x)"),
            )
        ],
        True,
    )
    ...
    assert residue_reduce(
        ...
        [
            (
                ...
                Poly(t**2 + 1 + x**2 / 2, t, domain="ZZ(x,nu)"),
            )
        ],
        True,
    )
...
def test_integrate_hyperexponential_polynomial():
    ...
    DE = DifferentialExtension(
        extension={"D": [Poly(1, x), Poly(1 / x, t0), Poly(2 * x * t1, t1)]}
    )
...
def test_integrate_hyperexponential_returns_piecewise():
    ...
    assert integrate_hyperexponential(DE.fa, DE.fd, DE) == (
        Piecewise(
            ((a * x - 1) * exp(a * x) / a**2, Ne(a**2, 0)), (x**2 / 2, True)
        ),
        0,
        True,
    )
...
def test_issue_13947():
    a, t, s = symbols("a t s")
    assert risch_integrate(2 ** (-pi) / (2**t + 1), t) == 2 ** (-pi) * t - 2 ** (
        -pi
    ) * log(2**t + 1) / log(2)
    assert risch_integrate(a ** (t - s) / (a**t + 1), t) == exp(-s * log(a)) * log(
        a**t + 1
    ) / log(a)
...
def test_integrate_nonlinear_no_specials():
    (
        ...
        x**2 * t**4 + x**2 * (x**2 + 2) * t**2 + x**2 + x**4 + x**6 / 4,
        t,
    )
...
def test_DifferentialExtension_handle_first():
    assert DifferentialExtension(
        exp(x) * log(x), x, handle_first="log"
    )._important_attrs == (
        Poly(t0 * t1, t1),
        Poly(1, t1),
    ...
    assert DifferentialExtension(
        exp(x) * log(x), x, handle_first="exp"
    )._important_attrs == (
        Poly(t0 * t1, t1),
        Poly(1, t1),
    ...
    assert (
        DifferentialExtension(
            -(x**x) * log(x) ** 2 + x**x - x**x / x, x, handle_first="exp"
        )._important_attrs
        == DifferentialExtension(
            -(x**x) * log(x) ** 2 + x**x - x**x / x, x, handle_first="log"
        )._important_attrs
        == (
...
def test_DifferentialExtension_Rothstein():
    ...
    assert DifferentialExtension(f, x)._important_attrs == (
        ...
        [
            ...
            Poly(t0, t0),
            Poly(
                -(10 + 21 * t0 + 10 * t0**2) / (1 + 2 * t0 + t0**2) * t1,
                t1,
                domain="ZZ(t0)",
            ),
        ],
...
def test_risch_integrate():
    ...
    assert (
        risch_integrate(sin(x), x, rewrite_complex=True)
        == -exp(I * x) / 2 - exp(-I * x) / 2
    )
    ...
    e2 = (log(-1 / y) / 2 - log(1 / y) / 2) / y - (
        log(1 - 1 / y) / 2 - log(1 + 1 / y) / 2
    ) / y
    ans2 = risch_integrate(e2, y)
    assert ans2 == log(1 / y) * log(1 - 1 / y) / 2 - log(1 / y) * log(
        1 + 1 / y
    ) / 2 + NonElementaryIntegral(
...
def test_risch_integrate_float():
    assert risch_integrate(
        (-60 * exp(x) - 19.2 * exp(4 * x)) * exp(4 * x), x
...
def test_NonElementaryIntegral():
    ...
    assert isinstance(
        NonElementaryIntegral(x**x * t0, x).subs(t0, log(x)), NonElementaryIntegral
    )
...
def test_issue_23948():
    f = (
        (
            (-2 * x**5 + 28 * x**4 - 144 * x**3 + 324 * x**2 - 270 * x)
            * log(x) ** 2
            + (-4 * x**6 + 56 * x**5 - 288 * x**4 + 648 * x**3 - 540 * x**2)
            * log(x)
            + (2 * x**5 - 28 * x**4 + 144 * x**3 - 324 * x**2 + 270 * x)
            * exp(x)
            + (2 * x**5 - 28 * x**4 + 144 * x**3 - 324 * x**2 + 270 * x)
        ...
        * log(-log(x) ** 2 - 2 * x * log(x) + exp(x) + log(5) - x**2 - x) ** 2
        + (
            (4 * x**5 - 44 * x**4 + 168 * x**3 - 216 * x**2 - 108 * x + 324)
            * log(x)
            + (-2 * x**5 + 24 * x**4 - 108 * x**3 + 216 * x**2 - 162 * x)
            * exp(x)
            + 4 * x**6
            - 42 * x**5
            + 144 * x**4
