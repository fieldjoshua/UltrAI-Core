...
def test_derivation():
    ...
    DE = DifferentialExtension(
        extension={"D": [Poly(1, x), Poly(-(t**2) - 3 / (2 * x) * t + 1 / (2 * x), t)]}
    )
...
def test_hermite_reduce():
    ...
    DE = DifferentialExtension(
        extension={"D": [Poly(1, x), Poly(-(t**2) - t / x - (1 - nu**2 / x**2), t)]}
    )
    ...
    d = Poly(
        x * t**6 - 4 * x**2 * t**5 + 6 * x**3 * t**4 - 4 * x**4 * t**3 + x**5 * t**2, t
    )
...
def test_residue_reduce():
    ...
    DE = DifferentialExtension(extension={"D": [Poly(1, x), Poly(1 + t**2, t)]})
    assert residue_reduce(
        Poly(-2 * x * t + 1 - x**2, t), Poly(t**2 + 2 * x * t + 1 + x**2, t), DE, z
...
def test_integrate_primitive():
    ...
    assert integrate_primitive(
        Poly(
            x**2 * t0**3 + (3 * x**2 + x) * t0**2 + (3 * x**2 + 2 * x) * t0 + x**2 + x,
            t0,
        ),
        Poly(
            x**2 * t0**4 + 4 * x**2 * t0**3 + 6 * x**2 * t0**2 + 4 * x**2 * t0 + x**2,
            t0,
        ),
        DE,
