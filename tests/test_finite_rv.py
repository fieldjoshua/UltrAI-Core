...
def test_discreteuniform():
    ...
    assert (
        simplify(
            variance(X) - ((a**2 + b**2 + c**2) / 3 - (a / 3 + b / 3 + c / 3) ** 2)
        )
        == 0
    )
    ...
    assert (
        ...
        == exp(I * a * t) / 3 + exp(I * b * t) / 3 + exp(I * c * t) / 3
    )
    assert (
        moment_generating_function(X)(t)
        == exp(a * t) / 3 + exp(b * t) / 3 + exp(c * t) / 3
    )
    # issue 18611
...
def test_dice():
    ...
    d = density(2 * X + Y**Z)
    assert (
        d[S(22)] == Rational(1, 108)
        and d[S(4100)] == Rational(1, 216)
        and S(3130) not in d
    )
    ...
    assert cf.dummy_eq(
        Sum(
            Piecewise((exp(ki * I * t) / n, (ki >= 1) & (ki <= n)), (0, True)),
            (ki, 1, n),
        )
    )
    assert (
        cf.subs(n, 3).doit() == exp(3 * I * t) / 3 + exp(2 * I * t) / 3 + exp(I * t) / 3
    )
...
def test_domains():
    ...
    d = where(And(X > Y, Y > 3))
    assert d.as_boolean() == Or(
        And(Eq(x, 5), Eq(y, 4)), And(Eq(x, 6), Eq(y, 5)), And(Eq(x, 6), Eq(y, 4))
    )
...
def test_bernoulli():
    ...
    assert characteristic_function(X)(t) == p * exp(I * a * t) + (-p + 1) * exp(
        I * b * t
    )
    ...
    assert quantile(X)(z) == Piecewise(
        (nan, (z > 1) | (z < 0)), (0, z <= 1 - p), (1, z <= 1)
    )
    ...
    assert coskewness(Y + 2 * X + Z, X + 2 * Y + Z, X + 2 * Z + Y).simplify() == sqrt(
        1529
    ) * Rational(12, 16819)
    assert coskewness(
        Y + 2 * X + Z, X + 2 * Y + Z, X + 2 * Z + Y, X < 2
    ).simplify() == -sqrt(357451121) * Rational(2812, 4646864573)
...
def test_cdf():
    ...
    assert cdf(D) == sympify(
        {1: o / 6, 2: o / 3, 3: o / 2, 4: 2 * o / 3, 5: 5 * o / 6, 6: o}
    )
...
def test_binomial_symbolic():
    ...
    assert (
        characteristic_function(X)(t)
        == p**2 * exp(2 * I * t) + 2 * p * (-p + 1) * exp(I * t) + (-p + 1) ** 2
    )
    assert (
        moment_generating_function(X)(t)
        == p**2 * exp(2 * t) + 2 * p * (-p + 1) * exp(t) + (-p + 1) ** 2
    )
...
def test_beta_binomial():
    ...
    assert characteristic_function(X)(t) == exp(2 * I * t) * beta(a + 2, b) / beta(
        a, b
    ) + 2 * exp(I * t) * beta(a + 1, b + 1) / beta(a, b) + beta(a, b + 2) / beta(a, b)
    assert moment_generating_function(X)(t) == exp(2 * t) * beta(a + 2, b) / beta(
        a, b
...
def test_hypergeometric_symbolic():
    ...
    assert set(dens.subs({N: 3, m: 2, n: 1}).doit().values()) == {
        Rational(1, 3),
        Rational(2, 3),
    }
    k = Dummy("k", integer=True)
    assert expec.dummy_eq(
        Sum(
            Piecewise(
                (k * binomial(m, k) * binomial(N - m, -k + n) / binomial(N, n), k > 2),
                (0, True),
            ),
            (k, 0, n),
        )
...
def test_ideal_soliton():
    ...
    for i in k_vals:
        ...
        assert (
            variance(sol.subs(k, i))
            == (i - 1) + harmonic(i) - harmonic(i) ** 2
            == cmoment(sol.subs(k, i), 2)
        )
    ...
    assert exp(I * t) / 10 + Sum(exp(I * t * x) / (x * x - x), (x, 2, k)).subs(
        k, 10
    ).doit() == characteristic_function(sol.subs(k, 10))(t)
    assert exp(t) / 10 + Sum(exp(t * x) / (x * x - x), (x, 2, k)).subs(
        k, 10
    ).doit() == moment_generating_function(sol.subs(k, 10))(t)
...
def test_robust_soliton():
    ...
    for x in k_vals:
        for y in delta_vals:
            for z in c_vals:
                assert E(robSol.subs({k: x, delta: y, c: z})) == moment(
                    robSol.subs({k: x, delta: y, c: z}), 1
                )
                assert variance(robSol.subs({k: x, delta: y, c: z})) == cmoment(
                    robSol.subs({k: x, delta: y, c: z}), 2
                )
                assert skewness(robSol.subs({k: x, delta: y, c: z})) == smoment(
                    robSol.subs({k: x, delta: y, c: z}), 3
                )
                assert kurtosis(robSol.subs({k: x, delta: y, c: z})) == smoment(
                    robSol.subs({k: x, delta: y, c: z}), 4
                )
...
def test_FiniteRV():
    ...
    raises(
        ValueError, lambda: FiniteRV("F", {1: S.Half, 2: S.Half, 3: S.Half}, check=True)
    )
    raises(
        ValueError,
        lambda: FiniteRV("F", {1: S.Half, 2: Rational(-1, 2), 3: S.One}, check=True),
    )
    raises(
        ValueError,
        lambda: FiniteRV(
...
def test_symbolic_conditions():
    ...
    Y = P(Eq(B, b))
    Z = E(D > n)
    assert Y == Piecewise((Rational(1, 4), Eq(b, 1)), (0, True)) + Piecewise(
        (Rational(3, 4), Eq(b, 0)), (0, True)
    )
    assert Z == Piecewise((Rational(1, 4), n < 1), (0, True)) + Piecewise(
        (S.Half, n < 2), (0, True)
    ) + Piecewise((Rational(3, 4), n < 3), (0, True)) + Piecewise(
        (S.One, n < 4), (0, True)
    )
