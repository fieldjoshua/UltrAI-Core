...
from sympy.core import Derivative, EulerGamma, Subs, diff
...
from sympy.functions.elementary.hyperbolic import asinh, cosh, coth, sinh
...
def test_laplace_transform():
    ...
    assert LT(sinh(2 * sqrt(a * t)), t, s) == (
        sqrt(pi) * sqrt(a) * exp(a / s) / s ** (S(3) / 2),
        0,
        True,
    )
    ...
    assert LT(sin(2 * sqrt(a * t)) / t, t, s) == (
        pi * erf(sqrt(a) * sqrt(1 / s)),
        0,
        True,
    )
    ...
    assert LT(erf(sqrt(a / t) / 2), t, s, simplify=True) == (
        1 / s - exp(-sqrt(a) * sqrt(s)) / s,
        0,
        True,
    )
    assert LT(erfc(sqrt(a * t)), t, s, simplify=True) == (
        -sqrt(a) / (s * sqrt(a + s)) + 1 / s,
        -a,
        True,
    )
...
def test_inverse_laplace_transform_old():
    ...
    assert ILT(
        a**b * (s + sqrt(s**2 + a**2)) ** (-b) / sqrt(s**2 + a**2),
        s,
        t,
        simplify=True,
