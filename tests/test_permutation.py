...
from sympy.matrices.expressions import BlockDiagMatrix, Determinant, Inverse, MatMul
...
def test_MatrixPermute_rewrite_MatMul():
    ...
    assert (
        MatrixPermute(A, p, 0).rewrite(MatMul).as_explicit()
        == MatrixPermute(A, p, 0).as_explicit()
    )
    assert (
        MatrixPermute(A, p, 1).rewrite(MatMul).as_explicit()
...
def test_MatrixPermute_doit():
    ...
    A = Identity(3)
    assert (
        MatrixPermute(A, p, 0).doit().as_explicit()
        == MatrixPermute(A, p, 0).as_explicit()
    )
    assert (
        MatrixPermute(A, p, 1).doit().as_explicit()
        == MatrixPermute(A, p, 1).as_explicit()
    )
