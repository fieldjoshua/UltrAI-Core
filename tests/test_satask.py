...
from sympy.assumptions.satask import extract_predargs, get_relevant_clsfacts, satask
...
def test_rational_irrational():
    ...
    assert (
        satask(
            Q.irrational(x * y * z), Q.irrational(x) & Q.irrational(y) & Q.rational(z)
        )
        is None
    )
    assert (
        satask(Q.irrational(x * y * z), Q.irrational(x) & Q.rational(y) & Q.rational(z))
        is True
    )
    ...
    assert (
        satask(Q.irrational(x * y * z), Q.rational(x) & Q.rational(y) & Q.rational(z))
        is False
    )
    assert (
        satask(Q.rational(x * y * z), Q.rational(x) & Q.rational(y) & Q.rational(z))
        is True
    )
...
def test_integer():
    ...
    assert (
        satask(Q.integer(x + y + z), Q.integer(x) & Q.integer(y) & ~Q.integer(z))
        is False
    )
    assert (
        satask(Q.integer(x + y + z), Q.integer(x) & ~Q.integer(y) & ~Q.integer(z))
    ...
    assert (
        satask(Q.integer(x * y * z), Q.integer(x) & Q.integer(y) & ~Q.rational(z))
        is False
    )
    assert (
        satask(Q.integer(x * y * z), Q.integer(x) & ~Q.rational(y) & ~Q.rational(z))
        is None
    )
...
def test_get_relevant_clsfacts():
    ...
    assert facts.clauses == {
        ...
        frozenset(
            {Literal(Q.positive(Abs(x * y)), False), Literal(Q.zero(Abs(x * y)), False)}
        ),
    }
