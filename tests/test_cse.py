...
from sympy.matrices.expressions import Inverse, MatAdd, MatMul, MatrixSymbol, Transpose
...
def test_numbered_symbols():
    ...
    assert list(itertools.islice(ns, 0, 10)) == [
        Symbol("y%s" % i) for i in range(0, 10)
    ]
    ns = cse_main.numbered_symbols(prefix="y")
    assert list(itertools.islice(ns, 10, 20)) == [
        Symbol("y%s" % i) for i in range(10, 20)
    ]
    ns = cse_main.numbered_symbols()
    assert list(itertools.islice(ns, 0, 10)) == [
        Symbol("x%s" % i) for i in range(0, 10)
    ]
...
def test_postprocess():
    ...
    assert cse(
        [eq, Eq(x, z + 1), z - 2, (z + 1) * (x + 1)], postprocess=cse_main.cse_separate
    ) == [
...
def test_issue_11577():
    def check(eq):
        r, c = cse(eq)
        assert eq.count_ops() >= len(r) + sum(i[1].count_ops() for i in r) + count_ops(
            c
        )
...
def test_cse_ignore():
    ...
    assert any(
        y in sub.free_symbols for _, sub in subst1
    ), "cse failed to identify any term with y"

    subst2, red2 = cse(exprs, ignore=(y,))  # y is not allowed in substitutions
    assert not any(
        y in sub.free_symbols for _, sub in subst2
    ), "Sub-expressions containing y must be ignored"
    assert any(
        sub - sqrt(x + 1) == 0 for _, sub in subst2
    ), "cse failed to identify sqrt(x + 1) as sub-expression"
...
def test_cse_matrix_kalman_filter():
    ...
    reduced_exprs_expected = [
        MatAdd(
            mu,
            MatMul(Sigma, x0, x1, MatAdd(MatMul(H, mu), MatMul(S.NegativeOne, data))),
        ),
