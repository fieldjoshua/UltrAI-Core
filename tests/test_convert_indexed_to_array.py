...
def test_arrayexpr_convert_index_to_array_support_function():
    ...
    expr = M[i, j] + N[j, i]
    assert _convert_indexed_to_array(expr) == (
        ArrayAdd(M, PermuteDims(N, Permutation([1, 0]))),
        (i, j),
    )
    expr = M[i, j] + M[j, i]
    assert _convert_indexed_to_array(expr) == (
        ArrayAdd(M, PermuteDims(M, Permutation([1, 0]))),
        (i, j),
    )
    expr = (M * N * P)[i, j]
    assert _convert_indexed_to_array(expr) == (
        _array_contraction(ArrayTensorProduct(M, N, P), (1, 2), (3, 4)),
        (i, j),
    )
...
def test_arrayexpr_convert_indexed_to_array_and_back_to_matrix():
    ...
    assert convert_array_to_matrix(p1) == ArrayAdd(
        ArrayTensorProduct(M, N), ArrayTensorProduct(N, M)
    )
    ...
    expr = Sum(
        (P[j, m] + P[m, j]) * (M[i, j] * N[m, n] + N[i, j] * M[m, n]),
        (j, 0, k - 1),
        (m, 0, k - 1),
    )
    p1, p2 = _convert_indexed_to_array(expr)
    assert (
        convert_array_to_matrix(p1) == M * P * N + M * P.T * N + N * P * M + N * P.T * M
    )
