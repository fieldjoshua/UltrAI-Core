...
def test_order_at():
    ...
    assert (
        order_at_oo(Poly(t**2 - 1, t), Poly(t + 1), t)
        == order_at_oo(Poly(t - 1, t), Poly(1, t), t)
        == -1
    )
...
def test_weak_normalizer():
    a = Poly(
        (1 + x) * t**5 + 4 * t**4 + (-1 - 3 * x) * t**3 - 4 * t**2 + (-2 + 2 * x) * t, t
    )
...
def test_normal_denom():
    ...
    raises(
        ...
        lambda: normal_denom(Poly(1, x), Poly(1, x), Poly(1, x), Poly(x, x), DE),
    )
...
def test_spde():
    ...
    raises(
        NonElementaryIntegralException,
        lambda: spde(Poly(t, t), Poly((t - 1) * (t**2 + 1), t), Poly(1, t), 0, DE),
    )
    DE = DifferentialExtension(extension={"D": [Poly(1, x), Poly(t, t)]})
    assert spde(
    ...
    assert spde(
        Poly(x**2 + x + 1, x),
        Poly(-2 * x - 1, x),
        Poly(x**5 / 2 + 3 * x**4 / 4 + x**3 - x**2 + 1, x),
        4,
        DE,
    ) == (
        Poly(0, x, domain="QQ"),
    ...
    DE = DifferentialExtension(extension={"D": [Poly(1, x)]})
    assert spde(
        Poly(x**2 - x, x), Poly(1, x), Poly(9 * x**4 - 10 * x**3 + 2 * x**2, x), 4, DE
    ) == (
        Poly(0, x, domain="ZZ"),
        Poly(0, x),
        0,
...
def test_solve_poly_rde_no_cancel():
    ...
    assert solve_poly_rde(
        Poly(t**2 + 1, t), Poly(t**3 + (x + 1) * t**2 + t + x + 2, t), oo, DE
    ) == Poly(t + x, t)
    # deg(b) small
    DE = DifferentialExtension(extension={"D": [Poly(1, x)]})
    assert solve_poly_rde(Poly(0, x), Poly(x / 2 - Rational(1, 4), x), oo, DE) == Poly(
        x**2 / 4 - x / 4, x
    )
    DE = DifferentialExtension(extension={"D": [Poly(1, x), Poly(t**2 + 1, t)]})
    assert solve_poly_rde(Poly(2, t), Poly(t**2 + 2 * t + 3, t), 1, DE) == Poly(
        t + 1, t, x
    )
    # deg(b) == deg(D) - 1
    DE = DifferentialExtension(extension={"D": [Poly(1, x), Poly(t**2 + 1, t)]})
    assert no_cancel_equal(
        Poly(1 - t, t), Poly(t**3 + t**2 - 2 * x * t - 2 * x, t), oo, DE
...
def test_solve_poly_rde_cancel():
    ...
    raises(
        NonElementaryIntegralException,
        lambda: cancel_primitive(Poly(1, t), Poly(t, t), oo, DE),
    )

    assert cancel_primitive(Poly(1, t), Poly(t + 1 / x, t), 2, DE) == Poly(t, t)
    assert cancel_primitive(
        Poly(4 * x, t), Poly(4 * x * t**2 + 2 * t / x, t), 3, DE
    ) == Poly(t**2, t)
...
def test_rischDE():
    ...
    DE.decrement_level()
    assert rischDE(
        Poly(-2 * x, x), Poly(1, x), Poly(1 - 2 * x - 2 * x**2, x), Poly(1, x), DE
    ) == (Poly(x + 1, x), Poly(1, x))
