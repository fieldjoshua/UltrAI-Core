...
class TestSystemBase:
    ...
    def _filled_system_check(self, exclude=()):
        ...
        assert "holonomic_constraints" in exclude or self.system.holonomic_constraints[
            :
        ] == [q[2] - q[0] + q[1]]
        assert (
            "nonholonomic_constraints" in exclude
            or self.system.nonholonomic_constraints[:] == [u[3] - qd[1] + u[2]]
        )
...
class TestSystem(TestSystemBase):
    ...
    def test_coordinates(
        self, _empty_system_setup, args, kwargs, exp_q_ind, exp_q_dep, exp_q
    ):
    ...
    def test_speeds(
        self, _empty_system_setup, args, kwargs, exp_u_ind, exp_u_dep, exp_u
    ):
        # Test add_speeds
    ...
    def test_nonholonomic_constraints(
        self, _filled_system_setup, args, kwargs, exp_con
    ):
    ...
    def test_velocity_constraints_overwrite(
        self, _filled_system_setup, constraints, expected
    ):
    ...
    def test_add_joints(self):
        ...
        assert system.kdes == ImmutableMatrix(
            [u1 - q1.diff(t), u3 - q3.diff(t), -(u2 - q2.diff(t))]
        )
    ...
    def test_form_eoms_kwargs_errors(
        self, _empty_system_setup, eom_method, kwargs, error
    ):
        self.system.q_ind = q[0]
        p = Particle("p", mass=symbols("m"))
        self.system.add_bodies(p)
...
class TestValidateSystem(TestSystemBase):
    ...
    @pytest.mark.parametrize(
        "method, with_speeds", [(KanesMethod, True), (LagrangesMethod, False)]
    )
...
class TestSystemExamples:
    def test_cart_pendulum_kanes(self):
        ...
        upd_sol = tuple(solve(system.form_eoms().xreplace(subs), up.diff(t)).values())[
            0
        ]
    ...
    def test_cart_pendulum_lagrange(self):
        ...
        for body in system.bodies:
            body.potential_energy = (
                body.mass
                * g
                * body.masscenter.pos_from(system.fixed_point).dot(system.y)
            )
        system.add_loads((cart.masscenter, F * rail.x))
        system.add_actuators(TorqueActuator(k * qp, cart.z, bob_frame, cart))
        ...
        qpdd_sol = solve(
            eoms[0].xreplace({lam1: lam1_sol}).xreplace(subs), qpd.diff(t)
        )[0]
        ...
        Md = ImmutableMatrix(
            [
                [l**2 * mp, l * mp * cos(qp), -l * cos(qp)],
                [l * mp * cos(qp), mc + mp, -1],
            ]
        )
        gd = ImmutableMatrix(
            [[-g * l * mp * sin(qp) + k * qp], [l * mp * sin(qp) * qpd**2 + F]]
        )
    ...
    def test_box_on_ground(self):
        ...
        system.apply_uniform_gravity(-g * system.y)
        system.add_loads(
            Force(P, N * system.y), Force(P, F * system.x - mu * N * system.x)
        )
        system.validate_system()
        system.form_eoms()
