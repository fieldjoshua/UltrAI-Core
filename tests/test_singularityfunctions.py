...
def test_singularityintegrate():
    ...
    assert (
        4 * singularityintegrate(SingularityFunction(x, a, 3), x)
        == 4 * SingularityFunction(x, a, 4) / 4
    )
    assert singularityintegrate(
        5 * SingularityFunction(x, 5, -2), x
    ) == 5 * SingularityFunction(x, 5, -1)
    assert singularityintegrate(
        6 * SingularityFunction(x, 5, -1), x
    ...
    assert singularityintegrate(SingularityFunction(x, 0, -1) * f(x), x) == f(
        0
    ) * SingularityFunction(x, 0, 0)
    assert singularityintegrate(SingularityFunction(x, 1, -1) * f(x), x) == f(
        1
    ) * SingularityFunction(x, 1, 0)
    assert singularityintegrate(
        y * SingularityFunction(x, 0, -1) ** 2, x
