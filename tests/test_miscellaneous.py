...
from sympy.testing.pytest import ignore_warnings, raises, skip
...
def test_Min():
    ...
    assert Min(0, -x, 1 - 2 * x).diff(x) == -Heaviside(
        x + Min(0, -2 * x + 1)
...
def test_Max():
    ...
    assert Max(x**2, 1 + x, 1).diff(x) == 2 * x * Heaviside(
        x**2 - Max(1, x + 1)
...
def test_rewrite_MaxMin_as_Heaviside():
    ...
    assert Max(0, x + 2, 2 * x).rewrite(Heaviside) == 2 * x * Heaviside(
        2 * x
    ) * Heaviside(x - 2) + (x + 2) * Heaviside(-x + 2) * Heaviside(x + 2)

    assert Min(0, x).rewrite(Heaviside) == x * Heaviside(-x)
    assert Min(3, x).rewrite(Heaviside) == x * Heaviside(-x + 3) + 3 * Heaviside(x - 3)
    assert Min(x, -x, -2).rewrite(Heaviside) == x * Heaviside(-2 * x) * Heaviside(
        -x - 2
    ) - x * Heaviside(2 * x) * Heaviside(x - 2) - 2 * Heaviside(-x + 2) * Heaviside(
        x + 2
    )
...
def test_rewrite_MaxMin_as_Piecewise():
    ...
    assert Max(x, y, z).rewrite(Piecewise) == Piecewise(
        (x, (x >= y) & (x >= z)), (y, y >= z), (z, True)
    )
    ...
    assert Min(x, y, z).rewrite(Piecewise) == Piecewise(
        (x, (x <= y) & (x <= z)), (y, y <= z), (z, True)
    )
    ...
    assert Min(va, vx, vy).rewrite(Piecewise) == Piecewise(
        (va, (va <= vx) & (va <= vy)), (vx, vx <= vy), (vy, True)
    )
...
def test_issue_11099():
    ...
    assert (
        Min(x, y).evalf(subs=fixed_test_data) == Min(x, y).subs(fixed_test_data).evalf()
    )
    assert (
        Max(x, y).evalf(subs=fixed_test_data) == Max(x, y).subs(fixed_test_data).evalf()
    )
    ...
    for i in range(20):
        ...
        assert (
            Min(x, y).evalf(subs=random_test_data)
            == Min(x, y).subs(random_test_data).evalf()
        )
        assert (
            Max(x, y).evalf(subs=random_test_data)
