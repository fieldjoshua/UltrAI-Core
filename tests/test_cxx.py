...
from sympy.functions import Ei, Max, Min, beta, frac, riemann_xi, sqrt, zeta
...
def test_CXX98CodePrinter():
    ...
    assert (
        CXX98CodePrinter().doprint(Min(x, 3, sqrt(x)))
        == "std::min(3, std::min(x, std::sqrt(x)))"
    )
...
def test_cxxcode_nested_minmax():
    assert (
        cxxcode(Max(Min(x, y), Min(u, v))) == "std::max(std::min(u, v), std::min(x, y))"
    )
    assert (
        cxxcode(Min(Max(x, y), Max(u, v))) == "std::min(std::max(u, v), std::max(x, y))"
    )
