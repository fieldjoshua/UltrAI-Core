...
from sympy.core import Add, Function, Mul, Rational, S, SympifyError, symbols
from sympy.core.exprtools import gcd_terms
from sympy.core.function import diff, expand
from sympy.core.relational import Eq
from sympy.core.symbol import Dummy, Str, Symbol
from sympy.functions import cbrt, cos, exp, sin, sqrt, tan
...
def test_matadd_simplify():
    ...
    assert simplify(
        MatAdd(A, ImmutableMatrix([[sin(x) ** 2 + cos(x) ** 2]]))
    ) == MatAdd(A, Matrix([[1]]))
...
def test_matmul_simplify():
    A = MatrixSymbol("A", 1, 1)
    assert simplify(
        MatMul(A, ImmutableMatrix([[sin(x) ** 2 + cos(x) ** 2]]))
    ) == MatMul(A, Matrix([[1]]))
...
def test_matrixelement_diff():
    ...
    assert w[k, p].diff(w[0, 0]) == KroneckerDelta(0, k, (0, n - 1)) * KroneckerDelta(
        0, p, (0, 0)
    )
    _i_1 = Dummy("_i_1")
    assert dexpr.dummy_eq(
        Sum(KroneckerDelta(_i_1, p, (0, n - 1)) * D[k, _i_1], (_i_1, 0, n - 1))
    )
...
def test_factor_expand():
    ...
    assert factor(expr) in [
        I - C,
        B**-1 * (A**-1 * (I - C) * B**-1) ** -1 * A**-1,
    ]
...
def test_numpy_conversion():
    ...
    np_array = array(
        [
            [MatrixElement(A, 0, 0), MatrixElement(A, 0, 1)],
