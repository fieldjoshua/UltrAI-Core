...
def test_TR2i():
    ...
    assert (
        TR2i((cos(4) + 1) ** (-a) * sin(4) ** a, half=True)
        == (cos(4) + 1) ** (-a) * sin(4) ** a
    )
    assert (
        TR2i((cos(5) + 1) ** (-a) * sin(5) ** a, half=True)
        == (cos(5) + 1) ** (-a) * sin(5) ** a
    )
    assert TR2i((cos(1) + 1) ** a * sin(1) ** (-a), half=True) == tan(S.Half) ** (-a)
    assert TR2i((cos(2) + 1) ** a * sin(2) ** (-a), half=True) == tan(1) ** (-a)
    assert TR2i((cos(4) + 1) ** a * sin(4) ** (-a), half=True) == (
        cos(4) + 1
    ) ** a * sin(4) ** (-a)
    assert TR2i((cos(5) + 1) ** a * sin(5) ** (-a), half=True) == (
        cos(5) + 1
    ) ** a * sin(5) ** (-a)

    i = symbols("i", integer=True)
    assert TR2i(((cos(5) + 1) ** i * sin(5) ** (-i)), half=True) == tan(5 * S.Half) ** (
        -i
    )
    assert (
        TR2i(1 / ((cos(5) + 1) ** i * sin(5) ** (-i)), half=True)
        == tan(5 * S.Half) ** i
    )
...
def test_TR9():
    ...
    assert TR9(cos(3) + cos(4) + cos(5) + cos(6)) == 4 * cos(S.Half) * cos(1) * cos(
        Rational(9, 2)
    )
    assert TR9(cos(3) + cos(3) * cos(2)) == cos(3) + cos(2) * cos(3)
    assert TR9(-cos(y) + cos(x * y)) == -2 * sin(x * y / 2 - y / 2) * sin(
        x * y / 2 + y / 2
    )
    assert TR9(-sin(y) + sin(x * y)) == 2 * sin(x * y / 2 - y / 2) * cos(
        x * y / 2 + y / 2
    )
    c = cos(x)
    s = sin(x)
...
def test_TR10i():
    ...
    assert TR10i(2 * cos(1) * sin(3) + 2 * sin(1) * cos(3) + cos(3)) == 2 * sin(
        4
    ) + cos(3)
    assert TR10i(cos(2) * cos(3) + sin(2) * (cos(1) * sin(2) + cos(2) * sin(1))) == cos(
        1
    )
    eq = (cos(2) * cos(3) + sin(2) * (cos(1) * sin(2) + cos(2) * sin(1))) * cos(
        5
    ) + sin(1) * sin(5)
    assert TR10i(eq) == TR10i(eq.expand()) == cos(4)
    assert TR10i(sqrt(2) * cos(x) * x + sqrt(6) * sin(x) * x) == 2 * sqrt(2) * x * sin(
        x + pi / 6
    )
    assert (
        TR10i(
    ...
    assert TR10i(cos(x) + sqrt(3) * sin(x) + 2 * sqrt(3) * cos(x + pi / 6)) == 4 * cos(
        x
    )
    assert TR10i(
        cos(x) + sqrt(3) * sin(x) + 2 * sqrt(3) * cos(x + pi / 6) + 4 * sin(x)
    ) == 4 * sqrt(2) * sin(x + pi / 4)
...
def test_TR11():
    ...
    assert TR11(sin(x * Rational(4, 3))) == 4 * (
        (-sin(x / 3) ** 2 + cos(x / 3) ** 2) * sin(x / 3) * cos(x / 3)
    )
...
def test__TR11():
    ...
    assert _TR11(sin(2 * x) * cos(x / 8) / sin(x / 4)) == sin(2 * x) / (
        2 * sin(x / 8)
    ), _TR11(sin(2 * x) * cos(x / 8) / sin(x / 4))
...
def test_fu():
    ...
    assert fu(sin(a) * (cos(b) - sin(b)) + cos(a) * (sin(b) + cos(b))) == sqrt(2) * sin(
        a + b + pi / 4
    )
...
def test_process_common_addends():
    ...
    assert (
        process_common_addends(
            Add(*[1, 2, 3, 4], evaluate=False), do, key2=lambda x: x % 2, key1=False
        )
        == 1**1 + 3**1 + 2**0 + 4**0
    )
...
def test_trig_split():
    ...
    assert trig_split(
        -sqrt(6) * cos(x) * sin(y), -sqrt(2) * sin(x) * sin(y), two=True
...
def test_TRmorrie():
    assert TRmorrie(7 * Mul(*[cos(i) for i in range(10)])) == 7 * sin(12) * sin(
        16
    ) * cos(5) * cos(7) * cos(9) / (64 * sin(1) * sin(3))
    assert TRmorrie(x) == x
...
def test_TR12i():
    ...
    assert (
        TR12i(eq.expand()) == -3 * tan(a + b) * tan(a + c) / (tan(a) + tan(b) - 1) / 2
    )
...
def test_issue_25590():
    ...
    assert (
        TR13(tan(2) * tan(3) * B * A)
        == (-tan(2) / tan(5) - tan(3) / tan(5) + 1) * B * A
    )

    # XXX The result may not be optimal than
    # sin(2*x)*B*A + cos(x)**2 and may change in the future
    assert (2 * cos(x) * sin(x) * B * A + cos(x) ** 2).simplify() == sin(
        2 * x
    ) * B * A + cos(2 * x) / 2 + S.One / 2
