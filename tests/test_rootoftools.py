...
from sympy.polys.rootoftools import CRootOf, RootOf, RootSum
from sympy.polys.rootoftools import _pure_key_dict as D
...
def test_CRootOf_real_roots():
    ...
    assert Poly(x**5 + x + 1).real_roots(radicals=False) == [
        rootof(x**3 - x**2 + 1, 0)
    ]
