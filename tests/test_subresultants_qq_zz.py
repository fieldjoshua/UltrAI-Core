...
def test_sylvester():
    ...
    assert (
        sylvester(x**3 - 7, 0, x) == sylvester(x**3 - 7, 0, x, 1) == Matrix([[0]])
    )
    assert (
        sylvester(0, x**3 - 7, x) == sylvester(0, x**3 - 7, x, 1) == Matrix([[0]])
    )
    assert sylvester(x**3 - 7, 0, x, 2) == Matrix([[0]])
    assert sylvester(0, x**3 - 7, x, 2) == Matrix([[0]])

    assert (
        sylvester(x**3 - 7, 7, x).det() == sylvester(x**3 - 7, 7, x, 1).det() == 343
    )
    assert (
        sylvester(7, x**3 - 7, x).det() == sylvester(7, x**3 - 7, x, 1).det() == 343
    )
...
def test_modified_subresultants_bezout():
    ...
    assert (
        modified_subresultants_bezout(p, q, x)[-1] != sylvester(p + x**8, q, x).det()
    )
