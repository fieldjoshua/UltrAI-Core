...
from sympy.functions.elementary.hyperbolic import acoth, acsch, asech, atanh, coth, sech
from sympy.functions.elementary.miscellaneous import sqrt
from sympy.functions.elementary.trigonometric import csc, sec
from sympy.sets import FiniteSet, ImageSet, Interval, Union
...
def test_is_increasing():
    ...
    assert (
        is_increasing(4 * x**3 - 6 * x**2 - 72 * x + 30, Interval(-2, 3)) is False
    )
...
def test_is_strictly_decreasing():
    ...
    assert is_strictly_decreasing(
        1 / (x**2 - 3 * x), Interval.open(Rational(3, 2), 3)
    )
