...
from src.orchestration.config import (LLMProvider, ModelConfig, OrchestratorConfig,
                                      RequestConfig)
...
class TestBaseOrchestrator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model_config = ModelConfig(
            provider=LLMProvider.MOCK, model_id="test-model", is_primary=True
        )

        self.orch_config = OrchestratorConfig(
            models=[self.model_config], cache_enabled=False, parallel_execution=True
        )
    ...
    def test_add_model(self, mock_get_adapter):
        ...
        new_model = ModelConfig(
            provider=LLMProvider.MOCK, model_id="new-model", is_primary=False
        )
    ...
    def test_get_available_models(self, mock_get_adapter):
        ...
        model1 = ModelConfig(
            provider=LLMProvider.MOCK, model_id="model1", is_primary=True, weight=1.0
        )

        model2 = ModelConfig(
            provider=LLMProvider.MOCK, model_id="model2", is_primary=False, weight=0.5
        )

        config = OrchestratorConfig(
            models=[model1, model2], cache_enabled=False, parallel_execution=True
        )
    ...
    def test_execute_request(self, mock_get_adapter):
        ...
        self.assertEqual(
            response.model_responses[model_key]["content"], "Mock response"
        )
    ...
    def test_execute_request_with_error(self, mock_get_adapter):
        ...
        # Mock the adapter factory
        mock_adapter = MagicMock()
        mock_adapter.generate = asyncio.coroutine(
            lambda **kwargs: (_ for _ in ()).throw(Exception("Test error"))
        )
        mock_get_adapter.return_value = mock_adapter
        ...
        self.assertIsInstance(response, OrchestratorResponse)
        self.assertEqual(
            response.content, "Error: All models failed to generate responses"
        )
        self.assertEqual(len(response.model_responses), 1)
