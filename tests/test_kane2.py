from sympy import Matrix, cos, pi, sin, symbols, tan, zeros
...
def test_aux_dep():
    ...
    C.set_ang_acc(
        N, C.ang_vel_in(N).diff(t, B) + cross(B.ang_vel_in(N), C.ang_vel_in(N))
    )
    ...
    f_v = Matrix(
        [
            dot(
                O.vel(N) - (P.vel(N) + cross(C.ang_vel_in(N), O.pos_from(P))), ai
            ).expand()
            for ai in A
        ]
    )
    ...
    Fr_star_steady = (
        Fr_star_c.subs(ud_zero)
        .subs(u_dep_dict)
        .subs(steady_conditions)
        .subs({q[3]: -r * cos(q[1])})
        .expand()
    )
    ...
    assert simplify(Matrix(Fr_star_steady).expand()).xreplace({0: 0.0}) == simplify(
...
def test_non_central_inertia():
    ...
    fr_star_expected = Matrix(
        [
            -(IA + 2 * J * b**2 / R**2 + 2 * K + mA * a**2 + 2 * mB * b**2) * u1.diff(t)
            - mA * a * u1 * u2,
    ...
    for rb, I_star in zip([rbA, rbB, rbC], [inertia_A, inertia_B, inertia_C]):
        I = I_star + inertia_of_point_mass(
            rb.mass, rb.masscenter.pos_from(pD), rb.frame
        )
