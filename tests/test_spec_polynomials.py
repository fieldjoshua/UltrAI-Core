...
from sympy.core.function import ArgumentIndexError, Derivative, diff
...
def test_jacobi():
    ...
    assert conjugate(jacobi(m, a, b, x)) == jacobi(
        m, conjugate(a), conjugate(b), conjugate(x)
    )
    ...
    assert diff(jacobi(n, a, b, x), x) == (a / 2 + b / 2 + n / 2 + S.Half) * jacobi(
        n - 1, a + 1, b + 1, x
    )
...
def test_gegenbauer():
    ...
    assert gegenbauer(3, a, x) == x**3 * (
        4 * a**3 / 3 + 4 * a**2 + a * Rational(8, 3)
    ) + x * (-2 * a**2 - 2 * a)
...
def test_legendre():
    ...
    assert (
        legendre(6, x)
        == ((231 * x**6 - 315 * x**4 + 105 * x**2 - 5) / 16).expand()
    )
    ...
    assert diff(legendre(n, x), x) == n * (x * legendre(n, x) - legendre(n - 1, x)) / (
        x**2 - 1
    )
...
def test_assoc_legendre():
    ...
    assert Plm(n, m, 0) == 2**m * sqrt(pi) / (
        gamma(-m / 2 - n / 2 + S.Half) * gamma(-m / 2 + n / 2 + 1)
    )

    assert diff(Plm(m, n, x), x) == (
        m * x * assoc_legendre(m, n, x) - (m + n) * assoc_legendre(m - 1, n, x)
    ) / (x**2 - 1)
...
def test_chebyshev():
    ...
    assert diff(chebyshevu(n, x), x) == (
        -x * chebyshevu(n, x) + (n + 1) * chebyshevt(n + 1, x)
    ) / (x**2 - 1)
...
def test_hermite():
    ...
    assert hermite(n, x).rewrite(hermite_prob) == sqrt(2) ** n * hermite_prob(
        n, x * sqrt(2)
    )
...
def test_hermite_prob():
    ...
    assert hermite_prob(n, x).rewrite(hermite) == sqrt(2) ** (-n) * hermite(
        n, x / sqrt(2)
    )
...
def test_assoc_laguerre():
    ...
    assert (
        assoc_laguerre(2, alpha, x).expand()
