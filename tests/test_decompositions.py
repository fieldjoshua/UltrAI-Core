...
from sympy.matrices import Matrix, SparseMatrix, eye, zeros
...
def test_LDLdecomposition():
    ...
    raises(
        ValueError, lambda: Matrix(((1, 2), (3, 4))).LDLdecomposition(hermitian=False)
    )
    A = Matrix(((1, 5), (5, 1)))
    ...
    assert L.is_lower
    assert L == Matrix(
        [[1, 0, 0], [Rational(3, 5), 1, 0], [Rational(-1, 5), Rational(1, 3), 1]]
    )
    ...
    raises(
        ValueError,
        lambda: SparseMatrix(((1, 2), (3, 4))).LDLdecomposition(hermitian=False),
    )
