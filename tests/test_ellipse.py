...
def test_ellipse_equation_using_slope():
    ...
    assert str(e1.equation(_slope=1)) == str(
    ...
    assert str(e3.equation(_slope=2)) == str(
        (-2 * x + y - 3) ** 2 / 20 + (x + 2 * y - 11) ** 2 / 180 - 1
    )
...
def test_object_from_equation():
    ...
    assert Circle(x**2 + y**2 + 3 * x + 4 * y - 8) == Circle(
        Point2D(S(-3) / 2, -2), sqrt(57) / 2
    )
    assert Circle(x**2 + y**2 + 6 * x + 8 * y + 25) == Circle(Point2D(-3, -4), 0)
    assert Circle(a**2 + b**2 + 6 * a + 8 * b + 25, x="a", y="b") == Circle(
        Point2D(-3, -4), 0
    )
    ...
    raises(
        GeometryError, lambda: Circle(6 * (x**2) + 4 * (y**2) + 6 * x + 8 * y + 25)
    )
...
def test_ellipse_geom():
    ...
    assert Circle(Point(0, 0), Point(1, 1), Point(2, 2)) == Segment2D(
    ...
    assert (
        e1.encloses(Polygon(Point(-0.5, -0.5), Point(-0.5, 0.5), Point(0.5, 0.5)))
        is True
    )
    ...
    raises(
        ValueError, lambda: Ellipse(Point(x, y), 1, 1).arbitrary_point(parameter="x")
    )
    ...
    assert intersection(Ellipse(Point(0, 0), 2, 1), Ellipse(Point(3, 0), 1, 2)) == [
        Point(2, 0)
    ]
    ...
    assert (
        intersection(Ellipse(Point(0, 0), 5, 17), Ellipse(Point(4, 0), 0.999, 0.2))
        == []
    )
    ...
    c = sqrt(3991)
    ans = [
        Point(-c / 68 + a, c * Rational(2, 17) + a / 2),
        Point(c / 68 + a, c * Rational(-2, 17) + a / 2),
    ]
...
def test_construction():
    ...
    raises(
        GeometryError, lambda: Ellipse(Point(3, 1), hradius=3, eccentricity=S(3) / 2)
    )
    raises(GeometryError, lambda: Ellipse(Point(3, 1), hradius=3, eccentricity=sec(5)))
    raises(
        GeometryError, lambda: Ellipse(Point(3, 1), hradius=3, eccentricity=S.Pi - S(2))
    )
...
def test_transform():
    ...
    assert Circle(Point(-8, -10), 6).scale(
        Rational(1, 3), Rational(1, 3), (4, 5)
    ) == Circle((0, 0), 2)
...
def test_section_modulus_and_polar_second_moment_of_area():
    ...
    assert e.polar_second_moment_of_area() == pi * a**3 * b / S(
        4
    ) + pi * a * b**3 / S(4)
...
def test_circumference():
    M = Symbol("M")
    m = Symbol("m")
    assert Ellipse(Point(0, 0), M, m).circumference == 4 * M * elliptic_e(
        (M**2 - m**2) / M**2
    )
    ...
    assert (
        abs(
            Ellipse(None, hradius=5, vradius=3).circumference.evalf(16)
            - 25.52699886339813
        )
        < 1e-10
    )
