...
def test_arrayexpr_derivatives1():
    ...
    assert res == _permute_dims(
        ArrayTensorProduct(I, A, I, B), [0, 4, 2, 3, 1, 5, 6, 7]
    )
    ...
    assert res.dummy_eq(
        ArrayDiagonal(
            ArrayTensorProduct(ElementwiseApplyFunction(cos, X), I, I), (0, 3), (1, 5)
        )
    )
    ...
    assert res.dummy_eq(
        ArrayDiagonal(
            ArrayTensorProduct(I, I, ArrayElementwiseApplyFunc(cos, X)), (1, 4), (3, 5)
        )
    )

    res = array_derive(A1, A1)
    assert res == PermuteDims(
        ArrayTensorProduct(Identity(3), Identity(2), Identity(k)), [0, 2, 4, 1, 3, 5]
    )
    ...
    assert res == Reshape(
        PermuteDims(ArrayTensorProduct(I, I), [0, 2, 1, 3]), (k, k, k**2)
    )
