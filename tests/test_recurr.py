    ...
    for p in funcs:
        ...
        for c in coeffs:
            ...
            if p.is_hypergeometric(n) and len(c) <= 3:
                assert (
                    rsolve_hyper(c, q, n).subs(zip(symbols("C:3"), [0, 0, 0])).expand()
                    == p
                )
...
def test_rsolve():
    ...
    assert (
        str(sol) == "C0*((-sqrt(1 - a**2) - 1)/a)**n + C1*((sqrt(1 - a**2) - 1)/a)**n"
    )
...
def test_diofantissue_294():
    ...
    assert rsolve(f, y(n), {y(0): -1, y(1): 1}) == (
...
def test_issue_15553():
    ...
    assert (
        rsolve(Eq(f(n + 1), 2 * f(n) + n**2 + 1), f(n))
        == 2**n * C0 - n**2 - 2 * n - 4
    )
    assert (
        rsolve(Eq(f(n + 1), 2 * f(n) + n**2 + 1), f(n), {f(1): 0})
        == 7 * 2**n / 2 - n**2 - 2 * n - 4
    )
    assert (
        rsolve(Eq(f(n), 2 * f(n - 1) + 3 * n**2), f(n))
        == 2**n * C0 - 3 * n**2 - 12 * n - 18
    )
    assert (
        rsolve(Eq(f(n), 2 * f(n - 1) + n**2), f(n))
        == 2**n * C0 - n**2 - 4 * n - 6
    )
