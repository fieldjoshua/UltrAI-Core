...
from sympy.core.function import ArgumentIndexError, diff, expand, expand_func
...
def test_erf():
    ...
    assert (
        limit(((1 - erf(x)) * exp(x**2) * sqrt(pi) * x - 1) * 2 * x**2, x, oo) == -1
    )
...
def test_erfc():
    ...
    assert (
        erfc(z).rewrite("uppergamma") == 1 - sqrt(z**2) * (1 - erfc(sqrt(z**2))) / z
    )
...
def mytn(expr1, expr2, expr3, x, d=0):
    from sympy.core.random import random_complex_number, verify_numerically
...
def mytd(expr1, expr2, x):
    from sympy.core.random import random_complex_number, test_derivative_numerically
...
def test_si():
    ...
    assert (
        Si(1 / x)._eval_as_leading_term(x, cdir=1)
        == Si(1 / x)._eval_as_leading_term(x, cdir=-1)
        == Si(1 / x)
    )

    assert Si(x).nseries(x, n=8) == x - x**3 / 18 + x**5 / 600 - x**7 / 35280 + O(
        x**8
    )
...
def test_integral_rewrites():  # issues 26134, 26144, 26306
    ...
    assert (
        fresnels(x)
        .rewrite(Integral)
        .dummy_eq(Integral(sin(pi * t**2 / 2), (t, 0, x)))
    )
    assert (
        fresnelc(x)
        .rewrite(Integral)
        .dummy_eq(Integral(cos(pi * t**2 / 2), (t, 0, x)))
    )
