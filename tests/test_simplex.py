...
from sympy.core.random import choice, random
...
from sympy.solvers.simplex import InfeasibleLPError, UnboundedLPError, _abcd
from sympy.solvers.simplex import _lp as lp
from sympy.solvers.simplex import _m, _primal_dual, _simplex, linprog, lpmax, lpmin
...
def test_lp():
    ...
    def make_random_problem(nvar=2, num_constraints=2, sparsity=0.1):
        ...
        constraints = [
            (sum(rand() * x for x in variables) <= rand())
...
def test_simplex():
    ...
    lpans = lpmin(
        f,
        cond + [x1 >= 0, x1 <= 1, x2 >= 0, x2 <= 1, x3 >= 0, x3 <= 1, x4 >= 0, x4 <= 1],
    )
