...
def test_collect_4():
    ...
    assert collect(a * x ** (2 * c) + b * x ** (2 * c), x**c) == x ** (2 * c) * (
        a + b
    )
...
def test_collect_5():
    ...
    assert collect(
        x**2 * y**4 + z * (x * y**2) ** 2 + z + a * z, [x * y**2, z]
    ) in [
...
def test_collect_pr19431():
    ...
    a = symbols("a")
    assert collect(a**2 * (a**2 + 1), a**2, evaluate=False)[a**2] == (
        a**2 + 1
    )
...
def test_collect_order():
    ...
    assert collect(t + t * x + t * x**2 + O(x**3), t) == t * (
        1 + x + x**2 + O(x**3)
    )
