...
from sympy.sets import FiniteSet, ImageSet, Intersection, Interval, Union
...
def test_FiniteSet_FiniteSet():
    assert (SetExpr(FiniteSet(1, 2, 3)) + SetExpr(FiniteSet(1, 2))).set == FiniteSet(
        2, 3, 4, 5
    )
    assert (SetExpr(FiniteSet(1, 2, 3)) * SetExpr(FiniteSet(1, 2))).set == FiniteSet(
        1, 2, 3, 4, 6
...
def test_SetExpr_Intersection():
    ...
    assert exp(se).set == Intersection(
        ImageSet(Lambda(x, exp(x)), set1), ImageSet(Lambda(x, exp(x)), set2)
    )
...
def test_SetExpr_Interval_pow():
    ...
    assert SetExpr(Interval(1, 2)) ** Rational(5, 2) == SetExpr(
        Interval(1, 4 * sqrt(2))
    )
    ...
    assert SetExpr(Interval(Rational(-1, 3), Rational(-1, 4))) ** oo == SetExpr(
        FiniteSet(0)
    )
