...
def test_trigintegrate_odd():
    ...
    assert (
        trigintegrate(sin(x) ** 7 * cos(x) ** 3, x)
        == -sin(x) ** 10 / 10 + sin(x) ** 8 / 8
    )
    assert (
        trigintegrate(sin(x) ** 3 * cos(x) ** 7, x)
        == cos(x) ** 10 / 10 - cos(x) ** 8 / 8
    )

    # both n, m are odd and -ve, and not necessarily equal
    assert trigintegrate(sin(x) ** -1 * cos(x) ** -1, x) == -log(
        sin(x) ** 2 - 1
    ) / 2 + log(sin(x))
...
def test_trigintegrate_even():
    ...
    assert (
        trigintegrate(sin(x) ** 2 * cos(x) ** 2, x)
    ...
    assert trigintegrate(sin(x) ** (-4), x) == -2 * cos(x) / (3 * sin(x)) - cos(x) / (
        3 * sin(x) ** 3
    )

    assert trigintegrate(cos(x) ** (-6), x) == sin(x) / (5 * cos(x) ** 5) + 4 * sin(
        x
    ) / (15 * cos(x) ** 3) + 8 * sin(x) / (15 * cos(x))
...
def test_trigintegrate_mixed():
    ...
    assert trigintegrate(cos(x) * cot(x), x) == log(cos(x) - 1) / 2 - log(
        cos(x) + 1
    ) / 2 + cos(x)
