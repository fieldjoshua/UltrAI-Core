...
from sympy.functions.elementary.trigonometric import acos, cos, sin, tan
...
def test_angle_between():
    ...
    assert feq(
        Line.angle_between(
            Line(Point(0, 0), Point(1, 1)), Line(Point(0, 0), Point(5, 0))
        ).evalf(),
        pi.evalf() / 4,
    )
    assert Line(a, o).angle_between(Line(b, o)) == pi / 2
    z = Point3D(0, 0, 0)
    assert Line3D.angle_between(
        Line3D(z, Point3D(1, 1, 1)), Line3D(z, Point3D(5, 0, 0))
    ) == acos(sqrt(3) / 3)
    # direction of points is used to determine angle
    assert Line3D.angle_between(
        Line3D(z, Point3D(1, 1, 1)), Line3D(Point3D(5, 0, 0), z)
    ) == acos(-sqrt(3) / 3)
...
def test_arbitrary_point():
    ...
    assert Ray3D((1, 1, 1), direction_ratio=[1, 2, 3]).arbitrary_point() == Point3D(
        t + 1, 2 * t + 1, 3 * t + 1
    )
    assert Segment3D(Point3D(0, 0, 0), Point3D(1, 1, 1)).midpoint == Point3D(
        S.Half, S.Half, S.Half
    )
    assert Segment3D(Point3D(x1, x1, x1), Point3D(y1, y1, y1)).length == sqrt(3) * sqrt(
        (x1 - y1) ** 2
    )
    assert Segment3D((1, 1, 1), (2, 3, 4)).arbitrary_point() == Point3D(
        t + 1, 2 * t + 1, 3 * t + 1
    )
...
def test_are_concurrent_3d():
    ...
    assert (
        Line3D.are_concurrent(l1, Line(Point3D(x1, x1, x1), Point3D(y1, y1, y1)))
        is False
    )
    assert (
        Line3D.are_concurrent(
            l1,
            Line3D(p1, Point3D(x1, x1, x1)),
            Line(Point3D(x1, x1, x1), Point3D(x1, 1 + x1, 1)),
        )
        is True
    )
...
def test_basic_properties_2d():
    ...
    assert (
        Segment(Point(0, -1), Segment(p1, Point(0, 1)).random_point()).slope
        == Segment(p1, Point(0, 1)).slope
    )
...
def test_basic_properties_3d():
    ...
    assert Line3D((1, 1, 1), direction_ratio=[2, 3, 4]) == Line3D(
        Point3D(1, 1, 1), Point3D(3, 4, 5)
    )
    assert Line3D((1, 1, 1), direction_ratio=[1, 5, 7]) == Line3D(
        Point3D(1, 1, 1), Point3D(2, 6, 8)
    )
    assert Line3D((1, 1, 1), direction_ratio=[1, 2, 3]) == Line3D(
        Point3D(1, 1, 1), Point3D(2, 3, 4)
    )
    ...
    assert Ray3D(Line3D(Point3D(0, 0, 0), Point3D(1, 0, 0))) == Ray3D(
        p1, Point3D(1, 0, 0)
    )
    ...
    assert r3 != r1
    assert Ray3D(Point3D(0, 0, 0), Point3D(1, 1, 1)) in Ray3D(
        Point3D(0, 0, 0), Point3D(2, 2, 2)
    )
    assert Ray3D(Point3D(0, 0, 0), Point3D(2, 2, 2)) in Ray3D(
...
def test_contains():
    ...
    assert (
        Segment3D(Point3D(0, 0, 0), Point3D(1, 0, 0)).contains(
            Segment3D(Point3D(2, 2, 2), Point3D(3, 2, 2))
        )
        is False
    )
    # Line contains
...
def test_equals():
    ...
    assert (
        Line3D(Point3D(0, 0, 0), Point3D(1, 0, 0)).equals(
            Line3D(Point3D(-5, 0, 0), Point3D(-1, 0, 0))
        )
        is True
    )
    assert (
        Line3D(Point3D(0, 0, 0), Point3D(1, 0, 0)).equals(Line3D(p1, Point3D(0, 1, 0)))
        is False
    )
    ...
    assert (
        Line3D((0, 0), (t, t))
        .perpendicular_segment(Point(0, 1, 0))
        .equals(Segment3D((0, 1), (S.Half, S.Half)))
    )
    assert Line3D(p1, Point3D(0, 1, 0)).equals(Point(1.0, 1.0)) is False
...
def test_intersection_2d():
    ...
    assert (
        intersection(
            Segment(Point(-10, 10), Point(10, 10)), Segment(Point(-5, -5), Point(-5, 5))
        )
        == []
    )
    assert intersection(r2, l3) == [r2]
    assert intersection(r1, Ray(Point(2, 2), Point(0, 0))) == [
        Segment(Point(1, 1), Point(2, 2))
    ]
    assert intersection(r1, Ray(Point(1, 1), Point(-1, -1))) == [Point(1, 1)]
    assert intersection(r1, Segment(Point(0, 0), Point(2, 2))) == [
        Segment(Point(1, 1), Point(2, 2))
    ]
    ...
    assert Ray3D((0, 0), (3, 0)).intersection(Ray3D((1, 0), (3, 0))) == [
        Ray3D((1, 0), (3, 0))
    ]
    assert Ray3D((1, 0), (3, 0)).intersection(Ray3D((0, 0), (3, 0))) == [
        Ray3D((1, 0), (3, 0))
    ]
    assert Ray(Point(0, 0), Point(0, 4)).intersection(
        Ray(Point(0, 1), Point(0, -1))
    ) == [Segment(Point(0, 0), Point(0, 1))]

    assert Segment3D((0, 0), (3, 0)).intersection(Segment3D((1, 0), (2, 0))) == [
        Segment3D((1, 0), (2, 0))
    ]
    assert Segment3D((1, 0), (2, 0)).intersection(Segment3D((0, 0), (3, 0))) == [
        Segment3D((1, 0), (2, 0))
    ]
    assert Segment3D((0, 0), (3, 0)).intersection(Segment3D((3, 0), (4, 0))) == [
        Point3D((3, 0))
    ]
    assert Segment3D((0, 0), (3, 0)).intersection(Segment3D((2, 0), (5, 0))) == [
        Segment3D((2, 0), (3, 0))
    ]
    assert Segment3D((0, 0), (3, 0)).intersection(Segment3D((-2, 0), (1, 0))) == [
        Segment3D((0, 0), (1, 0))
    ]
    assert Segment3D((0, 0), (3, 0)).intersection(Segment3D((-2, 0), (0, 0))) == [
        Point3D(0, 0)
    ]
    assert s1.intersection(Segment(Point(1, 1), Point(2, 2))) == [Point(1, 1)]
    assert s1.intersection(Segment(Point(0.5, 0.5), Point(1.5, 1.5))) == [
        Segment(Point(0.5, 0.5), p2)
    ]
...
def test_intersection_3d():
    ...
    assert intersection(r1, Ray3D(Point3D(1, 1, 1), Point3D(-1, -1, -1))) == [
        Point3D(1, 1, 1)
    ]
    ...
    assert intersection(
        Ray3D(Point3D(1, 0, 0), Point3D(-1, 0, 0)),
        Ray3D(Point3D(0, 1, 0), Point3D(0, -1, 0)),
    ) == [Point3D(0, 0, 0)]
    ...
    assert Line3D(Point3D(4, 0, 1), Point3D(0, 4, 1)).intersection(
        Line3D(Point3D(0, 0, 1), Point3D(4, 4, 1))
    ) == [Point3D(2, 2, 1)]
    assert Line3D((0, 1, 2), (0, 2, 3)).intersection(Line3D((0, 1, 2), (0, 1, 1))) == [
        Point3D(0, 1, 2)
    ]
    ...
    assert (
        Ray3D(Point3D(0, 0, 0), Point3D(0, 4, 0)).intersection(
...
def test_is_parallel():
    ...
    assert (
        Line3D(Point3D(4, 0, 1), Point3D(0, 4, 1)).is_parallel(
            Line3D(Point3D(0, 0, 1), Point3D(4, 4, 1))
        )
        is False
    )
    ...
    assert Line3D(p1, p2).parallel_line(p3.args) == Line3D(
        Point3D(x1, x1, x1), Point3D(x1 + 1, x1 + 1, x1 + 1)
    )
    assert (
        Line3D(Point3D(4, 0, 1), Point3D(0, 4, 1)).is_parallel(
...
def test_is_perpendicular():
    ...
    assert (
        Line3D.is_perpendicular(
            ...
            Line3D(Point3D(0, 0, 0), Point3D(0, 1, 0)),
        )
        is True
    )
    assert (
        Line3D.is_perpendicular(
            Line3D(Point3D(0, 0, 0), Point3D(1, 0, 0)),
            Line3D(Point3D(0, 1, 0), Point3D(1, 1, 0)),
        )
        is False
    )
    assert (
        Line3D.is_perpendicular(
            Line3D(Point3D(0, 0, 0), Point3D(1, 1, 1)),
            Line3D(Point3D(x1, x1, x1), Point3D(y1, y1, y1)),
        )
        is False
    )
...
def test_projection():
    ...
    assert Line(Point(x1, x1), Point(x1, 1 + x1)).projection(Point(1, 1)) == Point(
        x1, 1
    )
    assert Segment(Point(-2, 2), Point(0, 4)).projection(r1) == Segment(
        Point(-1, 3), Point(0, 4)
    )
    assert Segment(Point(0, 4), Point(-2, 2)).projection(r1) == Segment(
        Point(0, 4), Point(-1, 3)
    )
    ...
    assert r1.projection(Ray(Point(0, 4), Point(-1, -5))) == Segment(
        Point(1, 1), Point(2, 2)
    )
...
def test_perpendicular_bisector():
    ...
    on_line = Segment(
        Point(S.Half, S.Half), Point(Rational(3, 2), Rational(-1, 2))
    ).midpoint
...
def test_raises():
    ...
    raises(
        GeometryError,
        lambda: Line(Point(0, 0), Point(1, 0)).projection(Circle(Point(0, 0), 1)),
    )
...
def test_ray_generation():
    ...
    assert Ray((1, 1), angle=4.02 * pi) == Ray(
        Point(1, 1), Point(2, 1 + tan(4.02 * pi))
    )
    assert Ray((1, 1), angle=5) == Ray((1, 1), (2, 1 + tan(5)))

    assert Ray3D((1, 1, 1), direction_ratio=[4, 4, 4]) == Ray3D(
        Point3D(1, 1, 1), Point3D(5, 5, 5)
    )
    assert Ray3D((1, 1, 1), direction_ratio=[1, 2, 3]) == Ray3D(
        Point3D(1, 1, 1), Point3D(2, 3, 4)
    )
    assert Ray3D((1, 1, 1), direction_ratio=[1, 1, 1]) == Ray3D(
