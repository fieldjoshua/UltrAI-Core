...
from sympy.core.function import ArgumentIndexError, Subs, expand_func
...
def test_gamma():
    ...
    assert gamma(Rational(-11, 8)).expand(func=True) == Rational(64, 33) * gamma(
        Rational(5, 8)
    )
    assert gamma(Rational(-10, 3)).expand(func=True) == Rational(81, 280) * gamma(
        Rational(2, 3)
    )
    assert gamma(Rational(14, 3)).expand(func=True) == Rational(880, 81) * gamma(
        Rational(2, 3)
    )
    assert gamma(Rational(17, 7)).expand(func=True) == Rational(30, 49) * gamma(
        Rational(3, 7)
    )
    assert gamma(Rational(19, 8)).expand(func=True) == Rational(33, 64) * gamma(
        Rational(3, 8)
    )
...
def test_gamma_series():
    assert gamma(x + 1).series(x, 0, 3) == 1 - EulerGamma * x + x**2 * (
...
def test_lowergamma():
    ...
    assert lowergamma(x, 2).series(x, oo, 3) == 2**x * (1 + 2 / (x + 1)) * exp(
        -2
    ) / x + O(exp(x * log(2)) / x**3, (x, oo))
...
def test_polygamma():
    ...
    assert polygamma(n5, x).rewrite(zeta) == (-1) ** (n5 + 1) * factorial(n5) * zeta(
        n5 + 1, x
    )
    ...
    assert polygamma(k, exp_polar(2 * I * pi) * x).args == (
        k,
        exp_polar(2 * I * pi) * x,
    )
    ...
    assert (I / polygamma(I, 4)).rewrite(exp) == I * exp(
        -I * atan(im(polygamma(I, 4)) / re(polygamma(I, 4)))
    ) / Abs(polygamma(I, 4))

    # issue 12569
    assert unchanged(im, polygamma(0, I))
    assert (
        polygamma(Symbol("a", positive=True), Symbol("b", positive=True)).is_real
        is True
    )
...
def test_polygamma_expand_func():
    assert polygamma(0, x).expand(func=True) == polygamma(0, x)
    assert polygamma(0, 2 * x).expand(func=True) == polygamma(0, x) / 2 + polygamma(
        0, S.Half + x
    ) / 2 + log(2)
    assert (
        polygamma(1, 2 * x).expand(func=True)
    ...
    assert polygamma(0, 2 + x).expand(func=True) == 1 / x + 1 / (1 + x) + polygamma(
        0, x
    )
    assert polygamma(0, 3 + x).expand(func=True) == polygamma(0, x) + 1 / x + 1 / (
        1 + x
    ) + 1 / (2 + x)
    assert polygamma(0, 4 + x).expand(func=True) == polygamma(0, x) + 1 / x + 1 / (
        1 + x
    ) + 1 / (2 + x) + 1 / (3 + x)
    assert polygamma(1, 1 + x).expand(func=True) == polygamma(1, x) - 1 / x**2
    assert (
        polygamma(1, 2 + x).expand(func=True, multinomial=False)
    ...
    assert (
        polygamma(-1, x, evaluate=False).expand(func=True)
...
def test_trigamma():
    ...
    assert trigamma(x, evaluate=False).rewrite(harmonic) == trigamma(x).rewrite(
        polygamma
    ).rewrite(harmonic)
...
def test_loggamma():
    ...
    assert loggamma(n / 2) == log(
        2 ** (-n + 1) * sqrt(pi) * gamma(n) / gamma(n / 2 + S.Half)
    )
...
def test_polygamma_expansion():
    # A. & S., pa. 259 and 260
    assert polygamma(0, 1 / x).nseries(x, n=3) == -log(x) - x / 2 - x**2 / 12 + O(
        x**3
    )
    assert polygamma(1, 1 / x).series(x, n=5) == x + x**2 / 2 + x**3 / 6 + O(x**5)
    assert polygamma(3, 1 / x).nseries(
        x, n=11
