...
from sympy.series.gruntz import compare, gruntz, mrv, mrv_leadterm, rewrite, sign
...
def test_gruntz_evaluation():
    ...
    assert (
        gruntz(
            exp(x * exp(-x) / (exp(-x) + exp(-2 * x**2 / (x + 1)))) / exp(x), x, oo
        )
        == 1
    )
    # 8.19
...
def test_gruntz_I():
    ...
    assert (
        gruntz(y * 3 * sin(I) * x, x, oo).simplify().rewrite(_sign) == _sign(y) * I * oo
    )
