...
def test_cg_simp_add():
    ...
    assert cg_simp(a - b + c - d + e) == sqrt(2) * KroneckerDelta(j, 0) + sqrt(
        3
    ) * KroneckerDelta(j, 0)
    ...
    # numerical
    a = CG(S.Half, S(1) / 2, S.Half, Rational(-1, 2), 1, 0) * CG(
        S.Half, Rational(-1, 2), S.Half, S.Half, 1, 0
    )
    b = CG(S.Half, S(1) / 2, S.Half, Rational(-1, 2), 0, 0) * CG(
        S.Half, Rational(-1, 2), S.Half, S.Half, 0, 0
    )
    ...
    assert cg_simp(a + b + c + d + e + f + g + h + i) == KroneckerDelta(
        m1, m1p
    ) * KroneckerDelta(m2, m2p)
...
def test_cg_simp_sum():
    ...
    assert cg_simp(x * Sum(CG(a, alpha, b, 0, a, alpha), (alpha, -a, a))) == x * (
        2 * a + 1
    ) * KroneckerDelta(b, 0)
    assert cg_simp(
        x * Sum(CG(a, alpha, b, 0, a, alpha), (alpha, -a, a)) + CG(1, 0, 1, 0, 1, 0)
    ) == x * (2 * a + 1) * KroneckerDelta(b, 0) + CG(1, 0, 1, 0, 1, 0)
    assert cg_simp(2 * Sum(CG(1, alpha, 0, 0, 1, alpha), (alpha, -1, 1))) == 6
    # Varshalovich 8.7.1 Eq 2
    assert cg_simp(
        x * Sum((-1) ** (a - alpha) * CG(a, alpha, a, -alpha, c, 0), (alpha, -a, a))
    ) == x * sqrt(2 * a + 1) * KroneckerDelta(c, 0)
    assert cg_simp(
        3 * Sum((-1) ** (2 - alpha) * CG(2, alpha, 2, -alpha, 0, 0), (alpha, -2, 2))
    ) == 3 * sqrt(5)
    # Varshalovich 8.7.2 Eq 4
    assert cg_simp(
        Sum(
            CG(a, alpha, b, beta, c, gamma) * CG(a, alpha, b, beta, cp, gammap),
            (alpha, -a, a),
            (beta, -b, b),
        )
...
def test_doit():
    assert (
        Wigner3j(S.Half, Rational(-1, 2), S.Half, S.Half, 0, 0).doit() == -sqrt(2) / 2
    )
    ...
    assert (
        Wigner9j(2, 1, 1, Rational(3, 2), S.Half, 1, S.Half, S.Half, 0).doit()
        == sqrt(2) / 12
    )
