...
from sympy.matrices import Determinant, Matrix, MatrixSet, MatrixSymbol, Trace
...
def test_MatrixNormal():
    ...
    raises(
        ValueError,
        lambda: MatrixNormal("M", [1, 2], [[1, 0], [0, 1]], [[1, 0], [2, 1]]),
    )
    raises(
        ValueError,
        lambda: MatrixNormal("M", [1, 2], [[1, 0], [2, 1]], [[1, 0], [0, 1]]),
    )
    raises(
        ValueError,
        lambda: MatrixNormal("M", [1, 2], [[1, 0], [0, 1]], [[1, 0], [0, 1]]),
    )
    raises(
        ValueError, lambda: MatrixNormal("M", [1, 2], [[1, 0], [2]], [[1, 0], [0, 1]])
    )
    raises(
        ValueError, lambda: MatrixNormal("M", [1, 2], [[1, 0], [2, 1]], [[1, 0], [0]])
    )
...
def test_MatrixStudentT():
    ...
    exprd = (
        ...
        * Determinant(
            Matrix([[1]])
            + Sigma ** (-1) * (-Location + Y) * Omega ** (-1) * (-Location.T + Y.T)
        )
        ** (-v / 2 - 1)
    ...
    raises(
        ValueError,
        lambda: MatrixStudentT("M", 1, [1, 2], [[1, 0], [0, 1]], [[1, 0], [2, 1]]),
    )
    raises(
        ValueError,
        lambda: MatrixStudentT("M", 1, [1, 2], [[1, 0], [2, 1]], [[1, 0], [0, 1]]),
    )
    raises(
        ValueError,
        lambda: MatrixStudentT("M", 1, [1, 2], [[1, 0], [0, 1]], [[1, 0], [0, 1]]),
    )
    raises(
        ValueError,
        lambda: MatrixStudentT("M", 1, [1, 2], [[1, 0], [2]], [[1, 0], [0, 1]]),
    )
    raises(
        ValueError, lambda: MatrixStudentT("M", 1, [1, 2], [[1, 0], [2, 1]], [[1], [2]])
    )
    raises(
        ValueError, lambda: MatrixStudentT("M", 1, [[1, 2]], [[1, 0], [0, 1]], [[1, 0]])
    )
