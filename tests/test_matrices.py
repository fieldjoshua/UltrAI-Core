...
def test_matrix_transform():
    ...
    swap_row(l, 0, 1)
    assert l == [2, 1]
    assert extend(eye(3), [1, 2, 3]) == matrix(
...
def test_interval_matrix_mult_bug():
    ...
    x = convert(
        "1.00000000000001"
    )  # note: this is implicitly rounded to some near mpf float value
    A = matrix([[x]])
    B = iv.matrix(A)
    C = iv.matrix([[x]])
    assert B == C
    ...
    assert (
        mp.mpf("1.00000000000001998401444325291756783368705994138804689654") in B[0, 0]
    )
    assert (
        mp.mpf("1.00000000000001998401444325291756783368705994138804689654") in C[0, 0]
    )
