...
from sympy.core import I, S, symbols
...
def test_cyclic_convolution():
    ...
    assert convolution(a, b, prime=19 * 2**10 + 1, cycle=5) == [
        96,
        17146,
        2664,
        15534,
    ...
    assert convolution(a, b, prime=19 * 2**10 + 1, cycle=9) == convolution(
        a, b, prime=19 * 2**10 + 1
    ) + [0]
...
def test_convolution_fft():
    assert all(
        convolution_fft([], x, dps=y) == [] for x in ([], [1]) for y in (None, 3)
    )
    ...
    assert convolution_fft(
        [1 + 2 * I, 3 + 4 * I, 5 + 3 * I / 5], [Rational(2, 5) + 4 * I / 7]
    ) == [
    ...
    assert convolution_fft(
        ...
        [Rational(7, 8), Rational(1, 3), Rational(2, 5)],
    ) == [Rational(21, 32), Rational(47, 48), Rational(26, 45), Rational(1, 3)]

    assert convolution_fft(
        [Rational(1, 9), Rational(2, 3), Rational(3, 5)],
...
def test_convolution_ntt():
    ...
    assert all(
        convolution_ntt([], x, prime=y) == [] for x in ([], [1]) for y in (p, q, r)
    )
    ...
    assert convolution_ntt(
        [121913, 3171831, 31888131, 12], [17882, 21292, 29921, 312], q
    ...
    assert convolution_ntt(
        [12, 19, 21, 98, 67], [21, 76, 17, 78, 69], p
...
def test_convolution_fwht():
    ...
    assert convolution_fwht(
        [Rational(5, 7), Rational(6, 8), Rational(7, 3)], [2, 4, Rational(6, 7)]
...
def test_convolution_subset():
    ...
    assert convolution_subset([u, v], [x, y, z]) == convolution_subset(
        [x, y, z], [u, v]
    )
...
def test_intersecting_product():
    ...
    assert intersecting_product([u, v], [x, y, z]) == intersecting_product(
        [x, y, z], [u, v]
    )
...
def test_convolution_int():
    ...
    assert convolution_int(
        [-341, -5, 1, 3, -71, -99, 43, 87], [5, 6, 7, 12, 345, 21, -78, -7, -89]
    ) == [
        -1705,
        -2071,
        -2412,
