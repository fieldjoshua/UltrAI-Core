...
def test_tensor_product_adjoint():
    assert KroneckerProduct(I * A, B).adjoint() == -I * KroneckerProduct(
        A.adjoint(), B.adjoint()
    )
    assert KroneckerProduct(mat1, mat2).adjoint() == kronecker_product(
        mat1.adjoint(), mat2.adjoint()
    )


def test_tensor_product_conjugate():
    assert KroneckerProduct(I * A, B).conjugate() == -I * KroneckerProduct(
        A.conjugate(), B.conjugate()
    )
    assert KroneckerProduct(mat1, mat2).conjugate() == kronecker_product(
        mat1.conjugate(), mat2.conjugate()
    )


def test_tensor_product_transpose():
    assert KroneckerProduct(I * A, B).transpose() == I * KroneckerProduct(
        A.transpose(), B.transpose()
    )
    assert KroneckerProduct(mat1, mat2).transpose() == kronecker_product(
        mat1.transpose(), mat2.transpose()
    )
...
def test_KroneckerProduct_combine_pow():
    ...
    assert combine_kronecker(KroneckerProduct(X, Y) ** x) == KroneckerProduct(
        X**x, Y**x
    )
    assert combine_kronecker(x * KroneckerProduct(X, Y) ** 2) == x * KroneckerProduct(
        X**2, Y**2
    )
    assert combine_kronecker(
        x * (KroneckerProduct(X, Y) ** 2) * KroneckerProduct(A, B)
    ) == x * KroneckerProduct(X**2 * A, Y**2 * B)
    # cannot simplify because of non-square arguments to kronecker product:
    assert (
        combine_kronecker(KroneckerProduct(A, B.T) ** m)
        == KroneckerProduct(A, B.T) ** m
    )
...
def test_KroneckerProduct_expand():
    ...
    assert KroneckerProduct(X + Y, Y + Z).expand(
        kroneckerproduct=True
    ) == KroneckerProduct(X, Y) + KroneckerProduct(X, Z) + KroneckerProduct(
        Y, Y
    ) + KroneckerProduct(
        Y, Z
    )
...
def test_KroneckerProduct_entry():
    ...
    assert (
        KroneckerProduct(A, B)._entry(i, j)
        == A[Mod(floor(i / o), n), Mod(floor(j / p), m)] * B[Mod(i, o), Mod(j, p)]
    )
