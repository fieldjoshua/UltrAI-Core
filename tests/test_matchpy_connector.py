...
from sympy.utilities.matchpy_connector import Replacer, WildDot, WildPlus, WildStar
...
def test_matchpy_optional():
    ...
    from matchpy import ManyToOneReplacer, Pattern, ReplacementRule, Substitution
...
def _perform_test_replacer(info, lambdify):
    ...
    replacer.add(
        Eq(x1_, x2_),
        Eq(x1_ - x2_, 0),
        conditions_nonfalse=[Ne(x2_, 0), Ne(x1_, 0), Ne(x1_, x), Ne(x2_, x)],
        info=1,
    )
    ...
    assert replacer.replace(Eq(x**2 + 4 * y * x + 4 * y**2, 0)) == g(
        Eq(x, -2 * y), [3]
    )
