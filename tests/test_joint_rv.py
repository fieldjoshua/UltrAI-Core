...
from sympy.core.numbers import Rational, comp, oo, pi
...
def test_Normal():
    ...
    assert density(N)(0, 0) == exp(-((4 * x + y) / (2 * x * y))) / (
        2 * pi * sqrt(x * y)
    )
    ...
    eval_a = (
        density_X(obs)
        .subs({Sg: eye(3), mu: Matrix([0, 0, 0]), obs: Matrix([0, 0, 0])})
        .doit()
    )
...
def test_MultivariateEwens():
    ...
    assert marginal_distribution(ed, ed[1])(a[1]) == Piecewise(
        (
            ...
            * 2 ** (-a[1])
            * theta ** a[1]
            / ((theta + 1) * (theta + 2) * factorial(a[1])),
            Eq(2 * a[1] + 1, 3),
        ),
        (0, True),
    )
    raises(ValueError, lambda: MultivariateEwens("e1", 5, theta_f))
    assert ed.pspace.distribution.set == ProductSet(
        Range(0, 4, 1), Range(0, 2, 1), Range(0, 2, 1)
    )
    ...
    den = Piecewise(
        (
            factorial(n)
            * Product(
                theta ** a[j] * (j + 1) ** (-a[j]) / factorial(a[j]), (j, 0, n - 1)
            )
...
def test_Multinomial():
    ...
    assert density(M)(x1, x2, x3, x4) == Piecewise(
        (
            p1**x1 * p2**x2 * p3**x3 * p4**x4 * f(n) / (f(x1) * f(x2) * f(x3) * f(x4)),
            Eq(n, x1 + x2 + x3 + x4),
        ),
        (0, True),
    )
    assert (
        marginal_distribution(C, C[0])(x1).subs(x1, 1)
...
def test_NegativeMultinomial():
    ...
    assert N.pspace.distribution.set == ProductSet(
        Range(0, oo, 1), Range(0, oo, 1), Range(0, oo, 1), Range(0, oo, 1)
    )
...
def test_JointRV():
    ...
    assert marginal_distribution(X, 0)(2) == sqrt(2) * exp(Rational(-1, 2)) / (
        2 * sqrt(pi)
    )
