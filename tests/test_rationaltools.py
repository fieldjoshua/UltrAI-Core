...
def test_ratint():
    ...
    assert ratint(f / g, x) == -4 * log(x + 1) + 4 * log(x - 2) + (12 * x + 6) / (
        x**2 - 1
    )
    ...
    assert ratint(f / g, x) == atan(x) + atan(x**3) + atan(
        x / 2 - Rational(3, 2) * x**3 + S.Half * x**5
    )

    f = x**7 - 24 * x**4 - 4 * x**2 + 8 * x - 8
    g = x**8 + 6 * x**6 + 12 * x**4 + 8 * x**2

    assert ratint(f / g, x) == (4 + 6 * x + 8 * x**2 + 3 * x**3) / (
        4 * x + 4 * x**3 + x**5
    ) + log(x)

    assert ratint((x**3 * f) / (x * g), x) == -(
        12 - 16 * x + 6 * x**2 - 14 * x**3
    ) / (4 + 4 * x**2 + x**4) - 5 * sqrt(2) * atan(
        x * sqrt(2) / 2
    ) + S.Half * x**2 - 3 * log(
        2 + x**2
    )
    ...
    assert (
        ...
        == -log(1 - x + x**2) / 6
        + log(1 + x) / 3
        + sqrt(3) * atan(-sqrt(3) / 3 + 2 * x * sqrt(3) / 3) / 3
    )
    ...
    assert ratint(1 / (x**3 + 1), x, real=False) == log(1 + x) / 3 + (
        Rational(-1, 6) + I * 3**half / 6
    ) * log(-half + x + I * 3**half / 2) + (
        Rational(-1, 6) - I * 3**half / 6
    ) * log(
        -half + x - I * 3**half / 2
    )
...
def test_issue_6308():
    ...
    assert integrate((x**2 + 1 - k**2) / (x**2 + 1 + a0**2), x) == x - (
        a0**2 + k**2
    ) * atan(x / sqrt(a0**2 + 1)) / sqrt(a0**2 + 1)
...
def test_issue_25896():
    ...
    assert ratint((4 * x + 7) / (x**2 + 4 * x + 6) + 2 / x, x) == (
        2 * log(x)
        + 2 * log(x**2 + 4 * x + 6)
        - sqrt(2) * atan(sqrt(2) * x / 2 + sqrt(2)) / 2
    )
