...
def test_multiple_products():
    ...
    assert (
        product(f(n), (n, 1, m), (m, 1, k))
        == Product(f(n), (n, 1, m), (m, 1, k)).doit()
    )
    assert (
...
def test_special_products():
    # Wallis product
    assert product((4 * k) ** 2 / (4 * k**2 - 1), (k, 1, n)) == 4**n * factorial(
        n
    ) ** 2 / rf(S.Half, n) / rf(Rational(3, 2), n)

    # Euler's product formula for sin
    assert (
        product(1 + a / k**2, (k, 1, n))
...
def test__eval_product():
    ...
    assert product(2**i, (i, k, m)) == 2 ** (
        -(k**2) / 2 + k / 2 + m**2 / 2 + m / 2
    )
    n = Symbol("n", negative=True, integer=True)
    p = Symbol("p", positive=True, integer=True)
    assert product(2**i, (i, n, p)) == 2 ** (
        -(n**2) / 2 + n / 2 + p**2 / 2 + p / 2
    )
...
def test_simplify_prod():
    ...
    assert _simplify(
        Product(x * y, (x, n, m), (y, a, k)) * Product(y, (x, n, m), (y, a, k))
    ) == Product(x * y**2, (x, n, m), (y, a, k))
    assert _simplify(
        3 * y * Product(x, (x, n, m)) * Product(x, (x, m + 1, a))
    ) == 3 * y * Product(x, (x, n, a))
    assert _simplify(Product(x, (x, k + 1, a)) * Product(x, (x, n, k))) == Product(
        x, (x, n, a)
    )
    assert _simplify(Product(x, (x, k + 1, a)) * Product(x + 1, (x, n, k))) == Product(
        x, (x, k + 1, a)
    ) * Product(x + 1, (x, n, k))
    assert _simplify(
        Product(x, (t, a, b)) * Product(y, (t, a, b)) * Product(x, (t, b + 1, c))
    ) == Product(x * y, (t, a, b)) * Product(x, (t, b + 1, c))
    assert _simplify(
        Product(x, (t, a, b)) * Product(x, (t, b + 1, c)) * Product(y, (t, a, b))
    ) == Product(x * y, (t, a, b)) * Product(x, (t, b + 1, c))
    assert _simplify(Product(sin(t) ** 2 + cos(t) ** 2 + 1, (t, a, b))) == Product(
        2, (t, a, b)
    )
    assert _simplify(Product(sin(t) ** 2 + cos(t) ** 2 - 1, (t, a, b))) == Product(
        0, (t, a, b)
    ...
    assert _simplify(
        Product(v * Product(sin(t) ** 2 + cos(t) ** 2, (t, a, b)), (v, c, d))
...
def test_change_index():
    ...
    assert Product(x, (x, a, b)).change_index(x, x + 1, y) == Product(
        y - 1, (y, a + 1, b + 1)
    )
    assert Product(x**2, (x, a, b)).change_index(x, x - 1) == Product(
        (x + 1) ** 2, (x, a - 1, b - 1)
    )
    assert Product(x**2, (x, a, b)).change_index(x, -x, y) == Product(
        (-y) ** 2, (y, -b, -a)
    )
    assert Product(x, (x, a, b)).change_index(x, -x - 1) == Product(
        -x - 1, (x, -b - 1, -a - 1)
    )
...
def test_reorder():
    ...
    assert Product(x * y, (x, a, b), (y, c, d)).reorder((0, 1)) == Product(
        x * y, (y, c, d), (x, a, b)
    )
    assert Product(x, (x, a, b), (x, c, d)).reorder((0, 1)) == Product(
        x, (x, c, d), (x, a, b)
    )
    assert Product(x * y + z, (x, a, b), (z, m, n), (y, c, d)).reorder(
        (2, 0), (0, 1)
    ) == Product(x * y + z, (z, m, n), (y, c, d), (x, a, b))
    ...
    assert Product(x * y, (x, a, b), (y, c, d)).reorder((x, 1)) == Product(
        x * y, (y, c, d), (x, a, b)
    )
    assert Product(x * y, (x, a, b), (y, c, d)).reorder((y, x)) == Product(
        x * y, (y, c, d), (x, a, b)
    )
...
def test_reverse_order():
    ...
    assert Product(x * y, (x, 1, 5), (y, 0, 6)).reverse_order(0, 1) == Product(
        x * y, (x, 6, 0), (y, 7, -1)
    )
    ...
    assert Product(x, (x, a + 1, a + 5)).reverse_order(0) == Product(
        1 / x, (x, a + 6, a)
    )
    assert Product(x, (x, a + 1, a + 2)).reverse_order(0) == Product(
        1 / x, (x, a + 3, a)
    )
    assert Product(x, (x, a + 1, a + 1)).reverse_order(0) == Product(
        1 / x, (x, a + 2, a)
    )
    assert Product(x, (x, a, b)).reverse_order(0) == Product(1 / x, (x, b + 1, a - 1))
    assert Product(x, (x, a, b)).reverse_order(x) == Product(1 / x, (x, b + 1, a - 1))
    assert Product(x * y, (x, a, b), (y, 2, 5)).reverse_order(x, 1) == Product(
        x * y, (x, b + 1, a - 1), (y, 6, 1)
    )
    assert Product(x * y, (x, a, b), (y, 2, 5)).reverse_order(y, x) == Product(
        x * y, (x, b + 1, a - 1), (y, 6, 1)
    )
...
def test_rewrite_Sum():
    assert Product(1 - S.Half**2 / k**2, (k, 1, oo)).rewrite(Sum) == exp(
        Sum(log(1 - 1 / (4 * k**2)), (k, 1, oo))
    )
...
def test_issue_20848():
    ...
    assert (
        ...
        == Sum(
            Product(x, (y, 1, _i - 1)) * Product(x, (y, _i + 1, z)), (_i, 1, z)
        ).as_dummy()
    )
