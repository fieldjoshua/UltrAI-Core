...
from sympy.core.expr import Expr, ExprBuilder, UnevaluatedExpr, unchanged
...
def test_as_leading_term():
    ...
    assert e.as_leading_term(x) == (12 * sqrt(3) * x - 12 * S.ImaginaryUnit * x) / (
        4 * sqrt(3) + 12 * S.ImaginaryUnit
    )
...
def test_is_polynomial():
    ...
    assert (
        (x**2) * (y**2) + x * (y**2) + y * x + exp(x)
    ).is_polynomial() is False

    assert ((x**2) * (y**2) + x * (y**2) + y * x + exp(2)).is_polynomial(
        x, y
    ) is True
    assert ((x**2) * (y**2) + x * (y**2) + y * x + exp(x)).is_polynomial(
        x, y
    ) is False
...
def test_replace():
    ...
    assert e.replace(sin, lambda a: sin(2 * a)) == log(sin(2 * x)) + tan(
        sin(2 * x**2)
    )
    ...
    assert g.replace(lambda expr: expr.is_Number, lambda expr: expr**2) == 4 * sin(
        x**9
    )
    ...
    assert (x**2 + O(x**3)).replace(Pow, lambda b, e: b**e / e) == x**2 / 2 + O(
        x**3
    )
...
def test_extractions():
    ...
    assert (-2 * x * y - 4 * x**2 * y).extract_multiplicatively(
        -2 * y
    ) == 2 * x**2 + x
    assert (2 * x * y + 4 * x**2 * y).extract_multiplicatively(
        2 * y
...
def test_action_verbs():
    ...
    assert (
        cancel((x**2 + 5 * x + 6) / (x + 2))
        == ((x**2 + 5 * x + 6) / (x + 2)).cancel()
    )
