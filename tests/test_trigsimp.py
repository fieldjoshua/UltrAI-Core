...
def test_trigsimp1():
    ...
    assert trigsimp(5 * cos(x / 2) ** 2 + 2 * sin(x / 2) ** 2) == 3 * cos(
        x
    ) / 2 + Rational(7, 2)
    ...
    assert trigsimp(tanh(x + y) - tanh(x) / (1 + tanh(x) * tanh(y))) == sinh(y) / (
        sinh(y) * tanh(x) + cosh(y)
    )
...
def test_trigsimp1a():
    assert trigsimp(sin(2) ** 2 * cos(3) * exp(2) / cos(2) ** 2) == tan(2) ** 2 * cos(
        3
    ) * exp(2)
    assert trigsimp(tan(2) ** 2 * cos(3) * exp(2) * cos(2) ** 2) == sin(2) ** 2 * cos(
        3
    ) * exp(2)
...
def test_trigsimp2():
    ...
    assert (
        trigsimp(
            cos(x) ** 2 * sin(y) ** 2 + cos(x) ** 2 * cos(y) ** 2 + sin(x) ** 2,
            recursive=True,
        )
        == 1
    )
    assert (
        trigsimp(
            sin(x) ** 2 * sin(y) ** 2 + sin(x) ** 2 * cos(y) ** 2 + cos(x) ** 2,
            recursive=True,
        )
        == 1
    )
...
def test_issue_4280():
    a, x, y = symbols("a x y")
    assert (
        trigsimp(cos(x) ** 2 + cos(y) ** 2 * sin(x) ** 2 + sin(y) ** 2 * sin(x) ** 2)
        == 1
    )
    assert (
        trigsimp(
    ...
    assert (
        trigsimp(
            a**2 * cos(y) ** 2 * sin(x) ** 2 + a**2 * sin(y) ** 2 * sin(x) ** 2
        )
        == a**2 * sin(x) ** 2
    )
...
def test_trigsimp_issues():
    ...
    ).trigsimp() == 1 / tan(x) ** 2 / tan(y) ** 2
    assert trigsimp(cos(x) / sin(x) * cos(x + y) / sin(x + y)) == 1 / (
        tan(x) * tan(x + y)
    )
    ...
    assert (
        trigsimp(cos(2) * (cos(3) + 1) ** 2 * (cos(3) - 1) ** 2) == cos(2) * sin(3) ** 4
    )
...
def test_hyperbolic_simp():
    ...
    assert trigsimp(5 * cosh(x / 2) ** 2 - 2 * sinh(x / 2) ** 2) == 3 * cosh(
        x
    ) / 2 + Rational(7, 2)
...
def test_trigsimp_groebner():
    ...
    assert (
        trigsimp((-s + 1) / c + c / (-s + 1), method="groebner", polynomial=True)
        == 2 / c
    )
    ...
    assert trigsimp_groebner(
        (tanh(x) + tanh(y)) / (1 + tanh(x) * tanh(y)), hints=[(tanh, x, y)]
    ) == tanh(x + y)
...
def test_issue_6811_fail():
    ...
    eq = (
        4
        * (-19 * sin(x) * y + 5 * sin(3 * x) * y + 15 * cos(2 * x) * z - 21 * z)
        * xp
        / (9 * cos(x) - 5 * cos(3 * x))
    )
...
def test_Piecewise():
    ...
    assert trigsimp(Piecewise((e1, e3 < e2), (e3, True))) == Piecewise(
        (e1, e3 < s2), (e3, True)
    )
...
def test_trigsimp_old():
    ...
    assert (
        trigsimp(1 - sin(sin(x) ** 2 + cos(x) ** 2) ** 2, old=True, deep=True)
        == cos(1) ** 2
    )
