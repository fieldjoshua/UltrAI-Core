...
def test_str_format():
    ...
    assert (
        to_str(
            from_float(1.23456789000000e-2),
            15,
            strip_zeros=False,
            min_fixed=0,
            max_fixed=0,
    ...
    assert (
        to_str(from_float(2.1287e30), 15, max_fixed=1000)
        == "2128700000000000000000000000000.0"
    )
...
def test_conversion_methods():
    ...
    class SomethingComplex:
        def _mpmath_(self, prec, rounding):
            return mp.make_mpc(
                (from_str("1.3", prec, rounding), from_str("1.7", prec, rounding))
            )
