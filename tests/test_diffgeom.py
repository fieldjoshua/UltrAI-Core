...
from sympy.diffgeom.rn import R2, R2_origin, R2_p, R2_r, R3_c, R3_r, R3_s
from sympy.functions import atan2, sin, sqrt
from sympy.matrices import Matrix
from sympy.simplify import simplify, trigsimp
from sympy.testing.pytest import nocache_fail, raises, warns_deprecated_sympy
...
def test_R2():
    ...
    with warns_deprecated_sympy():
        assert m == R2_p.coord_tuple_transform_to(
            R2_r, R2_r.coord_tuple_transform_to(R2_p, m)
        ).applyfunc(simplify)
...
def test_R3():
    ...
    with warns_deprecated_sympy():
        assert m == R3_c.coord_tuple_transform_to(
            R3_r, R3_r.coord_tuple_transform_to(R3_c, m)
        ).applyfunc(simplify)
...
def test_products():
    ...
    assert (
        TensorProduct(R2.dx, R2.e_y)(R2.e_x, R2.y) == R2.dx(R2.e_x) * R2.e_y(R2.y) == 1
    )
    ...
    assert TensorProduct(R2.x, R2.e_x) == R2.x * R2.e_x
    assert (
        TensorProduct(R2.e_x, R2.dy)(R2.x, R2.e_y) == R2.e_x(R2.x) * R2.dy(R2.e_y) == 1
    )
    ...
    assert (
        TensorProduct(R2.e_y, R2.e_x)(R2.x**2 + R2.y**2, R2.x**2 + R2.y**2)
        == 4 * R2.x * R2.y
    )
