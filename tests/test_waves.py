...
def test_twave():
    ...
    assert w3.phase == atan2(
        A1 * sin(phi1) + A2 * sin(phi2), A1 * cos(phi1) + A2 * cos(phi2)
    )
    ...
    assert w3.rewrite("pde") == epsilon * mu * Derivative(E(x, t), t, t) + Derivative(
        E(x, t), x, x
    )
    assert w3.rewrite(cos) == sqrt(
        A1**2 + 2 * A1 * A2 * cos(phi1 - phi2) + A2**2
    ) * cos(
        pi * f * n * x * s / (149896229 * m)
        - 2 * pi * f * t
        + atan2(A1 * sin(phi1) + A2 * sin(phi2), A1 * cos(phi1) + A2 * cos(phi2))
    )
    assert w3.rewrite(exp) == sqrt(
        A1**2 + 2 * A1 * A2 * cos(phi1 - phi2) + A2**2
    ) * exp(
        I
        * (
    ...
    assert w5.rewrite("pde") == epsilon * mu * Derivative(E(x, t), t, t) + Derivative(
        E(x, t), x, x
    )
