...
def test_factor_and_dimension():
    ...
    assert (-(2996 + Rational(1, 4)), Dimension(1)) == SI._collect_factor_and_dimension(
        expr
    )
...
def test_issue_22819():
    from sympy.physics.units import Da, gram, tonne
...
def test_issue_24062():
    from sympy.core.numbers import E
    from sympy.physics.units import capacitance, farad, impedance, ohm, second, time
...
def test_issue_24211():
    from sympy.physics.units import acceleration, meter, second, time, velocity
...
def test_physics_constant():
    ...
    for name in dir(definitions):
        ...
        if name.endswith("_constant"):
            ...
            ), f"{name} is not marked as physics constant when it should be"
    ...
    for const in [
        ...
        ), f"{const} must be PhysicalConstant, but is {type(const)}"
        assert (
            const.is_physical_constant
        ), f"{const} is not marked as physics constant when it should be"
