...
def test_expand_func():
    ...
    assert expand_func(hyper([a, b], [c], 1)) == gamma(c) * gamma(-a - b + c) / (
        gamma(-a + c) * gamma(-b + c)
    )
    assert (
        abs(expand_func(hyper([a1, b1], [c1], 1)).n() - hyper([a1, b1], [c1], 1).n())
        < 1e-10
    )
    ...
    assert expand_func(meijerg([[1, 1], []], [[], []], z)) == meijerg(
        [[1, 1], []], [[], []], z
    )
...
def test_hyper_rewrite_sum():
    ...
    assert replace_dummy(hyper((1, 2), (1, 3), x).rewrite(Sum), _k) == Sum(
        x**_k / factorial(_k) * RisingFactorial(2, _k) / RisingFactorial(3, _k),
        (_k, 0, oo),
    )
...
def test_hyperrep():
    ...
    assert myrep(exp_polar(I * pi) * z).rewrite("nonrep") == Piecewise(
        (0, abs(z) > 1), (b, True)
    )
    assert myrep(exp_polar(2 * I * pi) * z).rewrite("nonrep") == Piecewise(
        (c, abs(z) > 1), (a, True)
    )
    assert myrep(exp_polar(3 * I * pi) * z).rewrite("nonrep") == Piecewise(
        (d, abs(z) > 1), (b, True)
    )
    assert myrep(exp_polar(4 * I * pi) * z).rewrite("nonrep") == Piecewise(
        (2 * c, abs(z) > 1), (a, True)
    )
    assert myrep(exp_polar(5 * I * pi) * z).rewrite("nonrep") == Piecewise(
        (2 * d, abs(z) > 1), (b, True)
    )
...
def test_meijerg_eval():
    ...
    for x_ in [0.5, 1.5]:
        for k_ in [0.5, Rational(1, 3), 0.25, 0.75, Rational(2, 3), 1.0, 1.5]:
            assert (
                abs((expr1 - expr2).n(subs={x: x_, k: k_ + eps, l: k_ - eps})) < 1e-10
            )
            assert (
                abs((expr1 - expr2).n(subs={x: x_, k: -k_ + eps, l: -k_ - eps})) < 1e-10
            )
...
def test_derivative_appellf1():
    ...
    assert (
        diff(appellf1(a, b1, b2, c, x, y), x)
        == a * b1 * appellf1(a + 1, b2, b1 + 1, c + 1, y, x) / c
    )
    assert (
        diff(appellf1(a, b1, b2, c, x, y), y)
        == a * b2 * appellf1(a + 1, b1, b2 + 1, c + 1, x, y) / c
    )
    assert diff(appellf1(a, b1, b2, c, x, y), z) == 0
    assert diff(appellf1(a, b1, b2, c, x, y), a) == Derivative(
        appellf1(a, b1, b2, c, x, y), a
    )
...
def test_eval_nseries():
    ...
    assert exp(x)._eval_nseries(x, 7, None) == hyper(
        (a1, b1), (a1, b1), x
    )._eval_nseries(x, 7, None)
    assert hyper((a1, a2), (b1, b2), x)._eval_nseries(z, 7, None) == hyper(
        (a1, a2), (b1, b2), x
    ) + O(z**7)
    assert hyper((-S(1) / 2, S(1) / 2), (1,), 4 * x / (x + 1)).nseries(
        x
    ) == 1 - x + x**2 / 4 - 3 * x**3 / 4 - 15 * x**4 / 64 - 93 * x**5 / 64 + O(
        x**6
    )
