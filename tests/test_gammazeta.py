...
def test_gamma_quotients():
    ...
    assert (
        abs(gammaprod([-4, -3], [-2, 0]) - G(-4 + h) * G(-3 + h) / G(-2 + h) / G(0 + h))
        < 1e-4
    )
    assert rf(3, 0) == 1
    assert rf(2.5, 1) == 2.5
    ...
    assert rf("-255.5815971722918", "-0.5119253100282322").ae(
        "-0.1952720278805729485"
    )  # issue 421
    assert ff(-2, 0) == 1
    assert ff(-2, 1) == -2
    assert ff(4, 3) == 24
...
def test_zeta_negative():
    ...
    mp.dps = 15
    assert (
        str(zeta(a)) == "2.55880492708712e+1233536161668617575553892558646631323374078"
    )
    mp.dps = 50
    assert (
        str(zeta(a))
...
def test_polygamma():
    ...
    assert (10**6 * psi(4, 1 + 10 * pi * j)).ae(
        -6.1491803479004446 - 0.3921316371664063j
    )
    ...
    assert digamma(-0.75 + 1j).ae(
        mpc(
            "0.46317279488182026118963809283042317",
            "2.4821070143037957102007677817351115",
        )
    )
    mp.dps = 15
...
def test_polygamma_high_order():
    ...
    assert (
        str(psi(50, pi + 14 * e * j))
        == "(-5.2251694115217e-17 - 1.81315704140701e-17j)"
    )
...
def test_gamma_huge_1():
    ...
    assert (
        str(gamma(x))
        == "6.2607532138951929201303779291707455874010420783933e+12458010678"
    )
    mp.dps = 15
...
def test_barnesg():
    ...
    assert [superfac(i) for i in range(8)] == [
        1,
        1,
        2,
        12,
    ...
    assert [hyperfac(n) for n in range(7)] == [
        1,
        1,
        4,
        108,
    ...
    assert [hyperfac(n) for n in range(0, -7, -1)] == [
        1,
        1,
        -1,
        -4,
...
def test_polylog():
    ...
    assert polylog(2, 1.1 * sqrt(j)).ae(
        0.58841571107611387722 + 1.09962542118827026011j
    )
    ...
    assert polylog(0.5 + j / 3, 0.5 + j / 2).ae(
        0.31739144796565650535 + 0.99255390416556261437j
    )
...
def test_bell_polyexp():
    ...
    assert (
        bell(50, -50)
        == 347469029146297202586097646631767227177164818163463279814268368579055777450
    )
    assert (
        bell(40, 50)
        == 5575520134721105844739265207408344706846955281965031698187656176321717550
    )
    assert (
        bell(74)
...
def test_rs_zeta():
    ...
    assert zeta(0.5 + 1000000j, derivative=3).ae(
        1647.7744105852674733 - 1423.1270943036622097j
    )
    assert zeta(1 + 1000000j, derivative=3).ae(
        3.4085866124523582894 - 18.179184721525947301j
    )
    assert zeta(1 + 1000000j, derivative=1).ae(
        -0.10423479366985452134 - 0.74728992803359056244j
    )
    assert zeta(0.5 - 1000000j, derivative=1).ae(
        11.636804066002521459 + 17.127254072212996004j
    )
    ...
    assert ae(
        fp.zeta(0.5 + 1e6j, derivative=1),
        11.636804066002521459 - 17.127254072212996004j,
    )
    assert ae(fp.zeta(1 + 1e6j), 0.94738726251047891048 + 0.59421999312091832833j)
    assert ae(
        fp.zeta(1 + 1e6j, derivative=1),
        -0.10423479366985452134 - 0.74728992803359056244j,
    )
    assert ae(
        fp.zeta(0.5 + 100000j, derivative=1),
        10.766962036817482375 - 30.92705282105996714j,
    )
    assert ae(
        fp.zeta(0.5 + 100000j, derivative=2),
        -119.40515625740538429 + 217.14780631141830251j,
    )
    assert ae(
        fp.zeta(0.5 + 100000j, derivative=3),
        1129.7550282628460881 - 1685.4736895169690346j,
    )
    assert ae(
        fp.zeta(0.5 + 100000j, derivative=4),
        -10407.160819314958615 + 13777.786698628045085j,
    )
    assert ae(
        fp.zeta(0.75 + 100000j, derivative=1),
        -0.41742276699594321475 - 6.4453816275049955949j,
    )
    assert ae(
        fp.zeta(0.75 + 100000j, derivative=2),
        -9.214314279161977266 + 35.07290795337967899j,
    )
    assert ae(
        fp.zeta(0.75 + 100000j, derivative=3),
        110.61331857820103469 - 236.87847130518129926j,
    )
    assert ae(
        fp.zeta(0.75 + 100000j, derivative=4),
        -1054.334275898559401 + 1769.9177890161596383j,
    )
...
def test_siegelz():
    ...
    assert siegelz(100000 + j, derivative=1).ae(
        678.214511857070283307 - 379.742160779916375413j
    )
