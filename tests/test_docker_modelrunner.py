            ...
            try:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        print(
                            f"Docker Model Runner API returned status {response.status}"
                        )
...
async def test_create_modelrunner_adapter(mock_get_available_models):
    ...
    adapter = await create_modelrunner_adapter(
        model="phi3:mini", base_url="http://localhost:8080"
    )
...
async def test_mock_llm_service_with_modelrunner(mock_adapter_generate):
    ...
    response = await service.analyze(
        prompt="Test prompt", model="phi3:mini", context="Some context"
    )
...
if __name__ == "__main__":
    ...
    if is_available:
        ...
        print(
            "Docker Model Runner is not available. Please check its installation and configuration."
        )
