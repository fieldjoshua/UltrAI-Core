...
def test_addition_initial_condition():
    ...
    r = HolonomicFunction(
        (-(x**2) - x + 1) + (x**2 + x) * Dx + (-x - 2) * Dx**3 + (x + 1) * Dx**4,
        x,
        2,
        [4, 1, 2, -5],
    )
...
def test_multiplication_initial_condition():
    ...
    r = HolonomicFunction(
        6 * Dx + 3 * Dx**2 + 2 * Dx**3 - 3 * Dx**4 + Dx**6, x, 0, [1, 5, 14, 17, 17, 2]
    )
    assert p * q == r
    p = expr_to_holonomic(sin(x))
    q = expr_to_holonomic(1 / x, x0=1)
...
def test_to_Sequence():
    ...
    q = [
        (
            HolonomicSequence(
                1 + (n + 2) * Sn**2 + (n**4 + 6 * n**3 + 11 * n**2 + 6 * n) * Sn**3
            ),
            0,
            1,
        )
    ...
    q = [
        (
            HolonomicSequence(
                1 + (n**4 - 2 * n**3 - n**2 + 2 * n) * Sn + (n**2 + 3 * n + 2) * Sn**2
            ),
            0,
            0,
        )
...
def test_to_Sequence_Initial_Coniditons():
    ...
    q = [
        (HolonomicSequence(1 + (n + 1) * Sn + (n**5 - 5 * n**3 + 4 * n) * Sn**2), 0, 3)
    ]
    assert p == q
...
def test_to_expr():
    ...
    assert (
        expr_to_holonomic(2 * x**3 - 3 * x**2).to_expr().expand() == 2 * x**3 - 3 * x**2
    )
    a = symbols("a")
...
def test_integrate():
    ...
    q = (
        x ** Rational(3, 2) * (2 * x**2 + 3 * x + 1) - x * sqrt(x + 1) * asinh(sqrt(x))
    ) / (4 * x * sqrt(x + 1))
    assert p == q
