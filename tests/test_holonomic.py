...
def test_DifferentialOperator():
    ...
    assert (x**2 + 1) + Dx + x * Dx**5 == DifferentialOperator(
        [x**2 + 1, 1, 0, 0, 0, x], R
    )
    assert (x * Dx + x**2 + 1 - Dx * (x**3 + x)) ** 3 == (-48 * x**6) + (
        -57 * x**7
    ) * Dx + (-15 * x**8) * Dx**2 + (-(x**9)) * Dx**3
...
def test_HolonomicFunction_multiplication():
    ...
    r = HolonomicFunction(
        ...
        * Dx
        + (8 * x**6 + 18 * x**5 + 15 * x**4 - 3 * x**3 - 6 * x**2 - 6 * x - 2)
        * Dx**2
...
def test_multiplication_initial_condition():
    ...
    r = HolonomicFunction(
        6 * Dx + 3 * Dx**2 + 2 * Dx**3 - 3 * Dx**4 + Dx**6,
        x,
        0,
        [1, 5, 14, 17, 17, 2],
    )
...
def test_from_meijerg():
    ...
    q = HolonomicFunction(
        (x + S.Half) * Dx + x * Dx**2, x, 1, [sqrt(pi) * erf(1), exp(-1)]
    )
    assert p == q
...
def test_to_Sequence():
    ...
    q = [
        (
            HolonomicSequence(
                1 + (n**4 + 14 * n**3 + 72 * n**2 + 163 * n + 140) * Sn**5
            ),
            0,
            0,
        )
    ...
    q = [
        (
            HolonomicSequence(
                ...
                [2, 3, -1, Rational(-1, 2), Rational(1, 12)],
            ),
            1,
        )
    ]
...
def test_series():
    ...
    assert expr_to_holonomic(sqrt(x**3 + x)).series(n=10) == sqrt(x**3 + x).series(
        n=10
    )
    assert (
    ...
    assert expr_to_holonomic(cos(x) ** 2 / x**2, y0={-2: [1, 0, -1]}).series(
        n=10
    ) == (cos(x) ** 2 / x**2).series(n=10)
    assert (
        expr_to_holonomic(cos(x) ** 2 / x**2, x0=1).series(n=10).together()
        == (cos(x) ** 2 / x**2).series(n=10, x0=1).together()
    )
    assert expr_to_holonomic(
        cos(x - 1) ** 2 / (x - 1) ** 2, x0=1, y0={-2: [1, 0, -1]}
    ).series(n=10) == (cos(x - 1) ** 2 / (x - 1) ** 2).series(x0=1, n=10)
...
def test_expr_to_holonomic():
    ...
    q = HolonomicFunction(
        ...
        + (x**5 + 2 * x**4 - x**3 - 7 * x**2 / 2 + x + Rational(5, 2))
        * Dx**4,
        x,
        0,
        [0, 1, 4, -1],
    ...
    q = HolonomicFunction(
        (-x - 3) * Dx + (x + 2) * Dx**2 + (-x - 3) * Dx**3 + (x + 2) * Dx**4,
        x,
        0,
        [2, 1, 1, 3],
    )
...
def test_to_hyper():
    ...
    assert p == q
    p = hyperexpand(
        HolonomicFunction(2 * x * Dx + Dx**2, x, 0, [1, -2 / sqrt(pi)]).to_hyper()
    )
    q = erfc(x)
    assert p.rewrite(erfc) == q
    p = hyperexpand(
        HolonomicFunction(
            (x**2 - 1) + x * Dx + x**2 * Dx**2, x, 0, [0, S.Half]
        ).to_hyper()
    )
    q = besselj(1, x)
    assert p == q
...
def test_to_expr():
    ...
    assert p == q
    p = (
        HolonomicFunction(2 + (4 * x - 1) * Dx + (x**2 - x) * Dx**2, x, 0, [1, 2])
        .to_expr()
        .expand()
    )
    q = 1 / (x**2 - 2 * x + 1)
    ...
    assert (
        expr_to_holonomic(2 * x**3 - 3 * x**2).to_expr().expand()
        == 2 * x**3 - 3 * x**2
    )
    a = symbols("a")
...
def test_integrate():
    ...
    assert p == q
    p = (
        expr_to_holonomic(cos(x) ** 2 / x**2, y0={-2: [1, 0, -1]})
        .integrate(x)
        .to_expr()
    )
    q = -Si(2 * x) - cos(x) ** 2 / x
    ...
    q = (
        x ** Rational(3, 2) * (2 * x**2 + 3 * x + 1)
        - x * sqrt(x + 1) * asinh(sqrt(x))
    ) / (4 * x * sqrt(x + 1))
    assert p == q
...
def test_diff():
    ...
    assert (
        p.diff(x, 2).to_expr().subs(C_0, Rational(-1, 3)).cancel()
        == q.diff(x, 2).cancel()
    )
    assert (
        p.diff(x, 3).series().subs({C_3: Rational(-1, 3), C_0: 0})
        == q.diff(x, 3).series()
    )
...
def test_extended_domain_in_expr_to_holonomic():
    ...
    q = HolonomicFunction(
        (-1.1329138213) + (1.1329138213 * x) * Dx, x, 0, {1: [1.1329138213]}
    )
    assert p == q
...
def test_to_meijerg():
    ...
    assert hyperexpand(
        expr_to_holonomic(besselj(2, x), lenics=3).to_meijerg()
    ) == besselj(2, x)
    ...
    assert (
        p.to_expr() - (D_0 * sin(x) + C_0 * cos(x) + C_1 * sin(x)) / sqrt(x)
    ).simplify() == 0
    p = expr_to_holonomic(
        besselj(S.Half, x),
        y0={Rational(-1, 2): [sqrt(2) / sqrt(pi), sqrt(2) / sqrt(pi)]},
    )
    assert (
        p.to_expr() - besselj(S.Half, x) - besselj(Rational(-1, 2), x)
    ).simplify() == 0
