...
def test_gammasimp():
    ...
    assert gammasimp(1 / gamma(x + 3) / gamma(1 - x)) == sin(pi * x) / (
        pi * x * (x + 1) * (x + 2)
    )

    assert gammasimp(factorial(n + 2)) == gamma(n + 3)
    assert gammasimp(binomial(n, k)) == gamma(n + 1) / (
        gamma(k + 1) * gamma(-k + n + 1)
    )

    assert powsimp(
        gammasimp(gamma(x) * gamma(x + S.Half) * gamma(y) / gamma(x + y))
    ) == 2 ** (-2 * x + 1) * sqrt(pi) * gamma(2 * x) * gamma(y) / gamma(x + y)
    assert gammasimp(
        1 / gamma(x) / gamma(x - Rational(1, 3)) / gamma(x + Rational(1, 3))
    ) == 3 ** (3 * x - Rational(3, 2)) / (2 * pi * gamma(3 * x - 1))
    assert (
        simplify(
            gamma(S.Half + x / 2) * gamma(1 + x / 2) / gamma(1 + x) / sqrt(pi) * 2**x
        )
        == 1
    )
    assert (
        gammasimp(gamma(Rational(-1, 4)) * gamma(Rational(-3, 4)))
        == 16 * sqrt(2) * pi / 3
    )

    assert powsimp(gammasimp(gamma(2 * x) / gamma(x))) == 2 ** (2 * x - 1) * gamma(
        x + S.Half
    ) / sqrt(pi)
    ...
    assert gammasimp(rf(x + n, k) * binomial(n, k)).simplify() == Piecewise(
        ...
        (
            ...
            * gamma(n + 1)
            * gamma(-n - x + 1)
            / (gamma(k + 1) * gamma(-k + n + 1) * gamma(-k - n - x + 1)),
            True,
        ),
    )
    ...
    assert gammasimp(binomial(n + 2, k + 2.0)) == gamma(n + 3) / (
        gamma(k + 3.0) * gamma(-k + n + 1)
    )
    ...
    assert gammasimp(gamma(4 * n + S.Half) / gamma(2 * n - R(3, 4))) == 2 ** (
        4 * n - R(5, 2)
    ) * (8 * n - 3) * gamma(2 * n + R(3, 4)) / sqrt(pi)
