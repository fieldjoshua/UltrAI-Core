...
def test_commutator_identities():
    ...
    assert Comm(A, C**-2).expand(commutator=True) == Comm(
        A, (1 / C) * (1 / D)
    ).expand(commutator=True).replace(D, C)
    assert Comm(A + B, C + D).expand(commutator=True) == Comm(A, C) + Comm(A, D) + Comm(
        B, C
    ) + Comm(B, D)
...
def test_commutator_dagger():
    comm = Comm(A * B, C)
    assert Dagger(comm).expand(commutator=True) == -Comm(Dagger(B), Dagger(C)) * Dagger(
        A
...
def test_eval_commutator():
    ...
    assert (
        Comm(T**2, F**3).expand(commutator=True).doit()
        == 2 * F * T * F + 2 * F**2 * T + 2 * T * F**2
    )
