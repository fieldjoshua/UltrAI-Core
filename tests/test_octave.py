...
from sympy.printing.octave import octave_code as mcode
...
def test_containers():
    ...
    assert (
        mcode((1, eye(3), Matrix(0, 0, []), [])) == "{1, [1 0 0; 0 1 0; 0 0 1], [], {}}"
    )
...
def test_specfun():
    ...
    assert (
        octave_code(dirichlet_eta(x))
        == "(((x == 1).*(log(2)) + (~(x == 1)).*((1 - 2.^(1 - x)).*zeta(x))))"
    )
    assert (
        octave_code(riemann_xi(x)) == "(pi.^(-x/2).*x.*(x - 1).*gamma(x/2).*zeta(x)/2)"
    )
