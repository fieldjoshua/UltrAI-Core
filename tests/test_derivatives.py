...
def test_matrix_derivative_by_scalar():
    ...
    assert (
        hadamard_power(x, i)
        .diff(i)
        .dummy_eq(HadamardProduct(x.applyfunc(log), HadamardPower(x, i)))
    )
...
def test_matrix_derivative_non_matrix_result():
    ...
    assert (2 * A).diff(A) == PermuteDims(
        ArrayTensorProduct(2 * I, I), Permutation(3)(1, 2)
    )
...
def test_matrix_derivatives_of_traces():
    ...
    assert expr[i, j].diff(A[m, n]).doit() == (
        KDelta(i, m) * KDelta(j, n) * Trace(A) + KDelta(m, n) * A[i, j]
    )
    ...
    assert (
        expr.diff(X)
        == -X.inv().T * A.T * X.inv() * C.inv().T * X.inv().T
        - X.inv().T * A * X.inv() * C.inv() * X.inv().T
    )
    ...
    assert (
        expr.diff(X)
        == -2 * C * X * (X.T * C * X).inv() * X.T * B * X * (X.T * C * X).inv()
        + 2 * B * X * (X.T * C * X).inv()
    )
    ...
    assert expr.diff(X) == B * X * Inverse(A + X.T * C * X) - C * X * Inverse(
        A + X.T * C * X
    ) * X.T * B * X * Inverse(A + X.T * C * X) - C.T * X * Inverse(
        A.T + (C * X).T * X
    ) * X.T * B.T * X * Inverse(
        A.T + (C * X).T * X
    ) + B.T * X * Inverse(
...
def test_derivatives_of_complicated_matrix_expr():
    expr = (
        ...
        * (
            ...
            * (
                a
                * b.T
                * (X * D * X.T + X * (X.T * B + A * X) * D * B - X.T * C.T * A)
                * B
    ...
    result = (
        (
            ...
            + a
            * b.T
            * (X * (A * X + X.T * B) * D * B + X * D * X.T - X.T * C.T * A)
            * B
        )
        * B
        * b
