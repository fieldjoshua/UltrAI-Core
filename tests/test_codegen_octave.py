...
def test_results_named_ordered():
    ...
    name_expr = ("test", [expr1, expr2, expr3])
    result = codegen(
        name_expr, "Octave", header=False, empty=False, argument_sequence=(x, z, y)
    )
...
def test_m_filename_match_first_fcn():
    ...
    raises(
        ValueError,
        lambda: codegen(name_expr, "Octave", prefix="bar", header=False, empty=False),
    )
...
def test_m_loops():
    ...
    j = Idx("j", n)
    (result,) = codegen(
        ("mat_vec_mult", Eq(y[i], A[i, j] * x[j])), "Octave", header=False, empty=False
    )
    source = result[1]
    expected = (
...
def test_m_InOutArgument_order():
    ...
    name_expr = ("test", expr)
    (result,) = codegen(
        name_expr, "Octave", header=False, empty=False, argument_sequence=(x, y)
    )
    source = result[1]
...
def test_global_vars_octave():
    ...
    result = codegen(
        ("f", x * y), "Octave", header=False, empty=False, global_vars=(y,)
    )
    source = result[0][1]
