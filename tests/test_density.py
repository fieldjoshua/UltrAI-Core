...
def test_eval_args():
    ...
    assert isinstance(
        Density([Qubit("00"), 1 / sqrt(2)], [Qubit("11"), 1 / sqrt(2)]), Density
    )
...
def test_doit():
    ...
    assert (
        0.5 * (PxKet() * Dagger(PxKet())) + 0.5 * (XKet() * Dagger(XKet()))
    ) == d.doit()

    # check for kets with expr in them
    d_with_sym = Density([XKet(x * y), 0.5], [PxKet(x * y), 0.5])
    assert (
        0.5 * (PxKet(x * y) * Dagger(PxKet(x * y)))
        + 0.5 * (XKet(x * y) * Dagger(XKet(x * y)))
    ) == d_with_sym.doit()
    ...
    assert d.doit() == (
        0.5 * TensorProduct(A * Dagger(A), B * Dagger(B))
...
def test_represent():
    ...
    d = Density([XKet(), 0.5], [PxKet(), 0.5])
    assert (
        represent(0.5 * (PxKet() * Dagger(PxKet())))
        + represent(0.5 * (XKet() * Dagger(XKet())))
    ) == represent(d)

    # check for kets with expr in them
    d_with_sym = Density([XKet(x * y), 0.5], [PxKet(x * y), 0.5])
    assert (
        represent(0.5 * (PxKet(x * y) * Dagger(PxKet(x * y))))
...
def test_eval_trace():
    ...
    assert d.doit() == (
        0.5 * OuterProduct(k1, k1.dual) + 0.5 * OuterProduct(k2, k2.dual)
    )
