from sympy import Dummy, Function, Lambda, cos, sin, symbols
from sympy.abc import n, w, x, y, z
from sympy.core.sympify import sympify
from sympy.parsing.mathematica import MathematicaParser, parse_mathematica
...
def test_mathematica():
    ...
    assert parse_mathematica("Sin[#] + Cos[#3] &").dummy_eq(
        Lambda((d1, d2, d3), sin(d1) + cos(d3))
    )
    assert parse_mathematica("Sin[#^2] &").dummy_eq(Lambda(d1, sin(d1**2)))
    assert parse_mathematica("Function[x, x^3]") == Lambda(x, x**3)
    assert parse_mathematica("Function[{x, y}, x^2 + y^2]") == Lambda(
        (x, y), x**2 + y**2
    )
...
def test_parser_mathematica_tokenizer():
    ...
    chain = lambda expr: parser._from_tokens_to_fullformlist(
        parser._from_mathematica_to_tokens(expr)
    )
    ...
    *) e
    ...
    assert chain("#1 + #2 & [x, y]") == [
        ["Function", ["Plus", ["Slot", "1"], ["Slot", "2"]]],
        "x",
        "y",
    ]
    assert chain("#1^2#2^3&") == [
        "Function",
        ["Times", ["Power", ["Slot", "1"], "2"], ["Power", ["Slot", "2"], "3"]],
    ]
...
def test_parser_mathematica_exp_alt():
    ...
    convert_chain3 = lambda expr: parser._from_fullformsympy_to_sympy(
        convert_chain2(expr)
    )
    ...
    assert parser._from_fullform_to_fullformlist(full_form1) == [
        "Sin",
        ["Times", "x", "y"],
    ]
    assert parser._from_fullform_to_fullformlist(full_form2) == [
        "Plus",
        ["Times", "x", "y"],
        "z",
    ]
    ...
    assert parser._from_fullform_to_fullformlist(full_form4) == [
        "Rational",
        ["Rational", "x", "y"],
        "z",
    ]
