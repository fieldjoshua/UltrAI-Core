...
from sympy.simplify.sqrtdenest import _sqrt_numeric_denest
...
def test_sqrtdenest2():
    assert sqrtdenest(sqrt(16 - 2 * r29 + 2 * sqrt(55 - 10 * r29))) == r5 + sqrt(
        11 - 2 * r29
    )
    ...
    assert sqrtdenest(sqrt(((1 + r5 + sqrt(1 + r3)) ** 2).expand())) == 1 + r5 + sqrt(
        1 + r3
    )

    assert (
        sqrtdenest(sqrt(((1 + r5 + r7 + sqrt(1 + r3)) ** 2).expand()))
        == 1 + sqrt(1 + r3) + r5 + r7
    )
...
def test_sqrtdenest_rec():
    assert (
        sqrtdenest(sqrt(-4 * sqrt(14) - 2 * r6 + 4 * sqrt(21) + 33))
        == -r2 + r3 + 2 * r7
    )
    assert sqrtdenest(sqrt(-28 * r7 - 14 * r5 + 4 * sqrt(35) + 82)) == -7 + r5 + 2 * r7
    assert (
        sqrtdenest(sqrt(6 * r2 / 11 + 2 * sqrt(22) / 11 + 6 * sqrt(11) / 11 + 2))
        == sqrt(11) * (r2 + 3 + sqrt(11)) / 11
    )
    assert (
        sqrtdenest(sqrt(468 * r3 + 3024 * r2 + 2912 * r6 + 19735))
        == 9 * r3 + 26 + 56 * r6
    )
    ...
    assert (
        sqrtdenest(sqrt(8 * r2 / 3 + 14 * r5 / 3 + Rational(154, 9)))
        == -r10 / 3 + r2 + r5 + 3
    )
    assert sqrtdenest(sqrt(sqrt(2 * r6 + 5) + sqrt(2 * r7 + 8))) == sqrt(
        1 + r2 + r3 + r7
    )
...
def test_sqrtdenest3():
    ...
    z = sqrt(sqrt(r2 + 2) + 2)
    assert sqrtdenest(z) == z
    assert sqrtdenest(sqrt(-2 * r10 + 4 * r2 * sqrt(-2 * r10 + 11) + 20)) == sqrt(
        -2 * r10 - 4 * r2 + 8 * r5 + 20
    )
    assert (
        sqrtdenest(sqrt((112 + 70 * r2) + (46 + 34 * r2) * r5))
        == r10 + 5 + 4 * r2 + 3 * r5
    )
    ...
    assert sqrtdenest(n / d) == r7 * (1 + r6 + r7) / (
        Mul(7, (sqrt(-2 * r29 + 11) + r5), evaluate=False)
    )
...
def test_sqrt_symbolic_denest():
    ...
    c2 = c**2
    assert (
        sqrtdenest(sqrt(2 * sqrt(1 + r3) * c + c2 + 1 + r3 * c2))
        == -1 - sqrt(1 + r3) * c
    )
    ra = sqrt(1 + r3)
    z = sqrt(
        20 * ra * sqrt(3 + 3 * r3) + 12 * r3 * ra * sqrt(3 + 3 * r3) + 64 * r3 + 112
    )
    assert sqrtdenest(z) == z
