...
from sympy.series.limits import Limit, heuristics, limit
...
def test_issue_6560():
    ...
    assert limit(e, y, oo) == 5 * x**3 / 4 + 3 * x**2 / 4 - 3 * x / 4 - Rational(
        1, 4
    )
...
def test_issue_10610():
    assert limit(3**x * 3 ** (-x - 1) * (x + 1) ** 2 / x**2, x, oo) == Rational(
        1, 3
    )
...
def test_issue_26250():
    ...
    e2 = pi**2 * (
        x**8 - 2 * x**7 - x**6 + 4 * x**5 - x**4 - 2 * x**3 + x**2
    )
