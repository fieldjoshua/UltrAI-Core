...
def test_RkGate():
    ...
    assert represent(RkGate(0, x), nqubits=1) == Matrix(
        [[1, 0], [0, exp(sign(x) * 2 * pi * I / (2 ** abs(x)))]]
    )


def test_quantum_fourier():
    assert QFT(0, 3).decompose() == SwapGate(0, 2) * HadamardGate(0) * CGate(
        (0,), PhaseGate(1)
    ) * HadamardGate(1) * CGate((0,), TGate(2)) * CGate(
        (1,), PhaseGate(2)
    ) * HadamardGate(
        2
    )

    assert IQFT(0, 3).decompose() == HadamardGate(2) * CGate(
        (1,), RkGate(2, -2)
    ) * CGate((0,), RkGate(2, -3)) * HadamardGate(1) * CGate(
        (0,), RkGate(1, -2)
    ) * HadamardGate(
        0
    ) * SwapGate(
    ...
    assert represent(QFT(0, 3), nqubits=3) == Matrix(
        [
            [exp(2 * pi * I / 8) ** (i * j % 8) / sqrt(8) for i in range(8)]
            for j in range(8)
        ]
    )
