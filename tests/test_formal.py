...
def test_fps__rational():
    ...
    f = log((1 + x) / (1 - x)) / 2 - atan(x)
    assert fps(f, x, full=True).truncate(n=10) == 2 * x**3 / 3 + 2 * x**7 / 7 + O(
        x**10
    )
...
def test_fps__hyper():
    ...
    f = exp(x) * sin(x) / x
    assert fps(f, x).truncate() == 1 + x + x**2 / 3 - x**4 / 30 - x**5 / 90 + O(
        x**6
    )
...
def test_fps_shift():
    ...
    f = x**2 * cos(sqrt(x))
    assert fps(f, x).truncate() == x**2 - x**3 / 2 + x**4 / 24 - x**5 / 720 + O(
        x**6
    )
...
def test_fps__Add_expr():
    ...
    assert fps(f, x).truncate() == x - 3 * x**2 / 2 - x**4 / 4 + x**5 / 5 + O(
        x**6
    )
...
def test_fps__slow():
    ...
    assert fps(f, x).truncate() == 2 * x**2 + 2 * x**3 - x**4 / 3 - x**5 + O(
        x**6
    )
