...
from sympy.solvers.ode.ode import _remove_redundant_solutions, allhints
from sympy.solvers.ode.single import (
    FirstLinear,
    NthOrderReducible,
    ODEMatchError,
    SingleODEProblem,
    ...
    def inner():
        ...
        for example in solver["examples"]:
            temp = {
                ...
                "checkodesol_XFAIL": solver["examples"][example].get(
                    "checkodesol_XFAIL", False
                ),
                "dsolve_too_slow": solver["examples"][example].get(
                    "dsolve_too_slow", False
                ),
                "checkodesol_too_slow": solver["examples"][example].get(
                    "checkodesol_too_slow", False
                ),
                "hint": solver["hint"],
            }
...
def _ode_solver_test(ode_examples, run_slow_test=False):
    for example in ode_examples:
        if ((not run_slow_test) and example["slow"]) or (
            run_slow_test and (not example["slow"])
        ):
            continue
...
def _test_particular_example(our_hint, ode_example, solver_flag=False):
    ...
    if our_hint in classify_ode(eq, func):
        ...
        try:
            ...
            if not solver_flag:
                traceback.print_exc()
            result["msg"] = exception_msg.format(
                e=str(e), hint=our_hint, example=example, eq=eq
            )
        ...
        if solver_flag and dsolve_sol != []:
            ...
            if expect_sol_check:
                message = expected_sol_message.format(
                    example=example, eq=eq, sol=expected_sol, dsolve_sol=dsolve_sol
                )
                raise AssertionError(message)
            ...
            if not checkodesol_too_slow:
                if not checkodesol_XFAIL:
                    if (
                        checkodesol(eq, dsolve_sol, func, solve_for_func=False)
                        != expected_checkodesol
                    ):
...
def test_SingleODESolver():
    ...
    problem = SingleODEProblem(
        f(x).diff(x, 4) + f(x).diff(x, 2) - f(x).diff(x, 3), f(x), x
    )
    ...
    problem = SingleODEProblem(
        f(x).diff(x, 3) + x * f(x).diff(x, 2) - f(x) ** 2, f(x), x
    )
...
def test_slow_examples_1st_homogeneous_coeff_ode():
    ...
    _ode_solver_test(
        _get_examples_ode_sol_1st_homogeneous_coeff_best, run_slow_test=True
    )
...
def test_slow_examples_nth_linear_constant_coeff_homogeneous():
    _ode_solver_test(
        _get_examples_ode_sol_nth_linear_constant_coeff_homogeneous, run_slow_test=True
    )
...
def test_2nd_2F1_hypergeometric_integral():
    ...
    sol = Eq(
        ...
        (
            ...
            + C2
            * Integral(
                exp(Integral((1 - x / 2) / (x * (x - 1)), x)) / (1 - x / 2) ** 2, x
            )
        )
...
def test_2nd_nonlinear_autonomous_conserved_integral():
    ...
    solved = dsolve(
        eq, hint="2nd_nonlinear_autonomous_conserved_Integral", simplify=False
    )
...
def test_slow_examples_nth_linear_constant_coeff_var_of_parameters():
    _ode_solver_test(
        _get_examples_ode_sol_nth_linear_var_of_parameters, run_slow_test=True
    )
...
def test_slow_examples_nth_linear_constant_coeff_undetermined_coefficients():
    _ode_solver_test(
        _get_examples_ode_sol_nth_linear_undetermined_coefficients, run_slow_test=True
    )
...
def test_nth_order_reducible():
    ...
    assert (
        F(D(x * f(y), y, 2) + D(u * y * f(x), x, 3)) == False
    )  # no simplification by design
...
def test_Liouville_ODE():
    hint = "Liouville"
    not_Liouville1 = classify_ode(
        diff(f(x), x) / x + f(x) * diff(f(x), x, x) / 2 - diff(f(x), x) ** 2 / 2, f(x)
    )
    not_Liouville2 = classify_ode(
        diff(f(x), x) / x + diff(f(x), x, x) / 2 - x * diff(f(x), x) ** 2 / 2, f(x)
    )
...
def _get_examples_ode_sol_euler_homogeneous():
    r1, r2, r3, r4, r5 = [
        rootof(x**5 - 14 * x**4 + 71 * x**3 - 154 * x**2 + 120 * x - 1, n)
        for n in range(5)
    ]
    return {
        ...
        "examples": {
            ...
            "euler_hom_02": {
                "eq": Eq(
                    3 * f(x) - 5 * diff(f(x), x) * x + 2 * x**2 * diff(f(x), x, x), 0
                ),
                "sol": [Eq(f(x), C1 * sqrt(x) + C2 * x**3)],
            },
            "euler_hom_03": {
                "eq": Eq(
                    4 * f(x) + 5 * diff(f(x), x) * x + x**2 * diff(f(x), x, x), 0
                ),
...
def _get_examples_ode_sol_euler_undetermined_coeff():
    return {
        ...
        "examples": {
            ...
            "euler_undet_02": {
                "eq": Eq(
                    x**2 * diff(f(x), x, x) - 2 * x * diff(f(x), x) + 2 * f(x), x**3
                ),
                "sol": [Eq(f(x), x * (C1 + C2 * x + Rational(1, 2) * x**2))],
            },
            "euler_undet_03": {
                "eq": Eq(
                    x**2 * diff(f(x), x, x) - x * diff(f(x), x) - 3 * f(x), log(x) / x
                ),
                "sol": [
                    Eq(f(x), (C1 + C2 * x**4 - log(x) ** 2 / 8 - log(x) / 16) / x)
                ],
            },
            "euler_undet_04": {
                "eq": Eq(
                    x**2 * diff(f(x), x, x) + 3 * x * diff(f(x), x) - 8 * f(x),
                    log(x) ** 3 - log(x),
                ),
                "sol": [
                    Eq(
            ...
            "euler_undet_06": {
                "eq": 2 * x**2 * f(x).diff(x, 2)
                + f(x)
                + sqrt(2 * x) * sin(log(2 * x) / 2),
                "sol": [
                    Eq(
                        f(x),
                        sqrt(x)
...
def _get_examples_ode_sol_euler_var_para():
    return {
        ...
        "examples": {
            ...
            "euler_var_02": {
                "eq": Eq(
                    3 * x**2 * diff(f(x), x, x) + 6 * x * diff(f(x), x) - 6 * f(x),
                    x**3 * exp(x),
                ),
                "sol": [
                    Eq(
            ...
            "euler_var_05": {
                "eq": -exp(x)
                + (x * Derivative(f(x), (x, 2)) + Derivative(f(x), x)) / x,
                "sol": [Eq(f(x), C1 + C2 * log(x) + exp(x) - Ei(x))],
            },
            "euler_var_06": {
                "eq": x**2 * f(x).diff(x, 2) + x * f(x).diff(x) + 4 * f(x) - 1 / x,
                "sol": [
                    Eq(f(x), C1 * sin(2 * log(x)) + C2 * cos(2 * log(x)) + 1 / (5 * x))
                ],
            },
        },
    }
    ...
    return {
        ...
        "examples": {
            "riccati_01": {
                ...
                "sol": [
                    Eq(f(x), (-sqrt(3) * tan(C1 + sqrt(3) * log(x) / 4) + 3) / (2 * x))
                ],
            },
        },
    }
...
def _get_examples_ode_sol_1st_rational_riccati():
    ...
    return {
        ...
        "examples": {
            ...
            # a(x) is a constant
            "rational_riccati_03": {
                "eq": 2 * x**2 * Derivative(f(x), x)
                - x * (4 * f(x) + Derivative(f(x), x) - 4)
                + (f(x) - 1) * f(x),
            ...
            # One pole of multiplicity 2
            "rational_riccati_06": {
                "eq": x**4 * Derivative(f(x), x)
                + x**2
                - x * (2 * f(x) ** 2 + Derivative(f(x), x))
                + f(x),
                ...
                "sol": [
                    Eq(
                        ...
                        (
                            C1
                            + x * exp(x**2 / 2)
                            - sqrt(2) * sqrt(pi) * erfi(sqrt(2) * x / 2) / 2
                        )
                        * exp(-(x**2) / 2),
                    )
                ],
...
def _get_examples_ode_sol_factorable():
    ...
    return {
        ...
        "examples": {
            ...
            "fact_17": {
                "eq": f(x).diff(x)
                - (a4 * x**4 + a3 * x**3 + a2 * x**2 + a1 * x + a0) ** (-1 / 2),
                "sol": [
                    Eq(
                        f(x),
                        C1
            ...
            "fact_18": {
                ...
                "sol": [
                    Eq(f(x), C1 + Integral(pi - asin(f(2 * x)), x)),
...
def _get_examples_ode_sol_liouville():
    ...
    return {
        ...
        "examples": {
            ...
            "liouville_03": {
                "eq": (
                    (diff(f(x), x) / x + diff(f(x), x, x) / 2 - diff(f(x), x) ** 2 / 2)
                    * exp(-f(x))
                    / exp(f(x))
                ).expand(),
                "sol": [Eq(f(x), log(x / (C1 + C2 * x)))],
                ...
                "sol": [
                    Eq(
                        ...
                        + NonElementaryIntegral(
                            exp(_y**3 / 3) * exp(_y**2 * n / 2), (_y, f(x))
                        ),
                        0,
                    )
                ],
    ...
    return {
        ...
        "examples": {
            ...
            "reducible_03": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1
                        + C2 * sin(2 ** Rational(3, 4) * x / 2)
                        + C3 * cos(2 ** Rational(3, 4) * x / 2),
                    )
                ],
                "slow": True,
            },
            ...
            "reducible_06": {
                ...
                + 4 * f(x).diff(x),
                "sol": [
                    Eq(f(x), C1 + C2 * exp(-2 * x) + C3 * exp(x) + C4 * exp(2 * x))
                ],
                "slow": True,
            },
            "reducible_07": {
            ...
            "reducible_08": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1 + C2 * x + C3 * exp(-sqrt(2) * x) + C4 * exp(sqrt(2) * x),
                    )
                ],
                "slow": True,
            },
        ...
        "examples": {
            ...
            "undet_02": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C3 * exp(x / 3) - x + (C1 + x * (C2 - x / 8)) * exp(-x) - 1,
                    )
                ],
                "slow": True,
            },
            ...
            "undet_06": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1 * exp(-2 * x) + C2 * exp(-x) + sin(x) / 10 - 3 * cos(x) / 10,
                    )
                ],
                "slow": True,
            },
            "undet_07": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1 * exp(-2 * x) + C2 * exp(-x) + 3 * sin(x) / 10 + cos(x) / 10,
                    )
                ],
                "slow": True,
            },
            ...
            "undet_09": {
                ...
                "sol": [
                    Eq(
                        ...
                        -2 * x
                        + x**2
                        + (C1 * sin(x * sqrt(3) / 2) + C2 * cos(x * sqrt(3) / 2))
                        * exp(-x / 2),
                    )
                ],
                "slow": True,
            ...
            "undet_10": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        -x * exp(x) - 2 * exp(-x) + C1 * exp(-2 * x) + C2 * exp(4 * x),
                    )
                ],
                "slow": True,
            },
            ...
            "undet_16": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        (C1 - x**2 / 8) * cos(2 * x) + (C2 + x / 16) * sin(2 * x),
                    )
                ],
                "slow": True,
            },
            ...
            "undet_18": {
                ...
                + x**2 * exp(-x),
                "sol": [
                    Eq(f(x), (C1 + x * (C2 + x * (C3 - x**3 / 60 + x / 3))) * exp(-x))
                ],
                "slow": True,
            },
            "undet_19": {
            ...
            "undet_27": {
                ...
                "sol": [
                    Eq(f(x), cos(3 * x) / 16 + C2 * cos(x) + (C1 + x / 4) * sin(x))
                ],
                "slow": True,
            },
            "undet_28": {
            ...
            "undet_31": {
                ...
                "sol": [
                    Eq(f(x), C1 * exp(-7 * x) + C2 * exp(7 * x) - sinh(3 * x) / 40)
                ],
            },
            "undet_32": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1 + C3 * exp(-x) + x * sinh(x) / 2 + (C2 + x / 2) * exp(x),
                    )
                ],
            },
            ...
            "undet_38": {
                "eq": Eq(
                    u(t).diff(t, t) + R / L * u(t).diff(t) + 1 / (L * C) * u(t), alpha
                ),
                "sol": [
                    Eq(
                        u(t),
                        C * L * alpha
                        + C2
                        * exp(-t * (R + sqrt(C * R**2 - 4 * L) / sqrt(C)) / (2 * L))
                        + C1
                        * exp(t * (-R + sqrt(C * R**2 - 4 * L) / sqrt(C)) / (2 * L)),
                    )
                ],
            ...
            "undet_39": {
                "eq": Eq(
                    L * C * u(t).diff(t, t) + R * C * u(t).diff(t) + u(t),
                    E_0 * exp(I * omega * t),
                ),
                "sol": [
                    Eq(
            ...
            "undet_40": {
                "eq": Eq(
                    Derivative(f(x), x, 2) - 2 * Derivative(f(x), x) + f(x), sin(x)
                ),
    ...
    return {
        ...
        "examples": {
            ...
            "separable_09": {
                ...
                "sol": [
                    Eq(f(x), sinh(C1 - log(log(x))))
                ],  # One more solution is f(x)=I
                "slow": True,
                "checkodesol_XFAIL": True,
            },
            "separable_10": {
            ...
            "separable_15": {
                ...
                "sol": [
                    Eq(f(x), tan(C1 / x))
                ],  # Two more solutions are f(x)=0 and f(x)=I
                "slow": True,
                "checkodesol_XFAIL": True,
            },
            "separable_16": {
            ...
            "separable_17": {
                "eq": exp(f(x) ** 2) * (x**2 + 2 * x + 1)
                + (x * f(x) + f(x)) * f(x).diff(x),
                "sol": [
            ...
            "separable_19": {
                ...
                "sol": [
                    Eq(f(x), pi - acos(C1 / cos(x) ** 2) / 2),
                    Eq(f(x), acos(C1 / cos(x) ** 2) / 2),
                ],
            },
            "separable_20": {
            ...
            "separable_22": {
                ...
                "XFAIL": [
                    "lie_group"
                ],  # It shows 'NoneType' object is not subscriptable for lie_group.
            },
            ...
            "separable_26": {
                ...
                "sol": [
                    Eq(v(t), -68.585712797928991 / tanh(C1 - 0.14288690166235204 * t))
                ],
                "func": v(t),
                "checkodesol_XFAIL": True,
            },
    ...
    return {
        ...
        "examples": {
            ...
            "1st_exact_02": {
                ...
                "XFAIL": [
                    "lie_group"
                ],  # It shows dsolve raises an exception: List index out of range for lie_group
                "slow": True,
                "checkodesol_XFAIL": True,
            },
            "1st_exact_03": {
                "eq": 2 * x
                + f(x) * cos(x)
                + (2 * f(x) + sin(x) - sin(f(x))) * f(x).diff(x),
            ...
            "1st_exact_07": {
                "eq": x * sqrt(x**2 + f(x) ** 2)
                - (x**2 * f(x) / (f(x) - sqrt(x**2 + f(x) ** 2))) * f(x).diff(x),
                "sol": [
                    Eq(
                        log(x),
                        C1
                ...
                "sol": [
                    Eq(
                        ...
                        + (
                            C2
                            + x
                            * (C3 - x / 8 + 3 * exp(I * x) / 2 + 3 * exp(-I * x) / 2)
                            + 5 * exp(2 * I * x) / 16
                            + 2 * I * exp(I * x)
                            - 2 * I * exp(-I * x)
                        )
            ...
            "var_of_parameters_14": {
                ...
                "sol": [
                    Eq(
                        ...
                        + (C4 + x * (C5 + I * x / 8) + 5 * I * exp(2 * I * x) / 16)
                        * cos(x)
                        - I * exp(I * x),
                    )
                ],
        ...
        "examples": {
            ...
            "2nd_lin_bessel_04": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1 * besselj(S(1) / 3, 9 * x) + C2 * bessely(S(1) / 3, 9 * x),
                    )
                ],
            },
            "2nd_lin_bessel_05": {
                ...
                + (x**4 - 4) * f(x),
                "sol": [
                    Eq(f(x), C1 * besselj(1, x**2 / 2) + C2 * bessely(1, x**2 / 2))
                ],
            },
            "2nd_lin_bessel_06": {
                "eq": x**2 * (f(x).diff(x, 2))
                ...
                "sol": [
                    Eq(
                        f(x),
                        (
                            C1 * besselj(sqrt(17) / 4, x**2 / 2)
                            + C2 * bessely(sqrt(17) / 4, x**2 / 2)
                        )
                        / sqrt(x),
                    )
                ],
            ...
            "2nd_lin_bessel_07": {
                ...
                + (x**2 - S(1) / 4) * f(x),
                "sol": [
                    Eq(f(x), C1 * besselj(S(1) / 2, x) + C2 * bessely(S(1) / 2, x))
                ],
            },
            "2nd_lin_bessel_08": {
                "eq": x**2 * (f(x).diff(x, 2))
                ...
                "sol": [
                    Eq(
                        f(x),
                        x**2
                        * (C1 * besselj(0, 4 * sqrt(x)) + C2 * bessely(0, 4 * sqrt(x))),
                    )
                ],
            },
            "2nd_lin_bessel_09": {
                ...
                "sol": [
                    Eq(
                        ...
                        x
                        * (
                            C1 * besselj(S(1) / 2, x**2)
                            + C2 * bessely(S(1) / 2, x**2)
                        ),
                    )
                ],
            },
            "2nd_lin_bessel_10": {
                "eq": (x - 2) ** 2 * (f(x).diff(x, 2))
                - (x - 2) * f(x).diff(x)
                + 4 * (x - 2) ** 2 * f(x),
                "sol": [
                    Eq(
            ...
            "2nd_lin_bessel_11": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        (C1 * besselj(S(1) / 2, x) + C2 * bessely(S(1) / 2, x))
                        / sqrt(x),
                    )
                ],
            },
        ...
        "examples": {
            "2nd_nonlinear_autonomous_conserved_01": {
                ...
                "sol": [
                    Eq(
                        Integral(
                            1 / sqrt(C1 - 2 * _u * log(_u) + 2 * _u - 2 * exp(_u)),
                            (_u, f(x)),
                        ),
                        C2 + x,
                    ),
            ...
            "2nd_nonlinear_autonomous_conserved_02": {
                ...
                "sol": [
                    Eq(
                        sqrt(2)
                        * Integral(
                            1 / sqrt(2 * C1 - 3 * _u ** Rational(4, 3) - 4 * log(_u)),
                            (_u, f(x)),
                        ),
                        C2 + x,
                    ),
            ...
            "2nd_nonlinear_autonomous_conserved_05": {
                ...
                "sol": [
                    Eq(
                        Integral(
                            1 / sqrt(C1 - 2 * _u * asin(_u) - 2 * sqrt(1 - _u**2)),
                            (_u, f(x)),
                        ),
                        C2 + x,
                    ),
    ...
    return {
        ...
        "examples": {
            ...
            "separable_reduced_03": {
                "eq": x * df + f(x) * (x**2 * f(x)),
                "sol": [
                    Eq(log(x**2 * f(x)) / 2 - log(x**2 * f(x) - 2) / 2, C1 + log(x))
                ],
                "simplify_flag": False,
            },
            "separable_reduced_04": {
                "eq": Eq(
                    f(x).diff(x) + f(x) / x * (1 + (x ** (S(2) / 3) * f(x)) ** 2), 0
                ),
                "sol": [
                    Eq(
                        -3 * log(x ** (S(2) / 3) * f(x))
                        + 3 * log(3 * x ** (S(4) / 3) * f(x) ** 2 + 1) / 2,
                        C1 + log(x),
                    )
                ],
                "simplify_flag": False,
    ...
    return {
        ...
        "examples": {
            ...
            "lie_group_10": {
                ...
                "XFAIL": [
                    "factorable"
                ],  # It raises Recursion Error (maixmum depth exceeded)
            },
            "lie_group_11": {
                "eq": x**2 * f(x) ** 2 + x * Derivative(f(x), x),
                "sol": [Eq(f(x), 2 / (C1 + x**2))],
            ...
            "lie_group_13": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        exp(-sin(x)) * (C1 + Integral(exp(2 * x) * exp(sin(x)), x)),
                    )
                ],
            },
            "lie_group_14": {
        ...
        "examples": {
            ...
            "lin_const_coeff_hom_06": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        C1 * exp(x * (-1 + sqrt(2))) + C2 * exp(-x * (sqrt(2) + 1)),
                    )
                ],
                "slow": True,
            },
            ...
            "lin_const_coeff_hom_27": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        (C1 + C2 * x) * sin(x * sqrt(2))
                        + (C3 + C4 * x) * cos(x * sqrt(2)),
                    )
                ],
                "slow": True,
            },
            "lin_const_coeff_hom_28": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        (C1 * sin(x * sqrt(3)) + C2 * cos(x * sqrt(3))) * exp(x)
                        + C3 * exp(-2 * x),
                    )
                ],
                "slow": True,
            ...
            "lin_const_coeff_hom_31": {
                ...
                "sol": [
                    Eq(
                        f(x),
                        (C1 * sin(sqrt(3) * x / 2) + C2 * cos(sqrt(3) * x / 2))
                        * exp(-x / 2)
                        + (C3 * sin(sqrt(3) * x / 2) + C4 * cos(sqrt(3) * x / 2))
                        * exp(x / 2),
                    )
                ],
                "slow": True,
            ...
            "lin_const_coeff_hom_33": {
                ...
                "sol": [
                    Eq(
                        ...
                        C5 * exp(r1 * x)
                        + exp(re(r2) * x)
                        * (C1 * sin(im(r2) * x) + C2 * cos(im(r2) * x))
                        + exp(re(r4) * x)
                        * (C3 * sin(im(r4) * x) + C4 * cos(im(r4) * x)),
                    )
                ],
            ...
            "lin_const_coeff_hom_36": {
                ...
                "sol": [
                    Eq(
                        ...
                        + C6 * exp(x * r16)
                        + exp(re(r17) * x)
                        * (C1 * sin(im(r17) * x) + C2 * cos(im(r17) * x))
                        + exp(re(r19) * x)
                        * (C3 * sin(im(r19) * x) + C4 * cos(im(r19) * x)),
                    )
                ],
                "checkodesol_XFAIL": True,  # It Hangs
            },
            ...
            "lin_const_coeff_hom_37": {
                ...
                "sol": [
                    Eq(
                        ...
                        (C1 + C2 * x) * exp(x * r21)
                        + (
                            -((C3 + C4 * x) * sin(x * im(r22)))
                            + (C5 + C6 * x) * cos(x * im(r22))
                        )
                        * exp(x * re(r22))
                        + (
                            -((C7 + C8 * x) * sin(x * im(r24)))
                            + (C10 * x + C9) * cos(x * im(r24))
                        )
                        * exp(x * re(r24)),
                    )
                ],
            ...
            "lin_const_coeff_hom_47": {
                ...
                "sol": [
                    Eq(f(x), (C1 * sin(x * sqrt(2)) + C2 * cos(x * sqrt(2))) * exp(-x))
                ],
            },
            # These were from issue: https://github.com/sympy/sympy/issues/6247
...
def _get_examples_ode_sol_1st_homogeneous_coeff_subs_dep_div_indep():
    return {
        ...
        "examples": {
            "dep_div_indep_01": {
                "eq": f(x) / x * cos(f(x) / x)
                - (x / f(x) * sin(f(x) / x) + cos(f(x) / x)) * f(x).diff(x),
            ...
            "dep_div_indep_03": {
                "eq": x * exp(f(x) / x)
                - f(x) * sin(f(x) / x)
                + x * sin(f(x) / x) * f(x).diff(x),
                "sol": [
                    Eq(
                        log(x),
                        C1
                        + exp(-f(x) / x) * sin(f(x) / x) / 2
                        + exp(-f(x) / x) * cos(f(x) / x) / 2,
                    )
                ],
                "slow": True,
            },
            "dep_div_indep_04": {
                ...
                "sol": [
                    Eq(f(x), x * (-acos(C1 + log(x)) + 2 * pi)),
                    Eq(f(x), x * acos(C1 + log(x))),
                ],
                "slow": True,
            },
    ...
    return {
        ...
        "examples": {
            ...
            "1st_homogeneous_coeff_best_05": {
                ...
                "sol": [
                    Eq(log(x), C1 - log(sqrt(1 + f(x) ** 2 / x**2)) + atan(f(x) / x))
                ],
            },
            ...
            "1st_homogeneous_coeff_best_08": {
                "eq": f(x) ** 2
                + (x * sqrt(f(x) ** 2 - x**2) - x * f(x)) * f(x).diff(x),
                "sol": [
                    Eq(f(x), -C1 * sqrt(-x / (x - 2 * C1))),
                    Eq(f(x), C1 * sqrt(-x / (x - 2 * C1))),
                ],
