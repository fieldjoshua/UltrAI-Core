...
def test_fdiff():
    ...
    expr_in = (
        4 * SingularityFunction(x, a, n)
        + 3 * SingularityFunction(x, a, -1)
        + -10 * SingularityFunction(x, a, 0)
    )
    expr_out = (
...
def test_rewrite():
    ...
    expr_in = (
        ...
        + SingularityFunction(x, a, -1)
        - SingularityFunction(x, a, -2)
    )
    expr_out = (
        (x - a) ** n * Heaviside(x - a, 1) + DiracDelta(x - a) + DiracDelta(a - x, 1)
    )
    assert expr_in.rewrite(Heaviside) == expr_out
    assert expr_in.rewrite(DiracDelta) == expr_out
    assert expr_in.rewrite("HeavisideDiracDelta") == expr_out
