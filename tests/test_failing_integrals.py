...
from sympy.abc import a, b, c, h, k, m, n, t, x, y, z
...
def test_issue_3880():
    ...
    assert not integrate(
        exp(x) * cos(2 * x) * sin(2 * x) * (x**3 + x**2) / (2 * (x**2 + x + 1)), x
    ).has(Integral)
...
def test_integrate_DiracDelta_no_meijerg():
    assert (
        integrate(
            integrate(
                integrate(DiracDelta(x - y - z), (z, 0, oo)), (y, 0, 1), meijerg=False
            ),
            (x, 0, 1),
        )
        == S.Half
...
def test_issue_4525():
    ...
    assert not integrate(
        (x**m * (1 - x) ** n * (a + b * x + c * x**2)) / (1 + x**2), (x, 0, 1)
    ).has(Integral)
...
def test_issue_4540():
    ...
    assert not integrate(
        (sin(1 / x) - x * exp(x)) / ((-sin(1 / x) + x * exp(x)) * x + x * sin(1 / x)), x
    ).has(Integral)
...
def test_issue_4992():
    ...
    assert not integrate(log(x) * x ** (k - 1) * exp(-x) / gamma(k), (x, 0, oo)).has(
        Integral
    )
...
def test_issue_15925a():
    assert not integrate(
        sqrt((1 + sin(x)) ** 2 + (cos(x)) ** 2), (x, -pi / 2, pi / 2)
    ).has(Integral)
...
def test_integrate_Piecewise_rational_over_reals():
    f = Piecewise(
        (0, t - 478.515625 * pi < 0),
        (
            13.2075145209219 * pi / (0.000871222 * t + 0.995) ** 2,
            t - 478.515625 * pi >= 0,
        ),
    )
...
def test_issue_20370():
    ...
    assert integrate((1 + a * cos(x)) ** -1, (x, 0, 2 * pi)) == (
        2 * pi / sqrt(1 - a**2)
    )
