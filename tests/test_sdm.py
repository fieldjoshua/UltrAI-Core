...
from sympy.polys.domains import EXRAW, QQ, ZZ
...
def test_SDM_from_list():
    ...
    raises(
        DMBadInputError, lambda: SDM.from_list([[ZZ(0)], [ZZ(0), ZZ(1)]], (2, 2), ZZ)
    )
...
def test_matmul_exraw():
    ...
    for a, b, c, d in product(*[values] * 4):
        ...
        Ad2 = dm(
            {
                0: {0: a * a + b * c, 1: a * b + b * d},
                1: {0: c * a + d * c, 1: c * b + d * d},
            }
        )
        assert Ad * Ad == Ad2
...
def test_SDM_rref():
    ...
    A_rref = SDM(
        {0: {0: QQ(1, 1), 2: QQ(4, 3)}, 1: {1: QQ(1, 1), 2: QQ(1, 3)}}, (2, 3), QQ
    )
    assert A.rref() == (A_rref, [0, 1])
