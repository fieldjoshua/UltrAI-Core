...
def test_rewrite1():
    assert _rewrite1(
        x**3 * meijerg([a], [b], [c], [d], x**2 + y * x**2) * 5, x
...
def test_recursive():
    ...
    assert simplify(e.expand()) == (
        sqrt(2)
        * sqrt(pi)
        * ((erf(sqrt(2) * (a + b) / 2) + 1) * exp(-(a**2) / 2 + a * b - b**2 / 2))
        / 4
    )
    e = integrate(
        exp(-((x - a) ** 2)) * exp(-((x - b) ** 2)) * exp(c * x),
        (x, 0, oo),
        meijerg=True,
    )
    assert simplify(e) == (
    ...
    assert simplify(
        integrate(exp(-((x - a - b - c) ** 2)), (x, 0, oo), meijerg=True)
...
def test_bessel():
    ...
    assert simplify(
        integrate(
            besselj(a, z) * besselj(b, z) / z, (z, 0, oo), meijerg=True, conds="none"
        )
    ) == 2 * sin(pi * (a / 2 - b / 2)) / (pi * (a - b) * (a + b))
    assert simplify(
        integrate(
            besselj(a, z) * besselj(a, z) / z, (z, 0, oo), meijerg=True, conds="none"
        )
    ) == 1 / (2 * a)
    ...
    assert (
        integrate(besselj(1, x) ** 2 / x, x, meijerg=True)
    ...
    assert (
        integrate(besselj(0, x) * besselj(1, x), x, meijerg=True)
        == -besselj(0, x) ** 2 / 2
    )
    assert (
        integrate(x**2 * besselj(0, x) * besselj(1, x), x, meijerg=True)
        == x**2 * besselj(1, x) ** 2 / 2
    )
...
def test_lookup_table():
    from sympy.core.add import Add
    from sympy.core.random import randrange, uniform
...
def test_branch_bug():
    ...
    assert powdenest(
        integrate(erf(x**3), x, meijerg=True).diff(x), polar=True
    ) == 2 * erf(x**3) * gamma(Rational(2, 3)) / 3 / gamma(Rational(5, 3))
    assert integrate(erf(x**3), x, meijerg=True) == 2 * x * erf(x**3) * gamma(
        Rational(2, 3)
    ) / (3 * gamma(Rational(5, 3))) - 2 * gamma(Rational(2, 3)) * lowergamma(
        Rational(2, 3), x**6
    ) / (
        3 * sqrt(pi) * gamma(Rational(5, 3))
    )
...
def test_probability():
    ...
    assert (
        integrate(x**2 * exponential(x, rate), (x, 0, oo), meijerg=True) == 2 / rate**2
    )
...
def test_messy():
    ...
    from sympy.functions.special.bessel import besselj
    from sympy.functions.special.error_functions import E1, Chi, Shi, Si
    ...
    assert integrate(E1(x) * besselj(0, x), (x, 0, oo), meijerg=True) == log(
        1 + sqrt(2)
    )
    assert integrate(E1(x) * besselj(1, x), (x, 0, oo), meijerg=True) == log(
        S.Half + sqrt(2) / 2
    )
...
def test_issue_6122():
    assert integrate(exp(-I * x**2), (x, -oo, oo), meijerg=True) == -I * sqrt(
        pi
    ) * exp(I * pi / 4)
...
def test_issue_6348():
    assert integrate(exp(I * x) / (1 + x**2), (x, -oo, oo)).simplify().rewrite(
        exp
    ) == pi * exp(-1)
...
def test_issue_11806():
    ...
    assert integrate(1 / sqrt(x**2 + y**2) ** 3, (x, -L, L)) == 2 * L / (
        y**2 * sqrt(L**2 + y**2)
    )
...
def test_indefinite_1_bug():
    assert integrate((b + t) ** (-a), t, meijerg=True) == -b * (1 + t / b) ** (
        1 - a
    ) / (a * b**a - b**a)
...
def test_issue_25949():
    ...
    assert (
        integrate(cosh(y * (x + 1)), (x, -1, -0.25), meijerg=True) == sinh(0.75 * y) / y
    )
