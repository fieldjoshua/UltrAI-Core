...
def test_divergence():
    ...
    assert (
        divergence(R[0] * R[1] * R[2] * (R.x + R.y + R.z), R)
        == R[0] * R[1] + R[0] * R[2] + R[1] * R[2]
    )
    assert divergence((1 / (R[0] * R[1] * R[2])) * (R.x + R.y + R.z), R) == -1 / (
        R[0] * R[1] * R[2] ** 2
    ) - 1 / (R[0] * R[1] ** 2 * R[2]) - 1 / (R[0] ** 2 * R[1] * R[2])
...
def test_conservative():
    ...
    assert (
        is_conservative(4 * R[0] * R[1] * R[2] * R.x + 2 * R[0] ** 2 * R[2] * R.y)
        is False
    )
...
def test_solenoidal():
    ...
    assert (
        is_solenoidal(R[1] * R[2] * R.x + R[0] * R[2] * R.y + R[0] * R[1] * R.z) is True
    )
...
def test_scalar_potential():
    ...
    assert (
        scalar_potential(R[1] * R[2] * R.x + R[0] * R[2] * R.y + R[0] * R[1] * R.z, R)
        == R[0] * R[1] * R[2]
    )
    assert scalar_potential(grad_field, R) == scalar_field
    assert scalar_potential(R[2] * P.x + P[0] * R.z, R) == R[0] * R[2] * cos(q) + R[
        1
    ] * R[2] * sin(q)
...
def test_scalar_potential_difference():
    ...
    assert (
        scalar_potential_difference(scalar_field, R, origin, genericpointR, origin)
        == scalar_field
    )
    assert (
        scalar_potential_difference(grad_field, R, origin, genericpointR, origin)
        == scalar_field
    )
    ...
    assert (
        scalar_potential_difference(
            grad_field, P, origin, genericpointP, origin
        ).simplify()
        == potential_diff_P
    )
