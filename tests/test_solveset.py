...
def test_solve_trig_hyp_by_inversion():
    ...
    assert solveset(eq, z, S.Complexes).dummy_eq(
        Union(
            ImageSet(
                Lambda(n, n * I * pi + acoth(CRootOf(x**5 - 4 * x + 1, 0))), S.Integers
            ),
            ImageSet(
                Lambda(n, n * I * pi + acoth(CRootOf(x**5 - 4 * x + 1, 1))), S.Integers
            ),
            ImageSet(
                Lambda(n, n * I * pi + acoth(CRootOf(x**5 - 4 * x + 1, 2))), S.Integers
            ),
            ImageSet(
                Lambda(n, n * I * pi + acoth(CRootOf(x**5 - 4 * x + 1, 3))), S.Integers
            ),
            ImageSet(
                Lambda(n, n * I * pi + acoth(CRootOf(x**5 - 4 * x + 1, 4))), S.Integers
            ),
        )
    )
...
def test_issue_5132_2():
    ...
    lam = Lambda(
        n, I * (2 * n * pi + arg(-(z**2) + sin(y))) / 2 + log(Abs(z**2 - sin(y))) / 2
    )
...
def test_issue_14987():
    ...
    raises(
        ValueError, lambda: linear_eq_to_matrix([(x + 1) ** 3 - x**3 - 3 * x**2 + 7], x)
    )
...
def test_issue_17882():
    assert solveset(
        -8 * x**2 / (9 * (x**2 - 1) ** (S(4) / 3)) + 4 / (3 * (x**2 - 1) ** (S(1) / 3)),
        x,
        S.Complexes,
