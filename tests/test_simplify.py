from sympy.abc import a, b, c, d, e, f, g, h, i, n, t, x, y, z
...
from sympy.core.numbers import E, Float, I, Rational, oo, pi, zoo
...
from sympy.testing.pytest import XFAIL, _both_exp_pow, slow
...
def test_posify():
    ...
    assert (
        str(Sum(posify(1 / x**n)[0], (n, 1, 3)).expand())
        == "Sum(_x**(-n), (n, 1, 3))"
    )
...
def test_nc_simplify():
    ...
    from sympy.matrices.expressions import Identity, MatPow
    ...
    _check(
        a**3 * b * a**4 * b * a**4 * b * a, a**3 * (b * a**4) ** 3 * a**-3
    )
    _check(
