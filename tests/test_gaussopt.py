...
def test_gauss_opt():
    ...
    assert CurvedRefraction(R, n1, n2) == Matrix(
    ...
    assert gaussian_conj(s_in, z_r_in, f)[0] == 1 / (
        -1 / (s_in + z_r_in**2 / (-f + s_in)) + 1 / f
    )
    assert gaussian_conj(s_in, z_r_in, f)[1] == z_r_in / (
        1 - s_in**2 / f**2 + z_r_in**2 / f**2
    )
    assert gaussian_conj(s_in, z_r_in, f)[2] == 1 / sqrt(
        1 - s_in**2 / f**2 + z_r_in**2 / f**2
    )
    ...
    assert (
        ...
        * (
            w_i**2 / w_o**2
            - sqrt(w_i**2 / w_o**2 - pi**2 * w_i**4 / (f**2 * l**2))
        )
        / w_i**2
    )
