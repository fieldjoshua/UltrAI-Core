...
def test_pin_joint():
    ...
    assert (
        C.masscenter.vel(N).express(N)
        == (u * sin(q) - u * cos(q)) * N.x + (-u * sin(q) - u * cos(q)) * N.y
    )
...
def test_particle_compatibility():
    ...
    assert C_frame.dcm(J.parent_interframe) == Matrix(
        [[1, 0, 0], [0, cos(q), sin(q)], [0, -sin(q), cos(q)]]
    )
...
def test_body_compatibility():
    ...
    assert C.frame.dcm(P.frame) == Matrix(
        [[1, 0, 0], [0, cos(q), sin(q)], [0, -sin(q), cos(q)]]
    )
...
def test_pin_joint_double_pendulum():
    ...
    assert simplify(N.dcm(B)) == Matrix(
        [[cos(q1 + q2), -sin(q1 + q2), 0], [sin(q1 + q2), cos(q1 + q2), 0], [0, 0, 1]]
    )
...
def test_pin_joint_chaos_pendulum():
    ...
    assert (
        ...
        == ((h / 4 - lB / 2) * omega + (h / 4 + lB / 2) * omega) * A.x
    )
...
def test_pin_joint_interframe():
    ...
    assert (
        C.masscenter.vel(N).simplify()
        == (-sqrt(6) - sqrt(2)) * u / 4 * A.y + (-sqrt(2) + sqrt(6)) * u / 4 * A.z
    )
...
def test_pin_joint_joint_axis():
    ...
    N, A, P, C, Pint, Cint = _generate_body(True)
    pin = PinJoint(
        "J", P, C, q, u, parent_interframe=Pint, child_interframe=Cint, joint_axis=P.y
    )
    ...
    N, A, P, C = _generate_body()
    pin = PinJoint(
        "J", P, C, q, u, parent_interframe=N.z, child_interframe=-C.z, joint_axis=N.z
    )
    assert pin.joint_axis == N.z
    assert N.dcm(A) == Matrix([[-cos(q), -sin(q), 0], [-sin(q), cos(q), 0], [0, 0, -1]])
    N, A, P, C = _generate_body()
    pin = PinJoint(
        "J", P, C, q, u, parent_interframe=N.z, child_interframe=-C.z, joint_axis=N.x
    )
    ...
    N, A, P, C, Pint, Cint = _generate_body(True)
    raises(
        ValueError, lambda: PinJoint("J", P, C, joint_axis=cos(q) * N.x + sin(q) * N.y)
    )
...
def test_pin_joint_arbitrary_axis():
    ...
    N, A, P, C = _generate_body()
    PinJoint(
        "J", P, C, parent_point=N.x, child_point=A.x, child_interframe=A.x + A.y - A.z
    )
    ...
    assert simplify(A.dcm(N)) == Matrix(
        [
            ...
            [
                sqrt(3) / 3,
                sqrt(6) * cos(q + pi / 12) / 3,
                sqrt(6) * sin(q + pi / 12) / 3,
            ],
            [
                -sqrt(3) / 3,
                sqrt(6) * cos(q + 5 * pi / 12) / 3,
    ...
    assert (A.x - A.y + A.z).express(N).simplify() == (
        -4 * cos(q) / 3 - S(1) / 3
    ) * N.x + (S(1) / 3 - 4 * sin(q + pi / 6) / 3) * N.y + (
        4 * cos(q + pi / 3) / 3 - S(1) / 3
    ) * N.z
    assert simplify(A.dcm(N)) == Matrix(
        [
            [
...
def test_pin_joint_axis():
    ...
    N, A, P, C, Pint, Cint = _generate_body(True)
    PinJoint(
        "J", P, C, q, u, parent_interframe=Pint, child_interframe=Cint, joint_axis=N.z
    )
...
def test_prismatic_joint():
    ...
    S = PrismaticJoint(
        ...
        P,
        C,
        parent_point=l * P.frame.x,
        child_point=m * C.frame.y,
        joint_axis=P.frame.z,
        parent_interframe=Pint,
    )
    ...
    S = PrismaticJoint(
        ...
        P,
        C,
        parent_point=l * P.frame.z,
        child_point=m * C.frame.x,
        joint_axis=P.frame.z,
        parent_interframe=Pint,
    )
    assert S.joint_axis == P.frame.z
...
def test_prismatic_joint_arbitrary_axis():
    ...
    N, A, P, C = _generate_body()
    PrismaticJoint(
        "S", P, C, parent_point=N.x, child_point=A.x, child_interframe=A.x + A.y
    )
    ...
    N, A, P, C = _generate_body()
    PrismaticJoint(
        "S", P, C, parent_point=N.x, child_point=A.x, child_interframe=A.x + A.y - A.z
    )
    ...
    assert C.masscenter.vel(N) == u * N.x
    assert (
        C.masscenter.vel(N).express(A)
        - (sqrt(3) * u / 3 * A.x + sqrt(3) * u / 3 * A.y - sqrt(3) * u / 3 * A.z)
    ).simplify()
    assert A.ang_vel_in(N) == 0
    assert N.ang_vel_in(A) == 0
    ...
    assert (
        C.masscenter.vel(N).express(N)
        - (sqrt(3) * u / 3 * N.x - sqrt(3) * u / 3 * N.y + sqrt(3) * u / 3 * N.z)
    ).simplify() == 0
    assert (
        C.masscenter.vel(N).express(A)
        - (sqrt(3) * u / 3 * A.x + sqrt(3) * u / 3 * A.y - sqrt(3) * u / 3 * A.z)
...
def test_planar_joint():
    ...
    assert Cj.kdes == Matrix(
        [u0_def - q0_def.diff(t), u1_def - q1_def.diff(t), u2_def - q2_def.diff(t)]
    )
    ...
    assert (
        C.masscenter.pos_from(P.masscenter) == m * N.x - q1 * N.y - q2 * N.z - l * A.y
    )
...
def test_spherical_joint_coords():
    ...
    N, A, P, C = _generate_body()
    raises(
        ValueError, lambda: SphericalJoint("S", P, C, Matrix([q0, q1]), Matrix([u0]))
    )
    # Test too many generalized coordinates
    raises(
        ValueError,
        lambda: SphericalJoint(
...
def test_deprecated_joint_pos():
    ...
    with warns_deprecated_sympy():
        slider = PrismaticJoint(
            "J", P, C, parent_joint_pos=N.z + N.y, child_joint_pos=C.y - C.x
        )
