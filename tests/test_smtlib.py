...
from sympy.functions import Piecewise, cos, exp, sin
from sympy.printing.smtlib import smtlib_code
from sympy.testing.pytest import Failed, raises
...
class _W(Enum):
    DEFAULTING_TO_FLOAT = re.compile(
        "Could not infer type of `.+`. Defaulting to float.", re.I
    )
    WILL_NOT_DECLARE = re.compile(
        "Non-Symbol/Function `.+` will not be declared.", re.I
    )
...
def test_smtlib_piecewise():
    ...
    with _check_warns([_W.DEFAULTING_TO_FLOAT, _W.WILL_NOT_ASSERT]) as w:
        assert (
            smtlib_code(
                Piecewise(
                    (x**2, x < 1), (x**3, x < 2), (x**4, x < 3), (x**5, True)
                ),
                auto_declare=False,
                log_warn=w,
            )
