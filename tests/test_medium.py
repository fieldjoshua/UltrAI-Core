...
def test_medium():
    ...
    m4 = Medium(
        "m4",
        7.0 * 10 ** (-12) * s**4 * A**2 / (m**3 * kg),
        1.15 * 10 ** (-6) * kg * m / (A**2 * s**2),
    )
    assert m4.refractive_index < m1.refractive_index
    m5 = Medium(
        "m5", permittivity=710 * 10 ** (-12) * s**4 * A**2 / (m**3 * kg), n=1.33
    )
    assert abs(
        m5.intrinsic_impedance - 6.24845417765552 * kg * m**2 / (A**2 * s**3)
    ...
    assert abs(
        m5.permittivity - 7.1e-10 * A**2 * s**4 / (kg * m**3)
    ) < 1e-20 * A**2 * s**4 / (kg * m**3)
    assert abs(
        m5.permeability - 2.77206575232851e-8 * kg * m / (A**2 * s**2)
    ) < 1e-20 * kg * m / (A**2 * s**2)
