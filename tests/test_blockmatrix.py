from sympy.core import Expr, Function, S, Tuple
from sympy.core.symbol import Symbol, symbols
from sympy.functions import im, re, transpose
from sympy.matrices import ImmutableMatrix, ImmutableSparseMatrix, Matrix, zeros
...
def test_bc_matadd():
    assert bc_matadd(BlockMatrix([[G, H]]) + BlockMatrix([[H, H]])) == BlockMatrix(
        [[G + H, H + H]]
    )


def test_bc_transpose():
    assert bc_transpose(Transpose(BlockMatrix([[A, B], [C, D]]))) == BlockMatrix(
        [[A.T, C.T], [B.T, D.T]]
    )
...
def test_block_plus_ident():
    ...
    assert (
        bc_block_plus_ident(X + Identity(m + n) + Z)
...
def test_squareBlockMatrix():
    ...
    Q = X + Identity(m + n)
    assert block_collapse(Q) == BlockMatrix(
...
def test_BlockMatrix_2x2_inverse_symbolic():
    ...
    assert isinstance(
        block_collapse(X.I), Inverse
    )  # Can't invert when none of the blocks is square
...
def test_BlockDiagMatrix():
    ...
    assert all(
        X.blocks[i, j].is_ZeroMatrix if i != j else X.blocks[i, j] in [A, B, C]
        for i in range(3)
        for j in range(3)
    )
    assert X.__class__(*X.args) == X
...
def test_issue_2460():
    ...
    assert block_collapse(bdm1 + bdm2) == BlockDiagMatrix(
...
def test_reblock_2x2():
    B = BlockMatrix(
        [[MatrixSymbol("A_%d%d" % (i, j), 2, 2) for j in range(3)] for i in range(3)]
    )
...
def test_deblock():
    B = BlockMatrix(
        [[MatrixSymbol("A_%d%d" % (i, j), n, n) for j in range(4)] for i in range(4)]
    )
...
def test_adjoint_and_special_matrices():
    ...
    assert re(X) == X
    assert X2.adjoint() == BlockMatrix(
        [[A, ZeroMatrix(3, 2)], [-S.ImaginaryUnit * OneMatrix(2, 3), D]]
    )
    assert im(X2) == BlockMatrix(
