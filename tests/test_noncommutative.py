...
def test_conjugate():
    ...
    assert (A * B - B * A).conjugate() == conjugate(A) * conjugate(B) - conjugate(
        B
    ) * conjugate(A)
    assert (A * B).conjugate() - (B * A).conjugate() == conjugate(A) * conjugate(
        B
...
def test_transpose():
    ...
    assert transpose(A * B - B * A) == transpose(B) * transpose(A) - transpose(
        A
    ) * transpose(B)
