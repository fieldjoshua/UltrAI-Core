...
from sympy.core.function import ArgumentIndexError, PoleError, expand_mul, expand_trig
...
def test_asinh_nseries():
    x = Symbol("x")
    # Tests concerning branch points
    assert asinh(x + I)._eval_nseries(x, 4, None) == I * pi / 2 + sqrt(x) * (
        1 - I
    ) + x ** (S(3) / 2) * (S(1) / 12 + I / 12) + x ** (S(5) / 2) * (
        -S(3) / 160 + 3 * I / 160
    ) + x ** (
        S(7) / 2
    ) * (
...
def test_acosh_leading_term():
    ...
    assert acosh(x**2 - I * x + S(1) / 3).as_leading_term(x, cdir=1) == -acosh(
        S(1) / 3
    )
    assert acosh(x**2 - I * x + S(1) / 3).as_leading_term(x, cdir=-1) == acosh(
        S(1) / 3
...
def test_acsch_nseries():
    x = Symbol("x")
    # Tests concerning branch points
    assert acsch(x + I)._eval_nseries(x, 4, None) == -I * pi / 2 + I * sqrt(x) + sqrt(
        x
    ) + 5 * I * x ** (S(3) / 2) / 12 - 5 * x ** (S(3) / 2) / 12 - 43 * I * x ** (
        S(5) / 2
    ) / 160 - 43 * x ** (
        S(5) / 2
    ) / 160 - 177 * I * x ** (
...
def test_acsch_rewrite():
    ...
    assert acsch(x).rewrite(atanh) == (
        sqrt(-(x**2))
        * (
            -sqrt(-((x**2 + 1) ** 2)) * atanh(sqrt(x**2 + 1)) / (x**2 + 1)
            + pi / 2
        )
        / x
    )
...
def test_atanh_leading_term():
    ...
    # Tests concerning im(ndir) == 0
    assert (
        atanh(-I * x**2 + x - 2).as_leading_term(x, cdir=1)
        == -log(3) / 2 - I * pi / 2
    )
    assert (
        atanh(-I * x**2 + x - 2).as_leading_term(x, cdir=-1)
