...
from sympy.sets.fancysets import ComplexRegion, ImageSet, Range, normalize_theta_set
...
def test_ImageSet():
    ...
    assert harmonics.contains(0.25) == Contains(
    ...
    assert Tuple(Rational(1, 8), 3, 9) in ImageSet(
        Lambda(((t, y, x),), (1 / y, t, x)), c3
    )
...
def test_Range_set():
    ...
    assert Range(oo, 1, -2).as_relational(x) == (x >= 3) & (x < oo) & Eq(
        Mod(x + 1, -2), 0
    )
...
def test_Integers_eval_imageset():
    ...
    assert imageset(x, (x, 1 / x), S.Integers) == ImageSet(
        Lambda(x, (x, 1 / x)), S.Integers
    )


def test_Range_eval_imageset():
    a, b, c = symbols("a b c")
    assert imageset(x, a * (x + b) + c, Range(3)) == imageset(
        x, a * x + a * b + c, Range(3)
    )
    eq = (x + 1) ** 2
...
def test_fun():
    assert FiniteSet(
        *ImageSet(Lambda(x, sin(pi * x / 4)), Range(-10, 11))
...
def test_infinitely_indexed_set_1():
    ...
    assert (
        imageset(Lambda(n, 2 * n), S.Integers).intersect(
            imageset(Lambda(m, 2 * m + 1), S.Integers)
        )
        is S.EmptySet
    )
    ...
    assert (
        imageset(x, x / 2 + Rational(1, 3), S.Integers).intersect(S.Integers)
        is S.EmptySet
    )
...
def test_infinitely_indexed_set_2():
    ...
    assert imageset(Lambda(n, n + pi), S.Integers) == imageset(
        Lambda(n, n + a + pi), S.Integers
    )
    assert imageset(Lambda(n, n), S.Integers) == imageset(Lambda(n, -n + a), S.Integers)
    assert imageset(Lambda(n, -6 * n), S.Integers) == ImageSet(
        Lambda(n, 6 * n), S.Integers
    )
    assert imageset(Lambda(n, 2 * n + pi), S.Integers) == ImageSet(
        Lambda(n, 2 * n + pi - 2), S.Integers
    )
...
def test_imageset_intersect_real():
    ...
    assert imageset(Lambda(n, n + (n - 1) * (n + 1) * I), S.Integers).intersect(
        S.Reals
    ) == FiniteSet(-1, 1)
    im = (n - 1) * (n + S.Half)
    assert imageset(Lambda(n, n + im * I), S.Integers).intersect(S.Reals) == FiniteSet(
        1
    )
    assert imageset(Lambda(n, n + im * (n + 1) * I), S.Naturals0).intersect(
        S.Reals
    ) == FiniteSet(1)
    assert imageset(Lambda(n, n / 2 + im.expand() * I), S.Integers).intersect(
        S.Reals
    ) == ImageSet(
        Lambda(x, x / 2), ConditionSet(n, Eq(n**2 - n / 2 - S(1) / 2, 0), S.Integers)
    )
    assert imageset(
        Lambda(n, n / (1 / n - 1) + im * (n + 1) * I), S.Integers
    ).intersect(S.Reals) == FiniteSet(S.Half)
    assert imageset(
        Lambda(n, n / (n - 6) + (n - 3) * (n + 1) * I / (2 * n + 2)), S.Integers
    ).intersect(S.Reals) == FiniteSet(-1)
    assert (
        imageset(
            Lambda(n, n / (n**2 - 9) + (n - 3) * (n + 1) * I / (2 * n + 2)), S.Integers
        ).intersect(S.Reals)
        is S.EmptySet
    )
    s = ImageSet(
        Lambda(n, -I * (I * (2 * pi * n - pi / 4) + log(Abs(sqrt(-I))))), S.Integers
    )
...
def test_infinitely_indexed_set_3():
    ...
    assert imageset(Lambda(n, 2 * n + 1), S.Integers) == imageset(
        Lambda(n, 2 * n - 1), S.Integers
    )
    assert imageset(Lambda(n, 3 * n + 2), S.Integers) == imageset(
        Lambda(n, 3 * n - 1), S.Integers
    )
...
def test_ImageSet_simplification():
    ...
    assert imageset(
        Lambda(n, sin(n)), imageset(Lambda(m, tan(m)), S.Integers)
...
def test_ComplexRegion_contains():
    ...
    assert c3.contains(r + 2 * I) == Contains(
        r + 2 * I, c3, evaluate=False
    )  # is in fact False
    assert c3.contains(1 / (1 + r**2)) == Contains(
        1 / (1 + r**2), c3, evaluate=False
    )  # is in fact True
...
def test_ComplexRegion_intersect():
    ...
    lower_half_disk = ComplexRegion(
        Interval(0, oo) * Interval(S.Pi, 2 * S.Pi), polar=True
    )
    right_half_disk = ComplexRegion(
        Interval(0, oo) * Interval(-S.Pi / 2, S.Pi / 2), polar=True
    )
...
def test_ComplexRegion_union():
    ...
    p1 = Union(
        Interval(0, 1) * Interval(0, 2 * S.Pi), Interval(0, 1) * Interval(0, S.Pi)
    )
    p2 = Union(
        Interval(0, oo) * Interval(0, S.Pi), Interval(0, oo) * Interval(S.Pi, 2 * S.Pi)
    )
    ...
    assert c5.union(Interval(2, 4)) == Union(
        c5, ComplexRegion.from_real(Interval(2, 4))
    )
...
def test_normalize_theta_set():
    # Interval
    assert normalize_theta_set(Interval(pi, 2 * pi)) == Union(
        FiniteSet(0), Interval.Ropen(pi, 2 * pi)
    )
    assert normalize_theta_set(Interval(pi * Rational(9, 2), 5 * pi)) == Interval(
        pi / 2, pi
    )
    assert normalize_theta_set(
        Interval(pi * Rational(-3, 2), pi / 2)
    ) == Interval.Ropen(0, 2 * pi)
    assert normalize_theta_set(Interval.open(pi * Rational(-3, 2), pi / 2)) == Union(
        Interval.Ropen(0, pi / 2), Interval.open(pi / 2, 2 * pi)
    )
    assert normalize_theta_set(
        Interval.open(pi * Rational(-7, 2), pi * Rational(-3, 2))
    ...
    assert normalize_theta_set(Interval(pi * Rational(-3, 2), -pi / 2)) == Interval(
    ...
    assert normalize_theta_set(
        Interval.open(4 * pi, pi * Rational(9, 2))
    ) == Interval.open(0, pi / 2)
    assert normalize_theta_set(
        Interval.Lopen(4 * pi, pi * Rational(9, 2))
    ) == Interval.Lopen(0, pi / 2)
    assert normalize_theta_set(
        Interval.Ropen(4 * pi, pi * Rational(9, 2))
    ) == Interval.Ropen(0, pi / 2)
    assert normalize_theta_set(Interval.open(3 * pi, 5 * pi)) == Union(
        Interval.Ropen(0, pi), Interval.open(pi, 2 * pi)
    )

    # FiniteSet
    assert normalize_theta_set(FiniteSet(0, pi, 3 * pi)) == FiniteSet(0, pi)
    assert normalize_theta_set(FiniteSet(0, pi / 2, pi, 2 * pi)) == FiniteSet(
        0, pi / 2, pi
    )
    assert normalize_theta_set(FiniteSet(0, -pi / 2, -pi, -2 * pi)) == FiniteSet(
        0, pi, pi * Rational(3, 2)
    )
    assert normalize_theta_set(FiniteSet(pi * Rational(-3, 2), pi / 2)) == FiniteSet(
        pi / 2
    )
    ...
    assert normalize_theta_set(
        Union(Interval(0, pi), Interval(2 * pi, pi * Rational(7, 3)))
    ) == Interval(0, pi)
...
def test_issue_9980():
    ...
    R = Union(c1, c2)
    assert simplify(R) == ComplexRegion(
        Union(Interval(1, 2) * Interval(2, 3), Interval(1, 5) * Interval(1, 3)), False
    )
...
def test_issue_18050():
    assert imageset(Lambda(x, I * x + 1), S.Integers) == ImageSet(
        Lambda(x, I * x + 1), S.Integers
    )
    assert imageset(Lambda(x, 3 * I * x + 4 + 8 * I), S.Integers) == ImageSet(
        Lambda(x, 3 * I * x + 4 + 2 * I), S.Integers
    )
    # no 'Mod' for next 2 tests:
    assert imageset(Lambda(x, 2 * x + 3 * I), S.Integers) == ImageSet(
        Lambda(x, 2 * x + 3 * I), S.Integers
    )
    r = Symbol("r", positive=True)
    assert imageset(Lambda(x, r * x + 10), S.Integers) == ImageSet(
        Lambda(x, r * x + 10), S.Integers
    )
    # reduce real part:
    assert imageset(Lambda(x, 3 * x + 8 + 5 * I), S.Integers) == ImageSet(
        Lambda(x, 3 * x + 2 + 5 * I), S.Integers
    )
