...
def test_deltaproduct_mul_add_x_y_kd():
    ...
    assert dp((x + y) * KD(i, j), (j, k, l)) == (x + y) * KD(i, l) * KD(k, l) + KD(
        k, l + 1
    )
...
def test_deltaproduct_add_kd_kd():
    ...
    assert dp(KD(i, k) + KD(j, k), (k, 1, l)) == KD(l, 0) + KD(i, 1) * KD(l, 1) + KD(
        j, 1
    ) * KD(l, 1) + KD(i, 1) * KD(j, 2) * KD(l, 2) + KD(j, 1) * KD(i, 2) * KD(l, 2)
    assert dp(KD(i, k) + KD(j, k), (k, l, 3)) == KD(l, 4) + KD(i, 3) * KD(l, 3) + KD(
        j, 3
    ) * KD(l, 3) + KD(i, 2) * KD(j, 3) * KD(l, 2) + KD(i, 3) * KD(j, 2) * KD(l, 2)
    assert dp(KD(i, k) + KD(j, k), (k, l, m)) == KD(l, m + 1) + KD(i, m) * KD(
        l, m
    ) + KD(j, m) * KD(l, m) + KD(i, m) * KD(j, m - 1) * KD(l, m - 1) + KD(
        i, m - 1
    ) * KD(
        j, m
    ) * KD(
...
def test_deltaproduct_mul_x_add_kd_kd():
    ...
    assert dp(x * (KD(i, k) + KD(j, k)), (k, 1, l)) == KD(l, 0) + x * KD(i, 1) * KD(
        l, 1
    ) + x * KD(j, 1) * KD(l, 1) + x**2 * KD(i, 1) * KD(j, 2) * KD(l, 2) + x**2 * KD(
        j, 1
    ) * KD(
        i, 2
    ) * KD(
    ...
    assert dp(x * (KD(i, k) + KD(j, k)), (k, l, 3)) == KD(l, 4) + x * KD(i, 3) * KD(
        l, 3
    ) + x * KD(j, 3) * KD(l, 3) + x**2 * KD(i, 2) * KD(j, 3) * KD(l, 2) + x**2 * KD(
        i, 3
    ) * KD(
        j, 2
    ) * KD(
    ...
    assert dp(x * (KD(i, k) + KD(j, k)), (k, l, m)) == KD(l, m + 1) + x * KD(i, m) * KD(
        l, m
    ) + x * KD(j, m) * KD(l, m) + x**2 * KD(i, m - 1) * KD(j, m) * KD(
        l, m - 1
    ) + x**2 * KD(
        i, m
    ) * KD(
...
def test_deltaproduct_mul_add_x_y_add_kd_kd():
    ...
    assert dp((x + y) * (KD(i, k) + KD(j, k)), (k, 1, 1)) == (x + y) * (
        KD(i, 1) + KD(j, 1)
    )
    assert dp((x + y) * (KD(i, k) + KD(j, k)), (k, 2, 2)) == (x + y) * (
        KD(i, 2) + KD(j, 2)
    )
    assert dp((x + y) * (KD(i, k) + KD(j, k)), (k, 3, 3)) == (x + y) * (
        KD(i, 3) + KD(j, 3)
    )
    assert dp((x + y) * (KD(i, k) + KD(j, k)), (k, 1, l)) == KD(l, 0) + (x + y) * KD(
        i, 1
    ) * KD(l, 1) + (x + y) * KD(j, 1) * KD(l, 1) + (x + y) ** 2 * KD(i, 1) * KD(
        j, 2
    ) * KD(
        l, 2
    ) + (
    ...
    assert dp((x + y) * (KD(i, k) + KD(j, k)), (k, l, 3)) == KD(l, 4) + (x + y) * KD(
        i, 3
    ) * KD(l, 3) + (x + y) * KD(j, 3) * KD(l, 3) + (x + y) ** 2 * KD(i, 2) * KD(
        j, 3
    ) * KD(
        l, 2
    ) + (
    ...
    assert dp((x + y) * (KD(i, k) + KD(j, k)), (k, l, m)) == KD(l, m + 1) + (
        x + y
    ) * KD(i, m) * KD(l, m) + (x + y) * KD(j, m) * KD(l, m) + (x + y) ** 2 * KD(
        i, m - 1
    ) * KD(
        j, m
    ) * KD(
...
def test_deltaproduct_add_mul_x_y_mul_x_kd():
    assert dp(x * y + x * KD(i, j), (j, 1, 3)) == (x * y) ** 3 + x * (x * y) ** 2 * KD(
        i, 1
...
def test_deltaproduct_mul_x_add_y_kd():
    assert dp(x * (y + KD(i, j)), (j, 1, 3)) == (x * y) ** 3 + x * (x * y) ** 2 * KD(
        i, 1
    ) + (x * y) * x * (x * y) * KD(i, 2) + (x * y) ** 2 * x * KD(i, 3)
    assert dp(x * (y + KD(i, j)), (j, 1, 1)) == x * (y + KD(i, 1))
    assert dp(x * (y + KD(i, j)), (j, 2, 2)) == x * (y + KD(i, 2))
...
def test_deltaproduct_mul_x_add_y_twokd():
    assert dp(x * (y + 2 * KD(i, j)), (j, 1, 3)) == (x * y) ** 3 + 2 * x * (
        x * y
    ) ** 2 * KD(i, 1) + 2 * x * y * x * x * y * KD(i, 2) + 2 * (x * y) ** 2 * x * KD(
        i, 3
    )
...
def test_deltaproduct_mul_add_x_y_add_y_kd():
    assert dp((x + y) * (y + KD(i, j)), (j, 1, 3)) == ((x + y) * y) ** 3 + (x + y) * (
        (x + y) * y
    ) ** 2 * KD(i, 1) + (x + y) * y * (x + y) ** 2 * y * KD(i, 2) + (
        (x + y) * y
    ) ** 2 * (
        x + y
    ) * KD(
    ...
    assert dp((x + y) * (y + KD(i, j)), (j, k, 3)) == ((x + y) * y) ** 4 * (
        (x + y) * y
    ) ** (-k) + Piecewise(
        (
            ((x + y) * y) ** i
    ...
    assert dp((x + y) * (y + KD(i, j)), (j, k, l)) == (x + y) * y * (
        (x + y) * y
    ) ** l * ((x + y) * y) ** (-k) + Piecewise(
        (
            ((x + y) * y) ** i
...
def test_deltaproduct_mul_add_x_kd_add_y_kd():
    ...
    assert dp((x + KD(i, k)) * (y + KD(i, j)), (j, 1, 1)) == (x + KD(i, k)) * (
        y + KD(i, 1)
    )
    assert dp((x + KD(i, k)) * (y + KD(i, j)), (j, 2, 2)) == (x + KD(i, k)) * (
        y + KD(i, 2)
    )
    assert dp((x + KD(i, k)) * (y + KD(i, j)), (j, 3, 3)) == (x + KD(i, k)) * (
        y + KD(i, 3)
    )
    ...
    assert dp((x + KD(i, k)) * (y + KD(i, j)), (j, k, l)) == (KD(i, k) + x) * y * (
        (KD(i, k) + x) * y
    ) ** l * ((KD(i, k) + x) * y) ** (-k) + Piecewise(
        (
            ((KD(i, k) + x) * y) ** i
            * ((KD(i, k) + x) * y) ** (-k)
            * (KD(i, k) + x)
...
def test_deltasummation_mul_x_add_y_kd():
    ...
    assert ds(x * (y + KD(i, j)), (j, 1, 1)) == Piecewise(
        (x * y + x, Eq(i, 1)), (x * y, True)
    )
    assert ds(x * (y + KD(i, j)), (j, 2, 2)) == Piecewise(
        (x * y + x, Eq(i, 2)), (x * y, True)
    )
    assert ds(x * (y + KD(i, j)), (j, 3, 3)) == Piecewise(
        (x * y + x, Eq(i, 3)), (x * y, True)
    )
    assert ds(x * (y + KD(i, j)), (j, 1, k)) == Piecewise(
        (k * x * y + x, And(1 <= i, i <= k)), (k * x * y, True)
...
def test_deltasummation_mul_x_add_y_twokd():
    ...
    assert ds(x * (y + 2 * KD(i, j)), (j, 1, 1)) == Piecewise(
        (x * y + 2 * x, Eq(i, 1)), (x * y, True)
    )
    assert ds(x * (y + 2 * KD(i, j)), (j, 2, 2)) == Piecewise(
        (x * y + 2 * x, Eq(i, 2)), (x * y, True)
    )
    assert ds(x * (y + 2 * KD(i, j)), (j, 3, 3)) == Piecewise(
        (x * y + 2 * x, Eq(i, 3)), (x * y, True)
    )
...
def test_deltasummation_mul_add_x_y_add_y_kd():
    ...
    assert ds((x + y) * (y + KD(i, j)), (j, 1, 1)) == Piecewise(
        ((x + y) * y + x + y, Eq(i, 1)), ((x + y) * y, True)
    )
    assert ds((x + y) * (y + KD(i, j)), (j, 2, 2)) == Piecewise(
        ((x + y) * y + x + y, Eq(i, 2)), ((x + y) * y, True)
    )
    assert ds((x + y) * (y + KD(i, j)), (j, 3, 3)) == Piecewise(
        ((x + y) * y + x + y, Eq(i, 3)), ((x + y) * y, True)
    )
