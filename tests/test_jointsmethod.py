...
def test_rigid_body_particle_compatibility():
    ...
    P = PinJoint(
        ...
        coordinates=q,
        speeds=u,
        child_interframe=b_frame,
        child_point=-l * b_frame.x,
        joint_axis=C.z,
    )
    with warns_deprecated_sympy():
...
def test_simple_pedulum():
    ...
    q = dynamicsymbols("q")
    P = PinJoint(
        "P", C, b, speeds=q.diff(t), coordinates=q, child_point=-l * b.x, joint_axis=C.z
    )
...
def test_chaos_pendulum():
    ...
    mA, mB, lA, lB, IAxx, IBxx, IByy, IBzz, g = symbols(
        "mA, mB, lA, lB, IAxx, IBxx, IByy, IBzz, g"
    )
    ...
    with warns_deprecated_sympy():
        ...
        plate = Body(
            "plate", mass=mB, frame=B, central_inertia=inertia(B, IBxx, IByy, IBzz)
        )
        C = Body("C")
    J1 = PinJoint(
        "J1",
...
def test_four_bar_linkage_with_manual_constraints():
    ...
    loop = (
        link4.masscenter.pos_from(link1.masscenter)
        + l1 / 2 * link1.x
        + l4 / 2 * link4.x
    )
    ...
    forcing_check = Matrix(
        [[-0.031211821321648], [-0.00066022608181], [0.001813559741243]]
    )
    eps = 1e-10
    ...
    assert all(
        abs(x) < eps for x in (Matrix(eval_f(q_vals, u_vals, p_vals)) - forcing_check)
    )
