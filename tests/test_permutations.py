...
from sympy.printing import latex, pretty, srepr, sstr
...
def test_Permutation():
    ...
    assert rmul(
        Permutation([[1, 2, 3], [0, 4]]), Permutation([[1, 2, 4], [0], [3]])
    ).cyclic_form == [[0, 4, 2], [1, 3]]
    ...
    assert (
        Permutation.from_inversion_vector(q.inversion_vector()).array_form
        == q.array_form
    )
...
def test_args():
    ...
    assert (
        Permutation([[0], [1]]) == Permutation(((0,), (1,))) == Permutation(((0,), [1]))
    )
...
def test_from_sequence():
    ...
    assert Permutation.from_sequence("SymPy", key=lambda x: x.lower()) == Permutation(
        4
    )(0, 2)(1, 3)
