...
def sample_document():
    ...
    content += "It contains information about Python and JavaScript programming languages.\n\n"
...
class TestAnalysisEndToEnd:
    ...
    def test_available_models_endpoint(self, client):
        ...
        for model in expected_models:
            assert model in data["available_models"], f"Model {model} not found in available models"
    ...
    def test_analyze_with_documents(self, client, sample_analysis_request, sample_document):
        ...
        response = client.post(
            "/api/analyze-with-docs",
            data=form_data,
            files=sample_document
        )
    ...
    def test_error_handling_during_processing(self, mock_process, client, sample_analysis_request):
    ...
    async def test_async_analysis_flow(self, client, sample_analysis_request):
        ...
        if results_response.status_code == 200:
            ...
            print("Warning: Analysis progress tracking timed out, but this may be expected in mock mode")
        else:
            # If completed but results aren't available, that's an error
            assert False, f"Analysis completed but results unavailable: {results_response.status_code}"
