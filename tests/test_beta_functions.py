...
def test_beta():
    ...
    assert (
        expand_func(beta(x, y))
        == expand_func(beta(x, y + 1) + beta(x + 1, y)).simplify()
    )
    ...
    assert (
        beta(x, y)
        .rewrite(Integral)
        .dummy_eq(Integral(t ** (x - 1) * (1 - t) ** (y - 1), (t, 0, 1)))
    )
    assert beta(Rational(-19, 10), Rational(-1, 10)) == S.Zero
    assert beta(Rational(-19, 10), Rational(-9, 10)) == 800 * 2 ** (S(4) / 5) * sqrt(
        pi
    ) * gamma(S.One / 10) / (171 * gamma(-S(7) / 5))
    assert beta(Rational(19, 10), Rational(29, 10)) == 100 / (
        551 * catalan(Rational(19, 10))
    )
...
def test_betainc():
    ...
    assert conjugate(betainc(I, 2, 3 - I, 1 + 4 * I)) == betainc(
        -I, 2, 3 + I, 1 - 4 * I
    )
...
def test_betainc_regularized():
    ...
    assert conjugate(
        betainc_regularized(3 * I, 1, 2 + I, 1 + 2 * I)
    ) == betainc_regularized(-3 * I, 1, 2 - I, 1 - 2 * I)

    assert betainc_regularized(a, b, 0, 1).rewrite(Integral) == 1
    assert betainc_regularized(1, 2, x1, x2).rewrite(hyper) == 2 * x2 * hyper(
        (1, -1), (2,), x2
    ) - 2 * x1 * hyper((1, -1), (2,), x1)
