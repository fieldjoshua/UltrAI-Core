...
def test_maxima_functions():
    ...
    assert parse_maxima("trigexpand(sin(2*x)+cos(2*x))") == -1 + 2 * cos(
        x
    ) ** 2 + 2 * cos(x) * sin(x)
    ...
    assert (
        parse_maxima(
            "sum(k, k, 1, n)",
            name_dict={"n": Symbol("n", integer=True), "k": Symbol("k", integer=True)},
        )
        == (n**2 + n) / 2
    )
    assert parse_maxima(
    ...
    assert Abs(
        parse_maxima("float(sec(%pi/3) + csc(%pi/3))") - 3.154700538379252
    ) < 10 ** (-5)
