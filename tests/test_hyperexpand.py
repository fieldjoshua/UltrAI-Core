...
def test_branch_bug():
    assert hyperexpand(
        hyper((Rational(-1, 3), S.Half), (Rational(2, 3), Rational(3, 2)), -z)
    ) == -(z ** S("1/3")) * lowergamma(exp_polar(I * pi) / 3, z) / 5 + sqrt(pi) * erf(
        sqrt(z)
    ) / (
        5 * sqrt(z)
    )
    assert hyperexpand(
        meijerg([Rational(7, 6), 1], [], [Rational(2, 3)], [Rational(1, 6), 0], z)
    ) == 2 * z ** S("2/3") * (
        2 * sqrt(pi) * erf(sqrt(z)) / sqrt(z)
        - 2 * lowergamma(Rational(2, 3), z) / z ** S("2/3")
    ) * gamma(
        Rational(2, 3)
    ) / gamma(
        Rational(5, 3)
...
def test_roach_fail():
    assert can_do(
        [Rational(-1, 2), 1], [Rational(1, 4), S.Half, Rational(3, 4)]
    )  # PFDD
    assert can_do([Rational(3, 2)], [Rational(5, 2), 5])  # struve function
    assert can_do(
        [Rational(-1, 2), S.Half, 1], [Rational(3, 2), Rational(5, 2)]
    )  # polylog, pfdd
    assert can_do([1, 2, 3], [S.Half, 4])  # XXX ?
    assert can_do(
        [S.Half], [Rational(-1, 3), Rational(-1, 2), Rational(-2, 3)]
    )  # PFDD ?
...
def test_hyperexpand_bases():
    ...
    assert (
        hyperexpand(hyper([S.Half, 2], [Rational(3, 2)], z))
        == -1 / (2 * z - 2) + atanh(sqrt(z)) / sqrt(z) / 2
    )
    assert hyperexpand(hyper([S.Half, S.Half], [Rational(5, 2)], z)) == (
        -3 * z + 3
    ) / 4 / (z * sqrt(-z + 1)) + (6 * z - 3) * asin(sqrt(z)) / (4 * z ** Rational(3, 2))
    assert hyperexpand(hyper([1, 2], [Rational(3, 2)], z)) == -1 / (2 * z - 2) - asin(
        sqrt(z)
    ) / (sqrt(z) * (2 * z - 2) * sqrt(-z + 1))
    assert hyperexpand(hyper([Rational(-1, 2) - 1, 1, 2], [S.Half, 3], z)) == sqrt(
        z
    ) * (z * Rational(6, 7) - Rational(6, 5)) * atanh(sqrt(z)) + (
        -30 * z**2 + 32 * z - 6
    ) / 35 / z - 6 * log(
        -z + 1
    ) / (
    ...
    assert (
        hyperexpand(hyper([1 + S.Half, 1, 1], [2, 2], z))
        == -4 * log(sqrt(-z + 1) / 2 + S.Half) / z
    )
    # TODO hyperexpand(hyper([a], [2*a + 1], z))
    # TODO [S.Half, a], [Rational(3, 2), a+1]
    assert hyperexpand(hyper([2], [b, 1], z)) == z ** (-b / 2 + S.Half) * besseli(
        b - 1, 2 * sqrt(z)
    ) * gamma(b) + z ** (-b / 2 + 1) * besseli(b, 2 * sqrt(z)) * gamma(b)
    # TODO [a], [a - S.Half, 2*a]
...
def test_hyperexpand_parametric():
    ...
    assert hyperexpand(hyper([a, Rational(-1, 2) + a], [2 * a], z)) == 2 ** (
        2 * a - 1
    ) * ((-z + 1) ** S.Half + 1) ** (-2 * a + 1)
...
def test_formulae():
    ...
    for formula in formulae:
        ...
        for d1, d2 in zip(deriv1, deriv2):
            assert tn(
                d1.subs(rep).replace(exp_polar, exp),
                d2.subs(rep).rewrite("nonrepsmall").replace(exp_polar, exp),
                z,
            )
...
def test_plan():
    ...
    assert tn(
        apply_operators(
            h,
            devise_plan(
                Hyper_Function((a1 + 1, a2), [b1]), Hyper_Function((a1, a2), [b1]), z
            ),
            op,
        ),
        h2,
    ...
    assert tn(
        apply_operators(
            h,
            devise_plan(
                Hyper_Function((a1 + 1, a2 - 1), [b1]),
                Hyper_Function((a1, a2), [b1]),
                z,
            ),
            op,
        ),
...
def test_meijerg_expand():
    ...
    assert hyperexpand(meijerg([[], []], [[S.Half], [0]], (z / 2) ** 2)) == sin(
        z
    ) / sqrt(pi)
    assert hyperexpand(meijerg([[], []], [[0], [S.Half]], (z / 2) ** 2)) == cos(
        z
    ) / sqrt(pi)
    ...
    assert (
        gammasimp(
            simplify(
                hyperexpand(meijerg([1], [1 - a], [-a / 2, -a / 2 + S.Half], [], 1 / z))
            )
        )
    ...
    assert (
        ...
        == hyper((a,), (a + 1, a + 1), z * exp_polar(I * pi))
        * z**a
        * gamma(a)
        / gamma(a + 1) ** 2
    )
...
def test_meijerg_lookup():
    ...
    assert hyperexpand(meijerg([a], [], [b, a], [], z)) == z**b * exp(z) * gamma(
        -a + b + 1
    ) * uppergamma(a - b, z)
    ...
    assert (
        ...
        * z ** (a - S.Half)
        * (
            2 * cos(2 * sqrt(z)) * (Si(2 * sqrt(z)) - pi / 2)
...
def test_meijerg_shift_operators():
    ...
    assert tn(
        MeijerShiftA(b1).apply(g, op), meijerg([a1], [a3, a4], [b1 + 1], [b3, b4], z), z
    )
    assert tn(
        MeijerShiftB(a1).apply(g, op), meijerg([a1 - 1], [a3, a4], [b1], [b3, b4], z), z
    )
    assert tn(
        MeijerShiftC(b3).apply(g, op), meijerg([a1], [a3, a4], [b1], [b3 + 1, b4], z), z
    )
    assert tn(
        MeijerShiftD(a3).apply(g, op), meijerg([a1], [a3 - 1, a4], [b1], [b3, b4], z), z
    )
...
def test_meijerg_confluence():
    ...
    assert t(
        meijerg([], [3, 1], [0, 0], [], z),
        -(z**2) / 4 + z - log(z) / 2 - Rational(3, 4),
        0,
    )
    assert t(
        meijerg([], [3, 1], [-1, 0], [], z),
        z**2 / 12 - z / 2 + log(z) / 2 + Rational(1, 4) + 1 / (6 * z),
        0,
    )
    assert t(meijerg([], [1, 1, 1, 1], [0, 0, 0, 0], [], z), -log(z) ** 3 / 6, 0)
    assert t(meijerg([1, 1], [], [], [0, 0], z), 0, -log(1 / z))
    assert t(
        meijerg([1, 1], [2, 2], [1, 1], [0, 0], z), -z * log(z) + 2 * z, -log(1 / z) + 2
    )
...
def test_meijerg_with_Floats():
    ...
    g = (
        a
        * z ** Rational(3, 2)
        * hyper((-0.5, Rational(3, 2)), (Rational(5, 2),), z * exp_polar(I * pi))
    )
...
def test_lerchphi():
    ...
    assert hyperexpand(
        hyper([1, a, a, a], [a + 1, a + 1, a + 1], z) / a**3
    ) == lerchphi(z, 3, a)
    assert hyperexpand(hyper([1] + [a] * 10, [a + 1] * 10, z) / a**10) == lerchphi(
        z, 10, a
    )
    assert gammasimp(
        hyperexpand(meijerg([0, 1 - a], [], [0], [-a], exp_polar(-I * pi) * z))
    ) == lerchphi(z, 1, a)
    assert gammasimp(
        hyperexpand(
            meijerg([0, 1 - a, 1 - a], [], [0], [-a, -a], exp_polar(-I * pi) * z)
        )
    ) == lerchphi(z, 2, a)
    assert gammasimp(
        hyperexpand(
            meijerg(
                [0, 1 - a, 1 - a, 1 - a], [], [0], [-a, -a, -a], exp_polar(-I * pi) * z
            )
        )
    ) == lerchphi(z, 3, a)
    ...
    assert hyperexpand(hyper([1, a, 1 + S.Half], [a + 1, S.Half], z)) == -2 * a / (
        z - 1
    ) + (-2 * a**2 + a) * lerchphi(z, 1, a)
...
def test_partial_simp():
    ...
    assert hyperexpand(hyper([3, a], [1, b], z)) == (
        -a * b / 2 + a * z / 2 + 2 * a
    ) * hyper([a + 1], [b], z) + (a * b / 2 - 2 * a + 1) * hyper([a], [b], z)
    assert tn(hyperexpand(hyper([3, d], [1, e], z)), hyper([3, d], [1, e], z), z)
    assert hyperexpand(hyper([3], [1, a, b], z)) == hyper((), (a, b), z) + z * hyper(
        (), (a + 1, b), z
    ) / (2 * a) - z * (b - 4) * hyper((), (a + 1, b + 1), z) / (2 * a * b)
    assert tn(hyperexpand(hyper([3], [1, d, e], z)), hyper([3], [1, d, e], z), z)


def test_hyperexpand_special():
    assert hyperexpand(hyper([a, b], [c], 1)) == gamma(c) * gamma(c - a - b) / gamma(
        c - a
    ) / gamma(c - b)
    assert hyperexpand(hyper([a, b], [1 + a - b], -1)) == gamma(1 + a / 2) * gamma(
        1 + a - b
    ) / gamma(1 + a) / gamma(1 + a / 2 - b)
    assert hyperexpand(hyper([a, b], [1 + b - a], -1)) == gamma(1 + b / 2) * gamma(
        1 + b - a
    ) / gamma(1 + b) / gamma(1 + b / 2 - a)
    assert hyperexpand(
        meijerg([1 - z - a / 2], [1 - z + a / 2], [b / 2], [-b / 2], 1)
    ) == gamma(1 - 2 * z) * gamma(z + a / 2 + b / 2) / gamma(
        1 - z + a / 2 - b / 2
    ) / gamma(
        1 - z - a / 2 + b / 2
    ) / gamma(
...
def test_Mod1_behavior():
    ...
    assert simplify(hyperexpand(meijerg([1], [], [n + 1], [0], z))) == lowergamma(
        n + 1, z
    )
...
def test_prudnikov_fail_3F2():
    assert can_do(
        [a, a + Rational(1, 3), a + Rational(2, 3)], [Rational(1, 3), Rational(2, 3)]
    )
    assert can_do(
        [a, a + Rational(1, 3), a + Rational(2, 3)], [Rational(2, 3), Rational(4, 3)]
    )
    assert can_do(
        [a, a + Rational(1, 3), a + Rational(2, 3)], [Rational(4, 3), Rational(5, 3)]
    )

    # page 421
    assert can_do(
        [a, a + Rational(1, 3), a + Rational(2, 3)],
        [a * Rational(3, 2), (3 * a + 1) / 2],
    )
    ...
    # PFDD
    assert can_do(
        [Rational(1, 8), Rational(3, 8), 1], [Rational(9, 8), Rational(11, 8)]
    )
    assert can_do(
        [Rational(1, 8), Rational(5, 8), 1], [Rational(9, 8), Rational(13, 8)]
    )
    assert can_do(
        [Rational(1, 8), Rational(7, 8), 1], [Rational(9, 8), Rational(15, 8)]
    )
    ...
    assert can_do(
        [Rational(1, 6), Rational(2, 3), 1], [Rational(5, 3), Rational(13, 6)]
    )
