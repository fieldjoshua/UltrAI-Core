...
def test_booleans():
    ...
    assert Intersection(Union(l1, l4), l3) == FiniteSet(
        Point(Rational(-1, 3), Rational(-1, 3)), Point(5, 1)
    )
    assert Intersection(l1, FiniteSet(Point(7, -7))) == EmptySet
    assert Intersection(Circle(Point(0, 0), 3), Line(p1, p2)) == FiniteSet(
        Point(-3, 0), Point(3, 0)
    )
    ...
    assert Union(poly1, FiniteSet(Point(0, 0), Point(3, 5))) == Union(
        poly1, FiniteSet(Point(3, 5))
    )
    # intersect two polygons that share an edge
    assert Intersection(poly1, poly3) == Union(
        FiniteSet(Point(Rational(3, 2), 1), Point(2, 1)),
