...
def test_K():
    ...
    assert (
        K(m)
        .rewrite(Integral)
        .dummy_eq(Integral(1 / sqrt(1 - m * sin(t) ** 2), (t, 0, pi / 2)))
    )
...
def test_F():
    ...
    assert F(z, m).diff(m) == E(z, m) / (2 * m * (1 - m)) - F(z, m) / (2 * m) - sin(
        2 * z
    ...
    assert (
        F(z, m)
        .rewrite(Integral)
        .dummy_eq(Integral(1 / sqrt(1 - m * sin(t) ** 2), (t, 0, z)))
    )
...
def test_P():
    ...
    assert P(1, z, m) == F(z, m) + (sqrt(1 - m * sin(z) ** 2) * tan(z) - E(z, m)) / (
        1 - m
    )
    ...
    assert (
        ...
        .dummy_eq(
            Integral(
                1 / ((1 - n * sin(t) ** 2) * sqrt(1 - m * sin(t) ** 2)), (t, 0, pi / 2)
            )
        )
    )
