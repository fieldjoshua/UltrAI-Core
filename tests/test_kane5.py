from sympy import Matrix, lambdify, pi, simplify, sqrt, symbols, zeros
...
def _create_rolling_disc():
    ...
    disc.masscenter.set_vel(
        ground.frame,
        cross(disc.frame.ang_vel_in(ground.frame), disc.masscenter.pos_from(cp)),
    )
...
def test_kane_rolling_disc_kdes_callable():
    ...
    assert all(
        abs(qdi - fui) < eps
        for qdi, fui in eval_kdes(zero_vals, zero_vals, zero_vals, p_vals)
    )
    ...
    qd_vals = tuple(
        map(
            float,
            (4, 1 / 3, 4 - 2 * sqrt(3), 0.25 * (2 * sqrt(3) - 3), 0.25 * (2 - sqrt(3))),
        )
    )
    u_vals = tuple(
        map(float, (-2, 4, 1 / 3, 0.25 * (-3 + 2 * sqrt(3)), 0.25 * (-sqrt(3) + 2)))
    )
    assert all(
        abs(qdi - fui) < eps for qdi, fui in eval_kdes(q_vals, qd_vals, u_vals, p_vals)
    )
