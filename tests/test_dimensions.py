...
def test_Dimension_add_sub():
    ...
    assert dimsys_SI.get_dimensional_dependencies(
        mass * length / time**2 + force - pressure * length**2
    ) == {length: 1, mass: 1, time: -2}

    raises(
        TypeError,
        lambda: dimsys_SI.get_dimensional_dependencies(
...
def test_Dimension_mul_div_exp():
    ...
    assert dimsys_SI.get_dimensional_dependencies(length ** (length / length)) == {
        length: Dimension(1)
    }
...
def test_Dimension_functions():
    ...
    raises(
        TypeError, lambda: dimsys_SI.get_dimensional_dependencies(atan2(length, time))
    )
    ...
    assert (
        dimsys_SI.get_dimensional_dependencies(log(length / length, length / length))
        == {}
    )
