...
from sympy.functions.elementary.trigonometric import acos, asin, atan, atan2, cos, sin
...
def test_quaternion_functions():
    ...
    assert q.normalize() == Quaternion(w, x, y, z) / sqrt(
        w**2 + x**2 + y**2 + z**2
    )
    assert q.inverse() == Quaternion(w, -x, -y, -z) / (
        w**2 + x**2 + y**2 + z**2
    )
