...
def test_deltaintegrate():
    ...
    assert deltaintegrate(y * DiracDelta(x, 1) ** 2, x) == -y * DiracDelta(
        0, 2
    ) * Heaviside(x)
    ...
    assert (
        deltaintegrate(DiracDelta(x**2 + x - 2), x)
    ...
    assert deltaintegrate(p, y) == x**3 * z * DiracDelta(x - z) ** 2 * Heaviside(
        y - x
    )
    assert deltaintegrate((x + 1) * DiracDelta(2 * x), x) == S.Half * Heaviside(x)
    assert deltaintegrate(
        (x + 1) * DiracDelta(x * Rational(2, 3) + Rational(4, 9)), x
    ) == S.Half * Heaviside(x + Rational(2, 3))

    a, b, c = symbols("a b c", commutative=False)
    assert deltaintegrate(DiracDelta(x - y) * f(x - b) * f(x - a), x) == f(y - b) * f(
        y - a
    ) * Heaviside(x - y)
    ...
    assert deltaintegrate(p, x) == DiracDelta(y - z) * f(y - b) * f(y - a) * Heaviside(
        x - y
    )
