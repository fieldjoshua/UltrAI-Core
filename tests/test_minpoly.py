...
from sympy.polys.polyerrors import GeneratorsError, NotAlgebraic
...
def test_minimal_polynomial():
    ...
    assert (
        minimal_polynomial(2 * I + sqrt(2 + I), x) == x**4 + 4 * x**2 + 8 * x + 37
    )
    ...
    assert (
        minimal_polynomial(a ** Q(3, 2), x) == 729 * x**4 - 506898 * x**2 + 84604519
    )
    ...
    assert (
        minimal_polynomial(TribonacciConstant + 3, x)
        == x**3 - 10 * x**2 + 32 * x - 34
    )
    assert (
...
def test_minpoly_compose():
    ...
    assert (
        minimal_polynomial(exp(I * pi / 10), x) == x**8 - x**6 + x**4 - x**2 + 1
    )
    assert (
        minimal_polynomial(sin(pi / 9), x)
        == 64 * x**6 - 96 * x**4 + 36 * x**2 - 3
    )
    assert (
...
def test_minpoly_issue_23677():
    r1 = CRootOf(
        4000000 * x**3 - 239960000 * x**2 + 4782399900 * x - 31663998001, 0
    )
    r2 = CRootOf(
        4000000 * x**3 - 239960000 * x**2 + 4782399900 * x - 31663998001, 1
    )
    num = (
