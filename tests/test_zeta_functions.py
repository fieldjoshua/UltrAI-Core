...
def test_zeta_series():
    assert zeta(x, a).series(a, z, 2) == zeta(x, z) - x * (a - z) * zeta(x + 1, z) + O(
        (a - z) ** 2, (a, z)
    )
...
def test_derivatives():
    ...
    assert (
        lerchphi(z, s, a).diff(z) == (lerchphi(z, s - 1, a) - a * lerchphi(z, s, a)) / z
    )
...
def myexpand(func, target):
    ...
    for a in func.free_symbols:
        subs[a] = randcplx()
    return (
        abs(func.subs(subs).n() - expanded.replace(exp_polar, exp).subs(subs).n())
        < 1e-10
    )
...
def test_polylog_series():
    assert polylog(1, z).series(z, n=5) == z + z**2 / 2 + z**3 / 3 + z**4 / 4 + O(
        z**5
    )
    assert polylog(1, sqrt(z)).series(z, n=3) == z / 2 + z**2 / 4 + sqrt(z) + z ** (
        S(3) / 2
    ) / 3 + z ** (S(5) / 2) / 5 + O(z**3)

    # https://github.com/sympy/sympy/issues/9497
    assert polylog(S(3) / 2, -z).series(z, 0, 5) == -z + sqrt(2) * z**2 / 4 - sqrt(
        3
    ) * z**3 / 9 + z**4 / 8 + O(z**5)
...
def test_issue_8404():
    ...
    assert (
        Abs(Sum(1 / (3 * i + 1) ** 2, (i, 0, S.Infinity)).doit().n(4) - 1.122) < 0.001
    )
...
def test_polylog_values():
    ...
    for z in [
        ...
        assert (
            Abs(polylog(2, z).evalf() - polylog(2, z, evaluate=False).evalf()) < 1e-15
        )
    z = Symbol("z")
    for s in [-1, 0]:
        for _ in range(10):
            ...
            assert verify_numerically(
                polylog(s, z), polylog(s, z, evaluate=False), z, a=2, b=-2, c=5, d=2
            )
