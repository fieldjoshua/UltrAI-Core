...
def test_linrec():
    ...
    assert all(
        linrec(coeffs=[1, 1], init=[0, 1], n=n) == fibonacci(n) for n in range(95, 115)
    )
    ...
    assert linrec(coeffs=a[:5], init=b[:4], n=80) == Rational(
        1726244235456268979436592226626304376013002142588105090705187189,
    ...
    assert linrec(
        coeffs=[0] * 60 + [Rational(2, 3), Rational(4, 5)], init=b, n=3000
    ) == Rational(26777668739896791448594650497024, 48084516708184142230517578125)

    raises(
        TypeError, lambda: linrec(coeffs=[11, 13, 15, 17], init=[1, 2, 3, 4, 5], n=1)
    )
    ...
    assert (
        linrec(coeffs=[x, y, z], init=[1, 1, 1], n=4) == x**2 + x * y + x * z + y + z
    )
    assert (
        linrec(coeffs=[1, 2, 1], init=[x, y, z], n=20)
