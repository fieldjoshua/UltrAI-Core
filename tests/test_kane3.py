...
def test_bicycle():
    ...
    WF_cont = WF_mc.locatenew(
        "WF_cont", WFrad * (dot(Fork.y, Y.z) * Fork.y - Y.z).normalize()
    )
    ...
    Frame_I = (
        inertia(TempFrame, Iframe11, Iframe22, Iframe33, 0, 0, Iframe31),
        Frame_mc,
    )
    ...
    PaperForkCgZ = 0.7
    FrameLength = (
        PaperWb * sin(HTA) - (rake - (PaperRadFront - PaperRadRear) * cos(HTA))
    ).evalf()
    FrameCGNorm = (
