...
def test_cgs_gauss_convert_constants():
    assert (
        convert_to(speed_of_light, centimeter / second, cgs_gauss)
    ...
    assert (
        convert_to(
            coulomb_constant, dyne * centimeter**2 / statcoulomb**2, cgs_gauss
        )
    ...
    assert convert_to(
        gravitational_constant, dyne * centimeter**2 / gram**2, cgs_gauss
    )
    assert (
        NS(convert_to(planck, erg * second, cgs_gauss)) == "6.62607015e-27*erg*second"
    )
    ...
    assert (
        convert_to(farad, centimeter, cgs_gauss)
        == 299792458**2 * centimeter / 10**5
    )
