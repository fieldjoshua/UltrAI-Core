...
from sympy.core.function import ArgumentIndexError, expand_log
...
from sympy.testing.pytest import XFAIL, _both_exp_pow, raises
...
def test_log_nseries():
    ...
    assert log(sqrt(-I * x**2 - 3) * sqrt(-I * x**2 - 1) - 2)._eval_nseries(
        x, 3, None, 1
    ) == -I * pi + log(sqrt(3) + 2) + I * x**2 * (-2 + 4 * sqrt(3) / 3) + O(x**3)
