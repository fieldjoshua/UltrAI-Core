...
from sympy.sets.sets import Interval, SetKind
...
def test_CondSet():
    ...
    assert C(y, y < 1, C(x, y < 2, I)) == C(x, (x < 1) & (y < 2), I), C(
        y, y < 1, C(x, y < 2, I)
    )
...
def test_free_symbols():
    ...
    assert (
        ConditionSet(x, Eq(x, 0), ImageSet(Lambda(y, y**2), S.Integers)).free_symbols
        == set()
    )
...
def test_bound_symbols():
    ...
    assert ConditionSet(
        x, x < 10, ImageSet(Lambda(y, y**2), S.Integers)
    ).bound_symbols == [x]
    assert ConditionSet(
        x, x < 10, ConditionSet(y, y > 1, S.Integers)
    ).bound_symbols == [x]
...
def test_as_dummy():
    ...
    assert ConditionSet(
        x, x < 1, ImageSet(Lambda(y, y**2), S.Integers)
    ).as_dummy() == ConditionSet(_0, _0 < 1, ImageSet(Lambda(_0, _0**2), S.Integers))
...
def test_subs_CondSet():
    ...
    assert ConditionSet(p, n < x, Interval(-5, 5)).subs(x, p) == Interval(
        -5, 5
    ), ConditionSet(p, n < x, Interval(-5, 5)).subs(x, p)
    ...
    assert (
        ConditionSet(x, Contains(y, Interval(-1, 1)), img1)
        .subs(y, S.One / 3)
        .dummy_eq(img2)
    )
...
def test_as_relational():
    assert ConditionSet((x, y), x > 1, S.Integers**2).as_relational((x, y)) == (
        x > 1
    ) & Contains(x, S.Integers) & Contains(y, S.Integers)
    assert ConditionSet(x, x > 1, S.Integers).as_relational(x) == Contains(
        x, S.Integers
    ) & (x > 1)
...
def test_SetKind_ConditionSet():
    assert ConditionSet(x, Eq(sin(x), 0), Interval(0, 2 * pi)).kind is SetKind(
        NumberKind
    )
