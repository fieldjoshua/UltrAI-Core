from sympy import Matrix, S, cos, expand, eye, sin, solve, sqrt, symbols, tan, zeros
...
def test_one_dof():
    ...
    assert simplify(KM.rhs() - KM.mass_matrix_full.LUsolve(KM.forcing_full)) == zeros(
        2, 1
    )
...
def test_two_dof():
    ...
    for explicit_kinematics in [False, True]:
        ...
        assert simplify(
            KM.rhs() - KM.mass_matrix_full.LUsolve(KM.forcing_full)
        ) == zeros(4, 1)
    ...
    raises(
        ValueError, lambda: KanesMethod(N, q_ind=[q1, q2], u_ind=[u1, u2], kd_eqs=kd)
    )
...
def test_aux():
    ...
    KM2 = KanesMethod(
        N, q_ind=[q1, q2, q3], u_ind=[u1, u2, u3], kd_eqs=kd, u_auxiliary=[u4, u5]
    )
...
def test_implicit_kinematics():
    ...
    J_B = inertia(
        B,
        *[
            S(f"J_B_{ax}") * (1 if ax[0] == ax[1] else -1)
    ...
    assert (mass_matrix_kin_implicit * KM.q.diff() - forcing_kin_implicit) == Matrix(
        kinematic_eqs
    )
...
def test_issue_24887():
    ...
    P = Particle("P", Po, m)
    kane = KanesMethod(
        N, [q1, q2, q3], [u1, u2, u3], kdes, bodies=[P], forcelist=[(Po, -m * g * N.y)]
    )
    ...
    expected_fd = Matrix(
        [
            [
                l
                * m
                * (g * (sin(q1) * sin(q3) - sin(q2) * cos(q1) * cos(q3)) - l * u2 * u3)
            ],
            [0],
            [
                l
                * m
                * (-g * (sin(q1) * cos(q3) + sin(q2) * sin(q3) * cos(q1)) + l * u1 * u2)
            ],
        ]
    )
