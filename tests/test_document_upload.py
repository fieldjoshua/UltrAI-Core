...
class DocumentUploadTester:
    def __init__(self, api_url=DEFAULT_API_URL, output_dir=DEFAULT_OUTPUT_DIR, documents_dir=SAMPLE_DOCUMENTS_DIR):
    ...
    async def upload_files(self, files, prompt="Summarize the key points in these documents", models=None):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(f"{self.api_url}/api/analyze-with-docs", data=data) as response:
        ...
        if status_code == 200 and "document_metadata" in response_data:
            result["chunks_used"] = response_data["document_metadata"].get("chunks_used", 0)
    ...
    async def run_upload_test(self, test_config):
        ...
        if not files:
            print(f"No test files found in {self.documents_dir}. Please add test files or use --generate option.")
            return None
        
        # Use specified or default prompt
        prompt = test_config.get("prompt", "Summarize the key points from these documents")
        
        # Use specified or default models
        models = test_config.get("models", ["claude", "chatgpt"])
        
        print(f"Uploading {len(files)} files, total size: {sum(os.path.getsize(f) for f in files) / (1024*1024):.2f} MB")
        ...
        if result['success']:
            ...
            print(f"Backend processing time: {result.get('processing_time', 'N/A'):.2f} seconds")
    ...
    async def run_size_scaling_test(self, file_sizes=None, prompt="Analyze the content of these documents"):
    ...
    def save_results(self):
        ...
        results_df.to_csv(f"{self.output_dir}/upload_results_{timestamp}.csv", index=False)
...
async def main():
    parser = argparse.ArgumentParser(description='Ultra Framework Document Upload Tester')
    ...
    parser.add_argument('--docs-dir', type=str, default=SAMPLE_DOCUMENTS_DIR,
                      help=f'Directory containing test documents (default: {SAMPLE_DOCUMENTS_DIR})')
    parser.add_argument('--mode', type=str, choices=['single', 'batch', 'size-scaling', 'count-scaling'], 
    ...
    parser.add_argument('--file-type', type=str, choices=['pdf', 'txt', 'docx', 'md'], 
                      help='File type to use (for existing files)')
    parser.add_argument('--prompt', type=str, default="Summarize the key points from these documents",
