...
def test_find_substitutions():
    ...
    assert (-(x**2), Rational(-1, 2), exp(u)) in find_substitutions(
        x * exp(-(x**2)), x, u
    )
...
def test_manualintegrate_trigonometry():
    ...
    assert (
        manualintegrate(sin(3 * x) * sec(x), x)
...
def test_manualintegrate_trigpowers():
    ...
    assert (
        manualintegrate(sin(x) ** 3 * cos(x) ** 2, x)
        == cos(x) ** 5 / 5 - cos(x) ** 3 / 3
    )
    ...
    assert manualintegrate(cot(x) ** 5 * csc(x), x) == -csc(x) ** 5 / 5 + 2 * csc(
        x
    ) ** 3 / 3 - csc(x)
    assert (
        manualintegrate(cot(x) ** 2 * csc(x) ** 6, x)
...
def test_manualintegrate_inversetrig():
    ...
    assert manualintegrate(1 / (ra + rb * x**2), x) == Piecewise(
        ...
        (
            (log(x - sqrt(-ra / rb)) - log(x + sqrt(-ra / rb)))
            / (2 * sqrt(rb) * sqrt(-ra)),
            True,
        ),
    )
    assert manualintegrate(1 / (4 + rb * x**2), x) == Piecewise(
        ...
        (
            -I
            * (log(x - 2 * sqrt(-1 / rb)) - log(x + 2 * sqrt(-1 / rb)))
            / (4 * sqrt(rb)),
            True,
        ),
    )
    ...
    assert manualintegrate(1 / sqrt(ra * x**2 + 1), x) == Piecewise(
        (asin(x * sqrt(-ra)) / sqrt(-ra), ra < 0),
        (asinh(sqrt(ra) * x) / sqrt(ra), ra > 0),
        (x, True),
    )
    ...
    assert (
        manualintegrate(1 / sqrt(4 * x**2 - 4), x)
        == log(8 * x + 4 * sqrt(4 * x**2 - 4)) / 2
    )
    assert (
        manualintegrate(1 / sqrt(9 * x**2 - 1), x)
        == log(18 * x + 6 * sqrt(9 * x**2 - 1)) / 3
    )
    assert manualintegrate(1 / sqrt(ra * x**2 - 4), x) == Piecewise(
        (log(2 * sqrt(ra) * sqrt(ra * x**2 - 4) + 2 * ra * x) / sqrt(ra), Ne(ra, 0)),
        (-I * x / 2, True),
    )
    assert manualintegrate(1 / sqrt(-ra + 4 * x**2), x) == Piecewise(
        (asinh(2 * x * sqrt(-1 / ra)) / 2, ra < 0),
        (log(8 * x + 4 * sqrt(-ra + 4 * x**2)) / 2, True),
    )
    ...
    assert (
        ...
        * Piecewise(
            (
                ...
                + log(
                    -2 * a**2 * x + 2 * sqrt(-(a**2)) * sqrt(-(a**2) * x**2 + 1)
                )
                / (2 * a**2 * sqrt(-(a**2))),
                Ne(a**2, 0),
            ),
    ...
    assert (
        ...
        == -a
        * (x / a**2 - atan(x / sqrt(a ** (-2))) / (a**4 * sqrt(a ** (-2))))
        / 2
        + x**2 * atan(a * x) / 2
    )
    # acsc
    assert manualintegrate(acsc(x), x) == x * acsc(x) + Integral(
        1 / (x * sqrt(1 - 1 / x**2)), x
    )
    assert (
        manualintegrate(acsc(a * x), x)
        == x * acsc(a * x) + Integral(1 / (x * sqrt(1 - 1 / (a**2 * x**2))), x) / a
    )
    assert manualintegrate(x * acsc(a * x), x) == x**2 * acsc(a * x) / 2 + Integral(
        1 / sqrt(1 - 1 / (a**2 * x**2)), x
    ) / (2 * a)
    # asec
    assert manualintegrate(asec(x), x) == x * asec(x) - Integral(
        1 / (x * sqrt(1 - 1 / x**2)), x
    )
    assert (
        manualintegrate(asec(a * x), x)
        == x * asec(a * x) - Integral(1 / (x * sqrt(1 - 1 / (a**2 * x**2))), x) / a
    )
    ...
    assert (
        ...
        == a * (x / a**2 - atan(x / sqrt(a ** (-2))) / (a**4 * sqrt(a ** (-2)))) / 2
        + x**2 * acot(a * x) / 2
    )
    ...
    assert manualintegrate(1 / sqrt(ra - rb * x**2), x) == Piecewise(
        ...
        (
            log(-2 * rb * x + 2 * sqrt(-rb) * sqrt(ra - rb * x**2)) / sqrt(-rb),
            Ne(rb, 0),
        ),
        (x / sqrt(ra), True),
    )
...
def test_issue_3796():
    ...
    assert (
        integrate(x * exp(x**4), x, risch=False)
        == -I * sqrt(pi) * erf(I * x**2) / 4
    )


def test_manual_true():
    assert (
        integrate(exp(x) * sin(x), x, manual=True)
        == (exp(x) * sin(x)) / 2 - (exp(x) * cos(x)) / 2
    )
...
def test_issue_6746():
    ...
    assert (
        manualintegrate(y ** (n * x), x)
        == Piecewise((y ** (n * x) / log(y), Ne(log(y), 0)), (n * x, True)) / n
    )
    y = Symbol("y", positive=True)
    assert manualintegrate((y + 1) ** (n * x), x) == (y + 1) ** (n * x) / (
        n * log(y + 1)
    )
    a = Symbol("a", negative=True)
    b = Symbol("b")
    assert manualintegrate(1 / (a + b * x**2), x) == atan(x / sqrt(a / b)) / (
        b * sqrt(a / b)
    )
    b = Symbol("b", negative=True)
    assert manualintegrate(1 / (a + b * x**2), x) == atan(
        x / (sqrt(-a) * sqrt(-1 / b))
    ) / (b * sqrt(-a) * sqrt(-1 / b))
    assert manualintegrate(
        1 / ((x**a + y**b + 4) * sqrt(a * x**2 + 1)), x
    ) == y ** (-b) * Integral(
        x ** (-a)
        / (
            y ** (-b) * sqrt(a * x**2 + 1)
    ...
    assert manualintegrate(1 / (x - a**x + x * b**2), x) == Integral(
        1 / (-(a**x) + b**2 * x + x), x
    )
...
@slow
def test_issue_2850():
    assert manualintegrate(asin(x) * log(x), x) == -x * asin(x) - sqrt(
        -(x**2) + 1
    ) + (x * asin(x) + sqrt(-(x**2) + 1)) * log(x) - Integral(
        sqrt(-(x**2) + 1) / x, x
    )
    assert manualintegrate(acos(x) * log(x), x) == -x * acos(x) + sqrt(
        -(x**2) + 1
    ) + (x * acos(x) - sqrt(-(x**2) + 1)) * log(x) + Integral(
        sqrt(-(x**2) + 1) / x, x
    )
    assert (
...
def test_issue_9462():
    assert (
        manualintegrate(sin(2 * x) * exp(x), x)
...
def test_issue_10847():
    ...
    assert (
        ...
        == rc
        * Piecewise(
            (atan(x / sqrt(-rc)) / sqrt(-rc), rc < 0),
            ((log(-sqrt(rc) + x) - log(sqrt(rc) + x)) / (2 * sqrt(rc)), True),
        )
        + x
    )
    ...
    assert (
        ...
        * y**2
        * Piecewise(
            (atan(sqrt(x - y) / sqrt(y)) / sqrt(y), Ne(y, 0)), (-1 / sqrt(x - y), True)
        )
        / 3
        - 4 * y * sqrt(x - y) / 3
    ...
    assert (
        ...
        * Piecewise(
            ...
            (
                (log(-sqrt(-ry) + sqrt(x - ry)) - log(sqrt(-ry) + sqrt(x - ry)))
                / (2 * sqrt(-ry)),
                True,
            ),
        )
    ...
    assert (
        manualintegrate(sqrt(x) * log(x), x)
        == 2 * x ** Rational(3, 2) * log(x) / 3 - 4 * x ** Rational(3, 2) / 9
    )
    ...
    assert expand(manualintegrate(sqrt(ra * x + rb) / (x + rc), x)) == Piecewise(
        (
            ...
            * Piecewise(
                (
                    atan(sqrt(ra * x + rb) / sqrt(ra * rc - rb)) / sqrt(ra * rc - rb),
                    ra * rc - rb > 0,
                ),
                (
                    log(-sqrt(-ra * rc + rb) + sqrt(ra * x + rb))
                    / (2 * sqrt(-ra * rc + rb))
                    - log(sqrt(-ra * rc + rb) + sqrt(ra * x + rb))
                    / (2 * sqrt(-ra * rc + rb)),
                    True,
    ...
    assert manualintegrate(sqrt(2 * x + 3) / (x + 1), x) == 2 * sqrt(2 * x + 3) - log(
        sqrt(2 * x + 3) + 1
    ) + log(sqrt(2 * x + 3) - 1)
    assert (
    ...
    assert manualintegrate(x ** (-3) * log(x), x) == -log(x) / (2 * x**2) - 1 / (
        4 * x**2
    )
    assert manualintegrate(log(y) / (y**2 * (1 - 1 / y)), y) == log(y) * log(
        -1 + 1 / y
    ) - Integral(log(-1 + 1 / y) / y, y)
...
def test_issue_12641():
    ...
    assert manualintegrate((sin(2 * x) * cos(x)) / (1 + cos(x)), x) == -2 * log(
        cos(x) + 1
    ) - cos(x) ** 2 + 2 * cos(x)
...
def test_issue_14470():
    assert_is_integral_of(
        1 / (x * sqrt(x + 1)), log(sqrt(x + 1) - 1) - log(sqrt(x + 1) + 1)
    )
...
def test_issue_9858():
    ...
    assert manualintegrate(exp(2 * x) * cos(exp(x)), x) == exp(x) * sin(exp(x)) + cos(
        exp(x)
    )
...
def test_quadratic_denom():
    ...
    assert (
        manualintegrate(f, x)
        == 5 * log(3 * x**2 - 2 * x + 8) / 6
        + 11 * sqrt(23) * atan(3 * sqrt(23) * (x - Rational(1, 3)) / 23) / 69
    )
    g = 3 / (2 * x**2 + 3 * x + 1)
...
def test_issue_23348():
    ...
    assert (
        constant_times_step.integrand
        == constant_times_step.constant * constant_times_step.other
    )
...
def test_issue_25093():
    ...
    assert manualintegrate(exp(a * x**2 + b), x) == sqrt(pi) * exp(b) * erfi(
        sqrt(a) * x
    ) / (2 * sqrt(a))
    assert manualintegrate(exp(ap * x**2 + b), x) == sqrt(pi) * exp(b) * erfi(
        sqrt(ap) * x
    ) / (2 * sqrt(ap))
    assert manualintegrate(exp(an * x**2 + b), x) == -sqrt(pi) * exp(b) * erf(
        an * x / sqrt(-an)
    ) / (2 * sqrt(-an))
    assert manualintegrate(sin(a * x**2 + b), x) == (
        ...
        * sqrt(pi)
        * (
            sin(b) * fresnelc(sqrt(2) * sqrt(a) * x / sqrt(pi))
            + cos(b) * fresnels(sqrt(2) * sqrt(a) * x / sqrt(pi))
        )
        / (2 * sqrt(a))
    )
    assert manualintegrate(cos(a * x**2 + b), x) == (
        ...
        * sqrt(pi)
        * (
            -sin(b) * fresnels(sqrt(2) * sqrt(a) * x / sqrt(pi))
            + cos(b) * fresnelc(sqrt(2) * sqrt(a) * x / sqrt(pi))
        )
        / (2 * sqrt(a))
    )
...
def test_nested_pow():
    ...
    assert manualintegrate(f, x) == Piecewise(
        (Piecewise((F1, Ne(d * e, -1)), (F2, True)), Ne(b, 0)),
        (x * (a**d * c) ** e, True),
    )
...
def test_manualintegrate_sqrt_linear():
    ...
    assert manualintegrate(x / sqrt(a + b * x) ** 3, x) == Piecewise(
        (Mul(2, b**-2, a / sqrt(a + b * x) + sqrt(a + b * x)), Ne(b, 0)),
        (x**2 / (2 * a ** (S(3) / 2)), True),
    )
    assert_is_integral_of(
...
def test_manualintegrate_sqrt_quadratic():
    ...
    assert manualintegrate(sqrt(a + c * x**2), x) == Piecewise(
        (
            ...
            * Piecewise(
                (
                    log(2 * sqrt(c) * sqrt(a + c * x**2) + 2 * c * x) / sqrt(c),
                    Ne(a, 0),
                ),
