...
def test_Piecewise():
    ...
    assert refine(Piecewise((1, x < 0), (3, True)), (y < 0)) == Piecewise(
        (1, x < 0), (3, True)
    )
    assert refine(Piecewise((1, x > 0), (3, True)), (x > 0)) == 1
    assert refine(Piecewise((1, x > 0), (3, True)), ~(x > 0)) == 3
    ...
    assert refine(Piecewise((1, Eq(x, 0)), (3, True)), (Eq(y, 0))) == Piecewise(
        (1, Eq(x, 0)), (3, True)
    )
    assert refine(Piecewise((1, Ne(x, 0)), (3, True)), (Ne(x, 0))) == 1
    assert refine(Piecewise((1, Ne(x, 0)), (3, True)), ~(Ne(x, 0))) == 3
    assert refine(Piecewise((1, Ne(x, 0)), (3, True)), (Ne(y, 0))) == Piecewise(
        (1, Ne(x, 0)), (3, True)
    )
...
def test_im():
    ...
    assert (
        refine(im(x * y * z), Q.imaginary(x) & Q.imaginary(y) & Q.imaginary(z))
        == -I * x * y * z
    )
...
def test_complex():
    ...
    assert (
        refine(
            re((w + I * x) * (y + I * z)), Q.real(w) & Q.real(x) & Q.real(y) & Q.real(z)
        )
        == w * y - x * z
    )
    assert (
        refine(
            im((w + I * x) * (y + I * z)), Q.real(w) & Q.real(x) & Q.real(y) & Q.real(z)
        )
        == w * z + x * y
    )
