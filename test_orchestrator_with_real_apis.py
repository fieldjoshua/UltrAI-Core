...
async def test_orchestrator():
    ...
    if not any([openai_key, anthropic_key, google_key]):
        logger.error("No API keys found. Please set at least one of OPENAI_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_API_KEY.")
        sys.exit(1)
    ...
    if anthropic_key:
        ...
        claude_client = SimpleClientAdapter("Claude-3-Opus", "anthropic", "claude-3-opus-20240229", anthropic_key)
        orchestrator.register_model("claude3opus", claude_client, weight=1.0)
    
    if google_key:
        logger.info("Registering Google Gemini Pro model")
        gemini_client = SimpleClientAdapter("Gemini-Pro", "google", "gemini-1.5-pro-latest", google_key)
