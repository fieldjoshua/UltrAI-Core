...
class MockAdapter(BaseAdapter):
    ...
    async def generate(
        ...
        **kwargs
        ...
        if not response:
            ...
            response = self._generate_mock_response(prompt, response_length, temperature)
    ...
    def _get_canned_response(self, prompt: str) -> Optional[str]:
        ...
        if "error" in prompt_lower:
            if random.random() < 0.5:  # 50% chance of returning error for error-related prompts
                    ...
                    with open(os.path.join(self.responses_dir, file_name), "r") as f:
                        ...
                        for item in saved_responses:
                            if "prompt" in item and "response" in item:
                                ...
                                if item["prompt"].lower() in prompt_lower or prompt_lower in item["prompt"].lower():
    ...
    def _generate_mock_response(self, prompt: str, length: int, temperature: float) -> str:
        """
        ...
        elif "code" in prompt_lower or "function" in prompt_lower or "script" in prompt_lower:
