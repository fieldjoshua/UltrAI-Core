...
from src.orchestration.config import ModelConfig, LLMProvider
...
def create_adapter(model_name: str) -> Any:
    ...
    if provider != LLMProvider.MOCK and not api_key:
        logger.warning(f"No API key found for {provider_name}. {'Using mock adapter instead.' if use_mock else 'Will likely fail.'}")
