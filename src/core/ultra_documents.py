...
class UltraDocuments(UltraBase):
    ...
    def embed_chunks(self, chunks: List[Tuple[str, str]]) -> None:
        ...
        self.logger.info(f"Created index with {len(self.chunks)} chunks from {len(set(self.doc_mapping.values()))} documents")
    
    def retrieve_relevant_chunks(self, query: str, top_k: int = 5) -> List[Tuple[str, str, float]]:
    ...
    def process_documents_with_embedding(self, file_paths: List[Union[str, Path]]) -> List[Tuple[str, str, float]]:
        ...
        return [(os.path.basename(doc_path), chunk, 0.0) for doc_path, chunk in chunked_docs]
    
    def process_query_with_documents(self, query: str, file_paths: List[Union[str, Path]], 
        ...
        if self.embedding_model is not None:
            try:
                ...
                for doc_name, chunk, score in relevant_chunks:
                    context_parts.append(f"[From: {doc_name}, Relevance: {100 * (1 - score):.1f}%]\n{chunk}")
                ...
                self.logger.error(f"Error in semantic retrieval: {e}. Falling back to basic document processing.")
                
        # Fallback: traditional approach (concatenate all document contents)
        documents = self.process_documents(file_paths)
        context = "\n\n".join([f"[Document: {os.path.basename(path)}]\n{content[:2000]}..." 
                           if len(content) > 2000 else f"[Document: {os.path.basename(path)}]\n{content}" 
