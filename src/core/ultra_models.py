...
class UltraModels:
    def __init__(self):
        self.available_models: Dict[str, ModelConfig] = {
            ...
            "gemini25_pro_max": ModelConfig(
                ...
                input_cost_per_1k_tokens=0.018,
                output_cost_per_1k_tokens=0.018,
                cost_per_1k_tokens=0.018,
                ...
                with open(self.config_file, "r") as f:
                    ...
                    for model_id, config in saved_config.items():
                        if model_id in self.available_models:
                            self.available_models[model_id].is_enabled = config.get(
                                "is_enabled", True
                            )
    ...
    def save_configuration(self):
        ...
        try:
            config = {
                model_id: {"is_enabled": model_config.is_enabled}
    ...
    def get_model_summary(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        summary = {category.value: [] for category in ModelCategory}
    ...
    async def select_best_models(
        self, inquiry: str, api_keys: Dict[str, str]
    ) -> List[Tuple[str, str]]:
        """
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a model selection expert. Select exactly three models that best suit the given inquiry.",
                    },
            ...
            for selection in selected_models:
                ...
                if (
                    model_id in self.available_models
                    and self.available_models[model_id].is_enabled
                ):
                    valid_selections.append(
                        (model_id, selection.get("reason", "No reason provided"))
                    )
