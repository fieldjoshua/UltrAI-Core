...
class ParallelOrchestrator(BaseOrchestrator):
    ...
    def _update_provider_performance(self, provider_id: str, latency: float, success: bool) -> None:
        """
    ...
    def _sort_providers_by_performance(self, provider_ids: List[str]) -> List[str]:
        ...
        # Calculate scores
        scores = {}
        max_latency = max(self.provider_performance[pid]["avg_latency"] for pid in providers_with_perf)
        
        for pid in providers_with_perf:
            perf = self.provider_performance[pid]
            # Normalize latency (higher is better)
            norm_latency = 1.0 - (perf["avg_latency"] / max_latency) if max_latency > 0 else 0.5
    ...
    async def _process_with_early_stopping(
        ...
        **options
        ...
        successful_count = 0
        max_providers = self.max_parallel_providers if self.max_parallel_providers > 0 else len(provider_ids)
        ...
        while remaining_providers and len(active_providers) < max_providers:
            ...
            for pid in active_providers:
                task = asyncio.create_task(self.send_request_to_provider(pid, prompt, **options))
                tasks[pid] = task
            ...
            for task in done:
                ...
                try:
                    ...
                    self.logger.info(f"Received successful response from provider '{pid}'")
                    
                    # Check if we have enough successful responses and early stopping is enabled
                    if self.use_early_stopping and successful_count >= self.min_responses_needed:
                        ...
                        self.logger.info(f"Early stopping after {successful_count} successful responses")
                        return responses
    ...
    async def _select_best_response(
        ...
        successful = {
            pid: info for pid, info in responses.items() 
        ...
        best_pid = min(
            candidates,
            key=lambda pid: successful[pid].get("metadata", {}).get("latency", float("inf"))
        )
    ...
    async def process(
        ...
        **options
        ...
        early_stopping_active = self.use_early_stopping and self.min_responses_needed < len(provider_ids)
        
        responses = await self._process_with_early_stopping(prompt, provider_ids, **options)
        ...
        try:
            best_provider_id, best_response = await self._select_best_response(responses)
            best_result = {
            ...
            "responses": [
                {
                    ...
                    "error": info.get("error") if not info.get("success", False) else None,
