...
class AdaptiveOrchestrator(BaseOrchestrator):
    ...
    def _update_strategy_performance(
        ...
        perf["avg_response_time"] = (1 - alpha) * perf[
            "avg_response_time"
        ] + alpha * response_time
    ...
    def _select_strategy(self, prompt: str, **options) -> OrchestrationStrategy:
        ...
        if "strategy" in options:
            ...
            try:
                ...
                self.logger.warning(
                    f"Invalid strategy '{strategy_name}', using default"
                )
    ...
    def _select_providers(
        self, strategy: OrchestrationStrategy, provider_ids: List[str], prompt: str
    ) -> List[str]:
        """
        ...
        for pid in provider_ids:
            ...
            avg_latency = (
                ...
                if stats.get("requests_successful", 0) > 0
            ...
            norm_cost = 1.0 - min(
                1.0, cost / 2.0
            )  # Normalize cost (0-1, where 1 is cheapest)
            ...
            if strategy == OrchestrationStrategy.QUALITY_OPTIMIZED:
                ...
                scores[pid] = (
                    (0.7 * success_rate) + (0.2 * norm_latency) + (0.1 * norm_cost)
                )

            elif strategy == OrchestrationStrategy.SPEED_OPTIMIZED:
                # Prioritize latency (speed)
                scores[pid] = (
                    (0.2 * success_rate) + (0.7 * norm_latency) + (0.1 * norm_cost)
                )

            elif strategy == OrchestrationStrategy.COST_OPTIMIZED:
                # Prioritize cost
                scores[pid] = (
                    (0.2 * success_rate) + (0.1 * norm_latency) + (0.7 * norm_cost)
                )
                ...
                # Balanced approach
                scores[pid] = (
                    (0.4 * success_rate) + (0.3 * norm_latency) + (0.3 * norm_cost)
                )

            else:
                # Default balanced scoring
                scores[pid] = (
                    (0.33 * success_rate) + (0.33 * norm_latency) + (0.33 * norm_cost)
                )

        # Sort providers by score
        sorted_providers = sorted(
            provider_ids, key=lambda pid: scores.get(pid, 0), reverse=True
        )
    ...
    async def _strategy_parallel(
        ...
        return await parallel_orchestrator.process(
            prompt, selected_providers, **options
        )
            ...
            "responses": [
                {
                    ...
                    "error": (
                        "Not attempted (previous provider succeeded)"
                        if pid != successful_provider
                        else None
                    ),
                }
                for pid in selected_providers
    ...
    async def _strategy_balanced(
        ...
        result = await parallel_orchestrator.process(
            prompt, selected_providers, **options
        )
    ...
    async def _strategy_cost_optimized(
        ...
        sorted_by_cost = sorted(
            provider_ids, key=lambda pid: self._estimate_request_cost(pid, prompt)
        )
    ...
    async def _strategy_quality_optimized(
        ...
        original_analysis_type = getattr(
            simple_orchestrator, "analysis_type", "comparative"
        )
        simple_orchestrator.analysis_type = "comparative"

        result = await simple_orchestrator.process(
            prompt, selected_providers, **options
        )
        ...
        if "metadata" in result:
            result["metadata"][
                "strategy"
            ] = OrchestrationStrategy.QUALITY_OPTIMIZED.value
    ...
    async def process(
        ...
        quality_score = (
            0.8  # Placeholder - would be calculated from actual response quality
        )
        cost = sum(
            self._estimate_request_cost(pid, prompt) for pid in valid_provider_ids
        )
