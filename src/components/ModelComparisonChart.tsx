...
const ModelComparisonChart: React.FC<ModelComparisonChartProps> = ({
    ...
    useEffect(() => {
        if (chartRef.current) {
            ...
            if (ctx) {
                ...
                const normalizedResponseTime = responseTime.map(time =>
                    100 - ((time / maxResponseTime) * 100)
                );
                ...
                chartInstance.current = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: models.map((model, index) => ({
                            ...
                            backgroundColor: themeColors.pointBackgroundColors[index % themeColors.pointBackgroundColors.length],
                            borderColor: themeColors.borderColors[index % themeColors.borderColors.length],
                            borderWidth: 2,
                            pointRadius: 5,
                            pointHoverRadius: 7
                        }))
    ...
    return (
        <div className="model-comparison-container" style={{ width: '100%', height: '400px' }}>
            <canvas ref={chartRef} />
        </div>
    );
};
