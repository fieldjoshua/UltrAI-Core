...
class PerformanceTester:
    ...
    async def make_request(
        self, session, prompt, selected_models, pattern="Confidence Analysis"
    ):
        ...
        try:
            ...
            async with session.post(
                f"{self.api_url}/api/analyze", json=payload
            ) as response:
    ...
    def generate_report(self, initial_metrics, final_metrics, test_config):
        ...
        report = {
            ...
            "system_metrics": {
                ...
                "max_cpu_percent": (
                    max([m["cpu_percent"] for m in self.system_metrics])
                    if self.system_metrics
                    else 0
                ),
                "avg_memory_mb": (
                ...
                "max_memory_mb": (
                    max([m["memory_mb"] for m in self.system_metrics])
                    if self.system_metrics
                    else 0
                ),
            },
            "backend_metrics": {
                ...
                "delta": {
                    "requests_processed": (
                        (
                            final_metrics.get("requests_processed", 0)
                            - initial_metrics.get("requests_processed", 0)
                        )
                        if final_metrics and initial_metrics
                        else None
                    ),
                    "avg_processing_time": (
                        final_metrics.get("avg_processing_time")
                        if final_metrics
                        else None
                    ),
                    "max_memory_usage": (
                        final_metrics.get("max_memory_usage") if final_metrics else None
                    ),
                },
            },
        }
        ...
        # Save to CSV
        results_df.to_csv(
            f"{self.output_dir}/{timestamp}_{test_name}_results.csv", index=False
        )
        metrics_df.to_csv(
            f"{self.output_dir}/{timestamp}_{test_name}_metrics.csv", index=False
        )
        ...
        self.generate_visualizations(
            results_df, metrics_df, report, f"{self.output_dir}/{timestamp}_{test_name}"
        )
    ...
    def generate_visualizations(self, results_df, metrics_df, report, output_prefix):
        ...
        plt.axhline(
            y=p95, color="r", linestyle="-", label=f"95th Percentile: {p95:.2f}s"
        )
        ...
        plt.plot(
            ...
            results_df["cumulative_requests"],
        ...
        # Add percentile lines
        plt.axvline(
            x=p95, color="r", linestyle="-", label=f"95th Percentile: {p95:.2f}s"
        )
    ...
    def print_summary(self, report):
        ...
        if backend["requests_processed"] is not None:
            ...
            print(
                f"Final average processing time: {backend['avg_processing_time']:.2f}s"
            )
...
async def run_single_test(
    api_url, output_dir, test_type, prompt_complexity, concurrency, models
):
...
async def run_continuous_monitoring(
    api_url, output_dir, duration_hours=24, interval_minutes=15
):
                ...
                try:
                    async with session.get(f"{api_url}/api/metrics") as response:
                        if response.status == 200:
                            ...
                            print(
                                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Metrics collected. "
...
async def main():
    ...
    parser.add_argument(
        "--test-type",
        type=str,
        choices=list(TEST_DURATIONS.keys()),
        default="standard",
    ...
    parser.add_argument(
        "--concurrency", type=int, default=2, help="Concurrent requests"
    )
    parser.add_argument(
        "--models",
        type=str,
    ...
    if args.mode == "single":
        ...
        await run_continuous_monitoring(
            args.url, args.output, args.duration, args.interval
        )
