...
class UltrAIBusinessModel:
    ...
    def get_base_cost(self, model, token_count):
        ...
        raw_price = self.calculator.calculate_cost(
            model=model, token_count=token_count, tier=self.base_tier
        )
        ...
        if raw_price["status"] == "error":
            ...
            raw_price = self.calculator.calculate_cost(
                model=fallback_model, token_count=token_count, tier=self.base_tier
            )
    ...
    def calculate_profit_margin(
        self, model, token_count, markup_type="standard", reserve_amount=0
    ):
        """
        ...
        for threshold, discount in sorted(
            self.reserve_discounts.items(), key=lambda x: int(x[0])
        ):
        ...
        result = {
            ...
            "cost_per_1k_tokens": {
                ...
                "customer_price": (
                    (final_price / token_count) * 1000 if token_count > 0 else 0
                ),
            },
            "effective_price_adjusted_for_efficiency": self.calculate_efficiency_adjusted_price(
                model, final_price, token_count
            ),
        }
    ...
    def calculate_efficiency_adjusted_price(self, model, price, token_count):
        ...
        effective_price_per_1k = (
            (effective_price / token_count) * 1000 if token_count > 0 else 0
        )
    ...
    def compare_pricing_strategies(
            ...
            for token_count in token_counts:
                for markup in markup_types:
                    for reserve in reserve_amounts:
                        ...
                        eff_price = result["effective_price_adjusted_for_efficiency"][
                            "effective_price_per_1k_tokens"
                        ]
                        if eff_price < min_effective_price and eff_price > 0:
                            ...
                            min_price_config = {
                                ...
                                "efficiency_factor": result[
                                    "effective_price_adjusted_for_efficiency"
                                ]["efficiency_factor"],
                            }

                        # Calculate value score (profit margin × efficiency)
                        # Higher profit and better efficiency (lower factor) gives higher score
                        value_score = result["profit_margin_percentage"] * (
                            1
                            / result["effective_price_adjusted_for_efficiency"][
                                "efficiency_factor"
                            ]
                        )
        ...
        for reserve in reserve_amounts:
            ...
            for markup in markup_types:
                ...
                for model in all_models:
                    for token_count in token_counts:
                        no_reserve = results["models"][model][token_count][markup][0][
                            "final_price_to_customer"
                        ]
                        with_reserve = results["models"][model][token_count][markup][
                            reserve
                        ]["final_price_to_customer"]
    ...
    def generate_optimal_tiers(self):
        ...
        standard_model = self.analysis_results["summary"]["most_cost_effective"][
            "model"
        ]
        standard_price_per_1k = self.analysis_results["summary"]["most_cost_effective"][
            "effective_price_per_1k"
        ]
    ...
    def generate_reserve_recommendations(self):
        ...
        for amount, discount in sorted(
            self.reserve_discounts.items(), key=lambda x: int(x[0])
        ):
            if int(amount) in self.analysis_results["reserve_analysis"]:
                avg_savings = self.analysis_results["reserve_analysis"][int(amount)][
        ...
        return {
            ...
            "optimal_entry_point": (
                min(reserve_levels, key=lambda x: x["breakeven_queries"])
                if reserve_levels
                else None
            ),
            "best_value_point": (
                max(reserve_levels, key=lambda x: x["avg_savings_percentage"])
                if reserve_levels
                else None
            ),
        }
    ...
    def generate_model_recommendations(self):
        ...
        most_profitable_model = self.analysis_results["summary"]["most_profitable"][
            "model"
        ]
        most_cost_effective_model = self.analysis_results["summary"][
            "most_cost_effective"
        ]["model"]
        ...
        efficient_models = sorted(
            [
                (model, factor)
                for model, factor in self.token_efficiency.items()
                if factor < 1.0
            ],
            key=lambda x: x[1],
        )
...
def main():
    ...
    analysis = model.compare_pricing_strategies(
        token_counts=[1000, 100000], reserve_amounts=[0, 10000, 100000]
    )
    ...
    print(
        f"  Markup type: {most_profitable['markup_type']} ({model.default_markup_strategy[most_profitable['markup_type']]}%)"
    )
    ...
    print(
        f"  Effective price per 1K tokens: ${cost_effective['effective_price_per_1k']:.6f}"
    )
    ...
    print(
        f"  Target price per 1K tokens: ${tiers['standard_tier']['target_price_per_1k']:.6f}"
    )
    ...
    print(
        f"  Price factor over standard: {tiers['premium_tier']['price_factor_over_standard']:.2f}x"
    )
    ...
    print(
        f"  Price factor over standard: {tiers['business_tier']['price_factor_over_standard']:.2f}x"
    )
