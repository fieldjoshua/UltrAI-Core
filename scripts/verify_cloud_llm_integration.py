...
async def verify_adapter_creation(provider: str, api_key: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if provider.lower() == "openai" and not isinstance(adapter, OpenAIAdapter):
            ...
            return False
        elif provider.lower() == "anthropic" and not isinstance(adapter, AnthropicAdapter):
...
async def verify_simple_generation(provider: str, api_key: Optional[str] = None) -> bool:
    """
...
async def verify_streaming_generation(provider: str, api_key: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        if not capabilities.get("supports_streaming", False):
            print(f"⚠️ Provider {provider} does not support streaming according to capabilities")
            return True
...
async def main():
    ...
    print("\n" + "=" * 50)
    overall_status = "✅ ALL PROVIDERS VERIFIED SUCCESSFULLY" if all_passed else "❌ VERIFICATION FAILED FOR SOME PROVIDERS"
