...
class TestProgressTracker:
    ...
    def complete_category(
        self, category: str, success: bool, stats: Dict[str, Any] = None
    ) -> None:
        ...
        if category in self.results:
            ...
            if self.results[category]["start_time"]:
                self.results[category]["duration"] = (
                    self.results[category]["end_time"]
                    - self.results[category]["start_time"]
                )
    ...
    def _save_results(self) -> None:
        ...
        report = {
            "summary": {
                ...
                "categories_pending": sum(
                    1 for c in self.results.values() if c["status"] == "pending"
                ),
                "categories_running": sum(
                    1 for c in self.results.values() if c["status"] == "running"
                ),
    ...
    def print_summary(self) -> None:
        ...
        for category, results in self.results.items():
            failed_tests = [
                d for d in results.get("details", []) if d.get("status") == "failed"
            ]
            if failed_tests:
                if not any_failures:
...
def update_from_test_run(
    tracker: TestProgressTracker, category: str, results_file: str
) -> None:
    ...
    for test in tests:
        ...
        message = (
            test.get("call", {}).get("longrepr", "") if outcome == "failed" else None
        )
...
def main():
    ...
    parser.add_argument(
        "--import-results", type=str, help="Import test results from a JSON file"
    )
    parser.add_argument(
        "--category", type=str, help="Test category when importing results"
    )
