...
async def check_model_availability(
    model_id: str, base_url: str = "http://localhost:8080"
) -> bool:
    """
...
async def pull_model(model_id: str, base_url: str = "http://localhost:8080") -> bool:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            create_payload = {"id": model_id, "name": model_id, "object": "model"}
            ...
            async with session.post(
                ...
                if response.status in (200, 201, 202):
                    ...
                    print(
                        f"❌ Failed to pull model {model_id}: {response.status} - {error_text[:200]}"
                    )
                    return False
...
async def main():
    ...
    if not await check_docker_model_runner(args.url):
        ...
        print(
            "5. Try manually pulling a model with 'docker model run phi3:mini \"Hello\"'"
        )
        return
    ...
    for model_id in models_to_pull:
        ...
        if not args.force and await check_model_availability(model_id, args.url):
            print(
                f"Skipping {model_id} as it's already available (use --force to override)"
            )
            continue
    ...
    if missing:
        ...
        print(
            "Some models failed to pull. Check the Docker Model Runner logs for details."
        )
    else:
