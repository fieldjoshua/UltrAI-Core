            ...
            try:
                async with session.get(f"{url}/v1/models", timeout=10) as response:
                    ...
                    if response.status == 200:
                        ...
                        model_list = ", ".join(models) if models else "No models available yet"
                        print(f"✅ Model Runner API is responding. Available models: {model_list}")
                        return True, models
                    else:
                ...
                print("1. Open Docker Desktop → Extensions → Model Runner → Check if it's running")
...
async def check_default_model(models: List[str]):
    ...
    preferred_models = ["phi3:mini", "phi3:small", "gemma:2b", "llama3:8b", "mistral:7b"]
...
async def pull_model(model: str, url: str = "http://localhost:8080"):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(pull_url, timeout=30) as response:
                    ...
                    if response.status in [200, 202, 204]:
                        ...
                        print("You can check status in Docker Desktop → Extensions → Model Runner")
                        print("\nThe model will be ready to use once it's downloaded.")
                        return True
                    else:
                        print(f"⚠️ Could not initiate pull via API (status: {response.status})")
                        response_text = await response.text()
                        print(f"Response: {response_text[:200]}...")
            ...
            except (aiohttp.ClientConnectorError, asyncio.TimeoutError, asyncio.CancelledError) as e:
                progress.stop()
...
async def main():
    ...
    print("======================================")
    print("This script will verify the minimum viable functionality of Docker Model Runner")
