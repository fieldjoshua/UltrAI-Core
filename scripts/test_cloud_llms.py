...
async def test_anthropic(
    api_key: Optional[str] = None, model: str = "claude-3-opus-20240229"
) -> bool:
    ...
    try:
        ...
        response = await adapter.generate("What is the Fermi Paradox? Answer briefly.")
...
async def test_gemini(
    api_key: Optional[str] = None, model: str = "gemini-1.5-flash-latest"
) -> bool:
...
async def test_provider_with_create_adapter(
...
async def main():
    ...
    if provider == "openai" or provider == "all":
        if provider == "openai" and model:
            ...
            results["openai_factory"] = await test_provider_with_create_adapter(
                "openai", model
            )
        else:
    ...
    if provider == "anthropic" or provider == "all":
        if provider == "anthropic" and model:
            ...
            results["anthropic_factory"] = await test_provider_with_create_adapter(
                "anthropic", model
            )
        else:
    ...
    if provider == "gemini" or provider == "all":
        if provider == "gemini" and model:
            ...
            results["gemini_factory"] = await test_provider_with_create_adapter(
                "gemini", model
            )
        else:
