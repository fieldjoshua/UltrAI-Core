...
class ProviderHealthManager:
    ...
    RECOVERY_WINDOW_MINUTES = int(os.getenv("PROVIDER_RECOVERY_WINDOW_MINUTES", "5"))
            ...
            for provider, health in self._provider_health.items():
                ...
                if health.status == "unhealthy" and health.last_failure:
                    ...
                    if time_since_failure > timedelta(
                        minutes=self.RECOVERY_WINDOW_MINUTES
                    ):
            ...
            async with self._lock:
                ...
                for provider, health in self._provider_health.items():
                    summary[provider] = {
                        ...
                        "last_error": health.last_error_message[:100]
    ...
    async def _update_health_status(self, provider: str) -> None:
        ...
        successful_latencies = [
            latency for success, latency in recent_requests if success and latency > 0
        ]
        if successful_latencies:
            health.average_latency_ms = sum(successful_latencies) / len(
                successful_latencies
            )
