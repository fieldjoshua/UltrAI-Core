...
class OutputFormatter:
    ...
    def format_pipeline_output(
        ...
        if isinstance(ultra_synthesis, dict) and "synthesis" in ultra_synthesis:
            formatted["synthesis"] = self._format_synthesis(ultra_synthesis["synthesis"])
        ...
        if include_initial_responses and initial_response:
            formatted["initial_responses"] = self._format_initial_responses(initial_response)
            ...
            if stage in pipeline_results:
                ...
                if isinstance(stage_data, dict):
                    if "successful_models" in stage_data:
                        summary["total_models_used"].update(stage_data["successful_models"])
    ...
    def _create_full_document(
        ...
        if "pipeline_summary" in formatted_data:
            ...
            doc_parts.append(self.subsection_separator)
            doc_parts.append(f"✅ Stages Completed: {', '.join(summary['stages_completed'])}")
            doc_parts.append(f"🤖 Models Used: {', '.join(summary['total_models_used'])}")
