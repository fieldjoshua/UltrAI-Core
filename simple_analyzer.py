...
# Import LLM adapters
try:
    from src.models.llm_adapter import create_adapter, OpenAIAdapter, AnthropicAdapter, GeminiAdapter
except ImportError:
...
def get_available_models(api_keys: Dict[str, str], use_mock: bool = False) -> Dict[str, Dict[str, Any]]:
...
async def generate_model_response(
    ...
    try:
        ...
        api_key = api_keys.get(provider, "") if provider != "openai" else api_keys.get("openai")
...
async def main():
    ...
    if args.models:
        ...
        if model_selection == "all" or model_selection == "":
            ...
            try:
                ...
                selected_models = [available_model_names[idx] for idx in selected_indices if 0 <= idx < len(available_model_names)]
                ...
                selected_models = [model for model in candidate_models if model in available_model_names]
    ...
    result = await analyze_prompt(
        args.prompt, selected_models, ultra_model, args.mock
    )
