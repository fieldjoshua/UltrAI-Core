      ...
      {options.map((opt) => (
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div>
              <h3 className="font-bold text-[11px] leading-tight">{opt.label}</h3>
              {opt.description && (
                <p className="text-[10px] text-foreground/80 leading-snug">{opt.description}</p>
              )}
              {opt.cost !== undefined && (
                <p className="text-[10px] text-pink-400 mt-1">+${opt.cost.toFixed(2)}</p>
              )}
            </div>
          </div>
