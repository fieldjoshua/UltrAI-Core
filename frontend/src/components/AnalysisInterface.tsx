  ...
  return [
    {
      ...
      useCases: ['Fact-based queries', 'Questions with objective answers', 'Time-sensitive analysis']
    },
    {
      id: 'confidence',
      name: 'Confidence Analysis',
      description: 'Evaluates the strength of each model response with confidence scoring',
      useCases: ['Uncertain queries', 'Risk assessment', 'Multiple possible interpretations']
    },
    {
      id: 'critique',
      name: 'Critique Analysis',
      description: 'Models critically evaluate each other\'s reasoning and answers',
      useCases: ['Complex reasoning tasks', 'Checks for logical flaws', 'Rigorous evaluation']
    },
    {
      id: 'fact_check',
      name: 'Fact Check Analysis',
      description: 'Verifies factual accuracy and cites sources for claims',
      useCases: ['Research topics', 'Checking controversial claims', 'Educational content']
    },
    {
      id: 'perspective',
...
const submitAnalysis = async (
  ...
  return {
    ...
    results: selectedModels.map(modelId => ({
      modelId,
      modelName: modelId === 'gpt4o' ? 'GPT-4o' : 
                 modelId === 'claude3opus' ? 'Claude 3 Opus' : 
                 modelId === 'gemini-pro' ? 'Gemini Pro' : modelId,
...
export const AnalysisInterface: React.FC = () => {
  ...
  const [analysisStatus, setAnalysisStatus] = useState<'idle' | 'preparing' | 'analyzing' | 'complete' | 'error'>('idle');
  ...
  const handleSubmit = async (submittedPrompt: string, options: AnalysisOptions) => {
    try {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="prompt">
          <Card className="p-6">
            ...
            <p className="text-gray-600 mb-6">
              Provide your query or task for the AI models to analyze. Be specific and include relevant details for best results.
            </p>
        ...
        <TabsContent value="models">
          ...
          {(analysisStatus === 'preparing' || analysisStatus === 'analyzing' || analysisStatus === 'complete' || analysisStatus === 'error') && (
            <div className="mt-6">
              <AnalysisProgress
