...
describe('AnalysisForm Component', () => {
  ...
  test('shows error when models fail to load', async () => {
    // Mock API failure
    apiService.fetchAvailableModels.mockRejectedValue(new Error('Failed to load models'));
  ...
  test('submits analysis request with correct data', async () => {
    ...
    fireEvent.change(promptInput, { target: { value: 'Test analysis prompt' } });
  ...
  test('displays results after successful analysis', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/combined ultra response from all models/i)).toBeInTheDocument();
      expect(screen.getByText(/test response from gpt-4o/i)).toBeInTheDocument();
      expect(screen.getByText(/test response from claude 3.7/i)).toBeInTheDocument();
    });
