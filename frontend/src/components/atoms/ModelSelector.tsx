...
export const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  const clearProvider = (provider: string) => {
    ...
    onSelectionChange(
      selectedModels.filter(id => !modelIds.includes(id))
    );
  };
      ...
      {Object.keys(groupedModels).length === 0 ? (
        ...
        <div className="space-y-4">
          {Object.entries(groupedModels).map(([provider, models]) => (
            <div key={provider} className="border border-gray-200 rounded-md overflow-hidden">
              {/* Provider header */}
              <div 
              ...
              >
                <div className="flex items-center">
                  ...
                  <div className="ml-2 text-sm text-gray-500">
                    ({countSelectedInProvider(provider)} of {models.filter(m => m.isAvailable).length} selected)
                  </div>
                </div>
                  ...
                  {models.map(model => (
                    ...
                    >
                      <Checkbox 
                        ...
                        onCheckedChange={() => model.isAvailable && toggleModel(model.id)}
                        disabled={
                          !model.isAvailable || 
