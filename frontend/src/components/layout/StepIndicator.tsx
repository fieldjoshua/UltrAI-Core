...
const StepIndicator: React.FC<StepIndicatorProps> = ({ currentStep }) => {
    ...
    const progressValue = currentStep === 'RESULTS' ? 100 : Math.round(((currentIndex) / (steps.length - 2)) * 100);
            ...
            <div className="flex justify-between w-full mb-2">
                {stepTitles.map((title, index) => (
                    ...
                    index > 0 && (
                        <div
                            key={title}
                            className={`text-xs md:text-sm font-medium text-center px-1 ${currentIndex >= index ? 'text-cyan-400' : 'text-gray-500'
                                }`}
                        >
                            {index}. {title}
                        </div>
