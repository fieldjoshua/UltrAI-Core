document.addEventListener('DOMContentLoaded', function () {
  ...
  async function fetchAnalysisTypes() {
    try {
      ...
      document.getElementById('analysis-types-loading').textContent =
        'Error loading analysis types. Please try again.';
    }
  }
  ...
  function renderAlacarteOptions(options) {
    ...
    options.forEach(option => {
      ...
      checkbox.addEventListener('change', () => {
        ...
        logDebug(
          `Selected a la carte options: ${selectedAlacarteOptions.join(', ')}`
        );
      });
  ...
  async function fetchOutputFormats() {
    try {
      ...
      document.getElementById('output-formats-loading').textContent =
        'Error loading output formats. Please try again.';
    }
  }
  ...
  function startProcessing() {
    ...
    const progressInterval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        showResults(
          'This is a simulated result. In production, this would be the output from the backend API.'
        );
      }
    }, 500);
