...
class TriLLMOrchestrator:
    def __init__(
        ...
        print(
            f"OpenAI: {api_keys.get('openai', '')[:5]}...{api_keys.get('openai', '')[-4:]}"
        )
        print(
            f"Google: {api_keys.get('google', '')[:5]}...{api_keys.get('google', '')[-4:]}"
        )
    ...
    def _create_meta_prompt(
        ...
        return self.prompt_templates.meta.format(
            responses=json.dumps(responses, indent=2), original_prompt=original_prompt
        )
    ...
    async def orchestrate_full_process(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            hyper_response = await self.get_hyper_response(
                ultra_response, meta_responses, initial_responses, prompt
            )
...
async def test_env() -> bool:
    ...
    if not openai_key:
        ...
        return False
    print(
        f"✓ OPENAI_API_KEY: {openai_key[:5]}...{openai_key[-4:]} (Length: {len(openai_key)})"
    )
    ...
    if not google_key:
        ...
        return False
    print(
        f"✓ GOOGLE_API_KEY: {google_key[:5]}...{google_key[-4:]} (Length: {len(google_key)})"
    )
...
async def main():
    ...
    while True:
        ...
        if choice in ["1", "2", "3"]:
            engine = {"1": "llama", "2": "chatgpt", "3": "gemini"}[choice]
            break
