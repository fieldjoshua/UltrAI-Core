...
from src.models.llm_adapter import create_adapter_async, LLMAdapter
...
async def initialize_adapters() -> Dict[str, LLMAdapter]:
    ...
    if anthropic_key:
        try:
            adapter = await create_adapter_async("anthropic", anthropic_key, model="claude-3-opus-20240229")
    ...
    if google_key:
        try:
            adapter = await create_adapter_async("gemini", google_key, model="gemini-pro")
    ...
    if not adapters:
        ...
        try:
            mock_adapter = await create_adapter_async("openai", "sk-mock-key", model="gpt-4")
            adapters["mock_openai"] = MockClientWrapper(mock_adapter)
            logger.info("Initialized mock OpenAI adapter")
            
            mock_anthropic = await create_adapter_async("anthropic", "sk-ant-mock-key", model="claude-3-opus-20240229")
...
async def test_multi_llm_orchestrator():
    ...
    prompt = "Explain the benefits of a microservices architecture over monolithic design"
    ...
    if response.get("status") != "success":
        logger.error(f"Error processing prompt: {response.get('error', 'Unknown error')}")
        return False
