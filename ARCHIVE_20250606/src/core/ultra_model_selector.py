...
from ultra_models import ModelCategory, UltraModels
...
class ModelSelector:
    ...
    def display_model_details(self, model_id: str):
        ...
        if info:
            ...
            print(
                f"Cost per 1k tokens: ${info['cost_per_1k_tokens']}"
                if info["cost_per_1k_tokens"]
                else "Cost per 1k tokens: Free"
            )
    ...
    async def select_best_models(self, inquiry: str):
        ...
        for model_id, reason in selected_models:
            ...
            if info:
                ...
                print(f"Category: {info['category']}")
                print(
                    f"Cost: {'Free' if info['cost_per_1k_tokens'] is None else f'${info['cost_per_1k_tokens']}/1k tokens'}"
                )
...
def main():
    ...
    parser.add_argument(
        "--save", action="store_true", help="Save current configuration"
    )
    parser.add_argument(
        "--enabled", action="store_true", help="Show currently enabled models"
    )
    parser.add_argument(
        "--select", type=str, help="Select best models for a given inquiry"
    )
