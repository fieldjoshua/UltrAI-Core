...
const MODEL_PRICING: Record<string, ModelPricing> = {
  ...
  'anthropic-claude-3-haiku': { input: 0.00025, output: 0.00125, context: 128000 },
  ...
  // Open source models
  'llama-2-13b': { input: 0.00075, output: 0.00100, context: 4096 },
  'llama-2-70b': { input: 0.00195, output: 0.00256, context: 4096 },
  'llama-3-8b': { input: 0.00050, output: 0.00080, context: 8192 },
...
export const PricingDisplay = ({
  ...
  const [effectiveInputTokens, setEffectiveInputTokens] = useState<number>(DEFAULT_INPUT_TOKENS);
  const [effectiveOutputTokens, setEffectiveOutputTokens] = useState<number>(DEFAULT_OUTPUT_TOKENS);
  ...
  useEffect(() => {
    ...
    if (promptText) {
      ...
      inputTokens = Math.max(inputTokens, Math.ceil(promptLength * TOKEN_RATIO_PER_CHAR));

      // Longer prompts generally produce longer responses
      const promptComplexityFactor = Math.min(2.0, 1.0 + (promptLength / 1000) * 0.25);
    ...
    if (attachments && attachments.length > 0) {
      ...
      const attachmentTokens = Math.ceil(totalAttachmentSizeKB * ATTACHMENT_TOKEN_PER_KB);
  ...
  }, [promptText, attachments, analysisType, estimatedInputTokens, estimatedOutputTokens]);
  ...
  }, [selectedModels, effectiveInputTokens, effectiveOutputTokens, analysisType, addonCost]);
  ...
  const getTokensSummary = () => {
    ...
    const factors = [];
    if (promptText && promptText.length > 100) factors.push("prompt complexity");
    if (attachments && attachments.length > 0) factors.push(`${attachments.length} attachment(s)`);
    if (analysisType && analysisType !== 'confidence') factors.push(`${analysisType} analysis`);
