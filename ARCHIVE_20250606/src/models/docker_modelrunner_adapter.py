...
class DockerModelRunnerAdapter(LLMAdapter):
    ...
    def __init__(
        self, api_key: Optional[str] = None, model: str = "phi3:mini", **kwargs
    ):
        """
        ...
        base_url = kwargs.get(
            "base_url", os.environ.get("MODEL_RUNNER_URL", "http://model-runner:8080")
        )
        ...
        self.timeout = kwargs.get(
            "timeout", int(os.environ.get("LOCAL_MODEL_TIMEOUT", 60000)) / 1000
        )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        self.logger.error(
                            f"Docker Model Runner error: {response.status} - {error_text}"
                        )
                        raise Exception(
                            f"Model Runner API returned status {response.status}: {error_text}"
                        )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        error_text = await response.text()
                        self.logger.error(
                            f"Docker Model Runner streaming error: {response.status} - {error_text}"
                        )
                        raise Exception(
                            f"Model Runner API returned status {response.status}: {error_text}"
                        )
                    ...
                    async for line in response.content:
                        ...
                        if line.startswith("data: "):
                            try:
                                ...
                                self.logger.error(
                                    f"Error parsing streaming response: {e}"
                                )
                                # Continue processing other chunks even if one fails

            self.logger.debug(
                f"Docker Model Runner streaming call completed for model {model}"
            )
...
def create_modelrunner_adapter(
    model: str = "phi3:mini", **kwargs
