...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Quick token price estimator for Ultra Framework"
    )

    parser.add_argument(
        "--text", type=str, help="Input text to estimate (wrap in quotes)"
    )
    ...
    if result["status"] == "error":
        ...
        print(
            f"Allowed models for {args.tier} tier: {', '.join(result['allowed_models'])}"
        )
        return
    ...
    if result["tokens_charged"] > 0:
        print(
            f"Tokens charged (after {result['included_tokens']} included): {result['tokens_charged']}"
        )
    else:
        print(
            f"Tokens charged: 0 (under the {result['included_tokens']} included tokens)"
        )
    ...
    if result["discount_amount"] > 0:
        print(
            f"Volume discount ({result['volume_discount_percentage']}%): -${result['discount_amount']:.6f}"
        )
