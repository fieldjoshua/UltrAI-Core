...
async def simple_orchestration_example():
    ...
    logger.info(f"Successful responses: {sum(1 for r in result['initial_responses'] if r.get('success', False))}")
...
async def parallel_orchestration_example():
    ...
    logger.info(f"Successful responses: {sum(1 for r in result['responses'] if r.get('success', False))}")
    logger.info(f"Early stopping triggered: {result['metadata'].get('early_stopping_triggered', False)}")
    
    # Display best response
    if result.get('best_response'):
        best_provider = result['best_response'].get('provider_id', 'unknown')
        best_response = result['best_response'].get('response', 'No best response available')
        logger.info(f"Best response from '{best_provider}' (truncated): {best_response[:200]}...")
...
async def adaptive_orchestration_example():
    ...
    strategies = [
        (OrchestrationStrategy.BALANCED, "What are the best practices for API security?"),
        (OrchestrationStrategy.QUALITY_OPTIMIZED, "Analyze the implications of quantum computing for modern cryptography."),
    ...
    for strategy, prompt in strategies:
        ...
        if 'responses' in result:
            logger.info(f"Received {sum(1 for r in result['responses'] if r.get('success', False))} successful responses")
        ...
        # For strategies that return synthesis
            ...
            logger.info(f"Number of providers: {len(result.get('initial_responses', []))}")
            synthesis = result['synthesis'].get('response', 'No synthesis available')
            logger.info(f"Synthesis (truncated): {synthesis[:200]}...")
