...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("parameter_editor.log"), logging.StreamHandler()],
)
...
def load_glossary() -> Dict[str, Any]:
    ...
    try:
        if not glossary_file.exists():
            ...
            print(
                f"{Colors.RED}Error: Glossary file not found at {glossary_file}{Colors.ENDC}"
            )
            print(
                f"Run 'python parameter_glossary_generator.py' first to generate the glossary."
            )
            sys.exit(1)
...
def save_changes(changes: List[Dict[str, Any]]) -> bool:
    ...
    for change in changes:
        ...
        try:
            ...
            patterns = [
                ...
                re.compile(
                    f"(os\\.getenv\\([\"']\\s*{simple_name}\\s*[\"'],\\s*(?:[\"'])?)(.*?)([\"']?\\s*\\))",
                    re.MULTILINE,
                ),
            ...
            if not replaced:
                logger.warning(
                    f"Could not find pattern to replace {param_name} in {file_path}"
                )
                print(
                    f"{Colors.YELLOW}Warning: Could not find pattern to replace {param_name} in {file_path}{Colors.ENDC}"
                )
                failure_count += 1
                continue
            ...
            print(
                f"{Colors.RED}Error updating {param_name} in {file_path}: {e}{Colors.ENDC}"
            )
            failure_count += 1
    ...
    if failure_count > 0:
        print(
            f"{Colors.YELLOW}{failure_count} changes failed (see log for details){Colors.ENDC}"
        )
...
def filter_parameters(
    glossary: Dict[str, Any], search_term: str
) -> List[Dict[str, Any]]:
...
def print_parameter_table(parameters: List[Dict[str, Any]], title: str = "Parameters"):
    ...
    print(
        f"{Colors.CYAN}{'-' * (name_width + value_width + type_width + path_width + 13)}{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}{'Name':<{name_width}}  {'Value':<{value_width}}  {'Type':<{type_width}}  {'File':<{path_width}}{Colors.ENDC}"
    )
    print(
        f"{Colors.CYAN}{'-' * (name_width + value_width + type_width + path_width + 13)}{Colors.ENDC}"
    )
    ...
    for i, param in enumerate(parameters):
        ...
        print(
            f"{Colors.BOLD if i % 2 == 0 else ''}{name:<{name_width}}  {value:<{value_width}}  {param_type:<{type_width}}  {file_path:<{path_width}}{Colors.ENDC}"
        )
    ...
    while True:
        ...
        try:
            ...
            if 0 <= file_index < len(files):
                ...
                file_changes = browse_file_parameters(
                    glossary["files"][file_path], file_path
                )
            ...
            print(
                f"{Colors.RED}Invalid input. Please enter a number or 'q'.{Colors.ENDC}"
            )
            input("Press Enter to continue...")
...
def browse_file_parameters(
    file_data: Dict[str, Any], file_path: str
) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        if choice == "q":
            ...
            filtered = [
                ...
                or search_term in (p.get("doc_string", "") or "").lower()
            ]

            if filtered:
                print_parameter_table(
                    filtered, f"Filtered Parameters - '{search_term}'"
                )
            else:
...
def browse_parameters_by_category(glossary: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    changes = []
    categories = {
        "Environment Variables": glossary.get("categories", {}).get(
    ...
    while True:
        ...
        try:
            ...
            if 0 <= category_index < len(category_names):
                ...
                category_changes = browse_category_parameters(
...
def browse_category_parameters(
    parameters: List[Dict[str, Any]], category_name: str
) -> List[Dict[str, Any]]:
...
def search_parameters(glossary: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        if not results:
            print(
                f"{Colors.YELLOW}No parameters found matching '{search_term}'.{Colors.ENDC}"
            )
            input("Press Enter to continue...")
            continue
...
def generate_new_glossary():
    ...
    clear_screen()
    print(
        f"{Colors.BOLD}{Colors.BLUE}Generating new parameter glossary...{Colors.ENDC}"
    )

    generator_path = script_dir / "parameter_glossary_generator.py"
    if not generator_path.exists():
        print(
            f"{Colors.RED}Error: Generator script not found at {generator_path}{Colors.ENDC}"
        )
        return False
...
def main():
    ...
    parser.add_argument(
        ...
        "-g",
        action="store_true",
        help="Generate a new parameter glossary before starting",
    )
    ...
    if args.search:
        ...
        if results:
            ...
            print(
                f"{Colors.YELLOW}No parameters found matching '{args.search}'.{Colors.ENDC}"
            )
        return
