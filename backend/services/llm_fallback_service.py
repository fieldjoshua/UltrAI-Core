...
class LLMFallbackService:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.provider_model_mapping = self.config.get(
            "PROVIDER_MODEL_MAPPING",
            {
                ...
                "gemini": {"gemini15": "gemini-1.5-pro", "gemini10": "gemini-pro"},
                "mistral": {
                    "mistral": "mistral-large-latest",
                    "mixtral": "mixtral-8x7b-32768",
                },
                "docker_modelrunner": {"llama3": "llama3:8b", "phi3": "phi3:mini"},
            },
        )
        ...
        logger.info(
            f"LLM Fallback Service initialized with {len(self.provider_priority)} providers"
        )
    ...
    async def _save_to_cache(
        self, prompt: str, model: str, options: Dict[str, Any], response: str
    ) -> None:
        """
        ...
        cache_key = self._get_cache_key(prompt, model, options)
        cache_data = {"response": response, "timestamp": time.time(), "model": model}

        await cache_service.set_json(
            CACHE_PREFIX, {"key": cache_key}, cache_data, ttl=self.cache_ttl
        )
    ...
    async def _generate_mock_response(self, prompt: str, model: str) -> str:
        ...
        try:
            ...
            word_count = min(
                len(prompt.split()), 100
            )  # Use prompt length to determine response length
            return (
    ...
    async def generate_with_fallback(
        ...
        for provider_name, native_model in model_providers:
            ...
            if not circuit.allow_request():
                logger.warning(
                    f"Circuit breaker open for provider {provider_name}, skipping"
                )
                continue
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    response = await client.generate(
                        prompt=prompt, model=native_model, **options
                    )
        ...
        raise last_error or RuntimeError(
            f"Failed to generate response for model {model}"
        )
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        """
        status = {"providers": {}, "circuits": circuit_registry.get_all_status()}
        ...
        for provider in self.provider_priority:
            ...
            status["providers"][provider] = {
                ...
                "priority": (
                    self.provider_priority.index(provider)
