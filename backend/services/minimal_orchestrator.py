...
class MinimalOrchestrator:
    ...
    def _init_adapters(self):
        ...
        if anthropic_key:
            self.adapters["anthropic"] = {
                "claude-3": AnthropicAdapter(anthropic_key, model="claude-3-sonnet"),
                "claude-3-opus": AnthropicAdapter(anthropic_key, model="claude-3-opus")
            }
    ...
    def _get_adapter(self, model_name: str):
        ...
        logger.warning(f"No adapter found for model: {model_name} (mapped to {backend_name})")
        return None
    
    async def _call_model(self, model_name: str, prompt: str, stage: str = "initial") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Model {model_name} timed out after {TIMEOUT_SECONDS}s")
            return {
    ...
    def _build_meta_prompt(self, original_prompt: str, initial_responses: Dict[str, str]) -> str:
        ...
        return f"""Several of your fellow LLMs were given the same prompt as you. Their responses are as follows:

{responses_text}

Do NOT assume that anything written is correct or properly sourced, but given these other responses, could you make your original response better? More insightful? More factual, more comprehensive when considering the initial user prompt? If you do believe you can make your original response better, please draft a new response to the initial inquiry.

Original inquiry: {original_prompt}"""
    
    def _build_ultra_prompt(self, original_prompt: str, meta_responses: Dict[str, str]) -> str:
        ...
        return f"""To the chosen synthesizer: You are tasked with creating the Ultra Synthesis: a fully-integrated intelligence synthesis that combines the relevant outputs from all methods into a cohesive whole, with recommendations that benefit from multiple cognitive frameworks. The objective here is not to be necessarily the best, but the most expansive synthesization of the many outputs. While you should disregard facts or analyses that are extremely anomalous or wrong, with the original prompt in mind, your final Ultra Synthesis should reflect all of the relevant meat from the meta level responses, organized in a manner that is clear and non repetitive.

Original prompt: {original_prompt}

Meta-level responses to synthesize:
{responses_text}"""