...
class PricingCalculator:
    ...
    def calculate_cost(self, model, token_count, tier="basic", features=None):
        ...
        volume_discount = 0
        for threshold, discount in sorted(self.pricing["volume_discounts"].items(), key=lambda x: int(x[0])):
    ...
    def estimate_monthly_cost(self, daily_queries, avg_tokens, model_distribution, tier="basic", features=None):
        """
        ...
        if total_percentage != 1.0:
            model_distribution = {k: v/total_percentage for k, v in model_distribution.items()}
