...
class PricingIntegration:
    ...
    def get_session_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        if session_id in self.session_token_usage:
            session_data = self.session_token_usage[session_id]
            session_data["duration"] = self._calculate_duration(session_data["start_time"])
            return session_data
    ...
    def estimate_request_cost(self, 
        ...
        if self.pricing_enabled and user_id in self.user_accounts:
            has_sufficient_balance = self.user_accounts[user_id]["balance"] >= cost_info["total_cost"]
...
async def track_request_cost(price_integration: PricingIntegration,
                            user_id: str,
                            model: str,
                            token_count: int,
...
async def check_request_authorization(price_integration: PricingIntegration,
                                    user_id: str,
                                    model: str,
                                    estimated_tokens: int,
                                    request_type: str = "completion") -> Dict[str, Any]:
    ...
    return {
        ...
        "estimated_cost": estimate["estimated_cost"],
        "current_balance": price_integration.user_accounts[user_id]["balance"],
        "remaining_balance": price_integration.user_accounts[user_id]["balance"] - estimate["estimated_cost"]
    }
