...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("parameter_glossary.log"), logging.StreamHandler()],
)
...
class ParameterInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "raw_value": (
                self.value
                if isinstance(self.value, (str, bool, int, float))
                else str(self.value)
            ),
...
class ParameterExtractor(ast.NodeVisitor):
            ...
            if isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name):
                        name = target.id
                        if name.isupper() or any(
                            re.match(pattern, name) for pattern in PARAMETER_PATTERNS
                        ):
                            try:
                                value = ast.literal_eval(item.value)
                                doc_string = (
                                    class_doc  # Use class docstring for context
                                )
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        for arg in node.args.defaults:
            if isinstance(
                arg, (ast.Str, ast.Num, ast.NameConstant, ast.List, ast.Dict)
            ):
                try:
    ...
    for line in content.split("\n"):
        for pattern in env_patterns:
            ...
            for match in matches:
                ...
                comment_pos = line.find("#")
                doc_string = (
                    line[comment_pos + 1 :].strip() if comment_pos >= 0 else None
                )
...
def generate_parameter_glossary(
    parameters_by_file: Dict[str, List[ParameterInfo]],
) -> Dict[str, Any]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate a parameter glossary for UltraAI"
    )
    parser.add_argument(
        "--output", "-o", default="parameters.json", help="Output JSON file"
    )
