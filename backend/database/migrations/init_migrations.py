...
def create_baseline_migration(
    message: str = "Initial database schema", dry_run: bool = False
) -> int:
    """
    ...
    return run_alembic_command(
        ["revision", "--autogenerate", "--message", message], dry_run=dry_run
    )
...
def init_migrations(
    message: str = "Initial database schema", stamp: bool = True, dry_run: bool = False
) -> int:
    """
...
def main():
    ...
    try:
        return init_migrations(
            message=args.message, stamp=not args.no_stamp, dry_run=args.dry_run
        )
