...
def backup_database(connection_string: Optional[str] = None, 
    ...
    try:
        ...
        logger.error(f"Backup failed: {e.stderr.decode('utf-8') if e.stderr else str(e)}")
        raise
...
def restore_database(backup_path: str, connection_string: Optional[str] = None) -> bool:
    ...
    try:
        ...
        logger.error(f"Restore failed: {e.stderr.decode('utf-8') if e.stderr else str(e)}")
        return False
...
def get_migration_status() -> Dict[str, Any]:
    ...
    try:
        ...
        total_migrations = len([line for line in history_output.splitlines() 
        ...
        try:
            ...
            if current_revision and current_revision.lower() != "current":
                ...
                cmd = ['alembic', '-c', ALEMBIC_CONFIG, 'history', '--verbose']
                history_output = subprocess.check_output(cmd, universal_newlines=True)
                pending_migrations = len([line for line in history_output.splitlines() 
                                      if line.startswith("Rev: ")])
        ...
        try:
            ...
            logger.warning("Could not import database connection module")
            db_connected = None
        ...
        logger.error(f"Error getting migration status: {e.output.decode('utf-8') if hasattr(e, 'output') else str(e)}")
        return {
            "error": str(e),
            "checked_at": datetime.now().isoformat(),
        }
...
def create_parser() -> argparse.ArgumentParser:
    ...
    upgrade_parser.add_argument("--sql", action="store_true", 
                             help="Don't execute the migration, just show SQL")
    upgrade_parser.add_argument("--backup", action="store_true", 
                             help="Create database backup before upgrading")
    
    # Downgrade command
    downgrade_parser = subparsers.add_parser("downgrade", help="Downgrade database schema")
    downgrade_parser.add_argument("revision", help="Target revision or relative revision (-1)")
    downgrade_parser.add_argument("--sql", action="store_true", 
                               help="Don't execute the migration, just show SQL")
    downgrade_parser.add_argument("--backup", action="store_true", 
    ...
    generate_parser.add_argument("--autogenerate", action="store_true", 
    ...
    verify_parser.add_argument("--head-only", action="store_true",
                            help="Only verify the head migration")
    ...
    for p in [status_parser, upgrade_parser, downgrade_parser, 
              generate_parser, verify_parser,
              backup_parser, restore_parser]:
        ...
        if args.command == "status":
            ...
            if args.backup:
                try:
                    ...
                    if input("Continue with upgrade without backup? (y/n): ").lower() != 'y':
                        return 1
            ...
            if args.backup:
                try:
                    ...
                    logger.error(f"Backup failed: {str(e)}")
                    if input("Continue with downgrade without backup? (y/n): ").lower() != 'y':
                        return 1
