...
class UltrAIPricingModel:
    ...
    def calculate_effective_cost(self, model, tokens, markup_percentage=15, reserve_amount=0):
        """
        ...
        """
        # Get base costs
        base_cost_per_1k = self.base_costs.get(model, 0.001)  # Default to GPT-3.5 if model not found
        ...
        return {
            ...
            "enterprise_savings": self.calculate_enterprise_savings(model, tokens, markup_percentage)
        }
    ...
    def calculate_enterprise_savings(self, model, tokens, markup_percentage):
        ...
        if model == "claude-3-opus":
            ...
            direct_api_cost = (tokens / 1000) * (self.base_costs[model] * 1.8)  # Mistral has higher direct costs
        else:
        ...
        ultrai_cost = (tokens / 1000) * self.base_costs[model] * (1 + markup_percentage/100)
        
        # Calculate savings
        savings = direct_api_cost - ultrai_cost if direct_api_cost else 0
        savings_percentage = (savings / direct_api_cost) * 100 if direct_api_cost and direct_api_cost > 0 else 0
    ...
    def find_optimal_markup(self, models=None, token_amounts=None, min_profit_margin=10):
        """
            ...
            for tokens in token_amounts:
                ...
                for markup in self.markup_percentages:
                    ...
                    for reserve, discount in self.reserve_discounts.items():
                        ...
                        results["models"][model][tokens][markup]["with_reserve"][reserve] = reserve_analysis
            ...
            for model in models:
                if tokens in results["models"][model]:
                    for markup in results["models"][model][tokens]:
                        analysis = results["models"][model][tokens][markup]["base"]
                        if (analysis["effective_cost_per_1k"] < best_price and 
                            analysis["profit_margin"] >= min_profit_margin):
        ...
        for reserve in self.reserve_discounts:
            ...
            for model in models:
                for tokens in token_amounts:
                    for markup in self.markup_percentages:
                        if (tokens in results["models"][model] and 
                            markup in results["models"][model][tokens] and
                            reserve in results["models"][model][tokens][markup]["with_reserve"]):
                            
                            base_cost = results["models"][model][tokens][markup]["base"]["final_cost"]
                            reserve_cost = results["models"][model][tokens][markup]["with_reserve"][reserve]["final_cost"]
                            
                            if base_cost > 0:
                                savings_pct = (base_cost - reserve_cost) / base_cost * 100
    ...
    def generate_pricing_recommendations(self, min_profit_margin=15):
        ...
        standard_markups = [analysis["summary"]["optimal_markups"].get(m) for m in standard_tier_models if analysis["summary"]["optimal_markups"].get(m)]
        premium_markups = [analysis["summary"]["optimal_markups"].get(m) for m in premium_tier_models if analysis["summary"]["optimal_markups"].get(m)]
        enterprise_markups = [analysis["summary"]["optimal_markups"].get(m) for m in enterprise_tier_models if analysis["summary"]["optimal_markups"].get(m)]
        ...
        optimal_reserve = max(reserve_impact.items(), key=lambda x: x[1] / self.reserve_discounts[x[0]])[0] if reserve_impact else 10000
            ...
            "reserve_accounts": {
                ...
                "breakeven_calculations": {
                    amount: {
                        "amount": amount,
                        "discount": discount,
                        "breakeven_queries": amount / (amount * discount / 100) if discount > 0 else float('inf')
                    } for amount, discount in self.reserve_discounts.items()
                }
            },
            "model_recommendations": {
                "efficiency_leaders": sorted([(m, f) for m, f in self.token_efficiency.items() if f < 1.0], key=lambda x: x[1])[:3],
                "cost_effectiveness_leaders": sorted([(m, c) for m, c in self.base_costs.items()], key=lambda x: x[1] * self.token_efficiency.get(x[0], 1.0))[:3],
        ...
        for tier_name, tier_info in recommendations["pricing_tiers"].items():
            for model in tier_info["models"][:2]:  # Just use first two models for examples
                for tokens in [1000, 10000, 100000]:
                    ...
                    tier_info["sample_pricing"][key] = {
                        ...
                        "savings_vs_direct": analysis["enterprise_savings"]["savings_percentage"]
                    }
    ...
    def print_recommendations(self, recommendations=None):
        ...
        for tier_name, tier_info in recommendations["pricing_tiers"].items():
            ...
            for key, pricing in tier_info["sample_pricing"].items():
                print(f"    • {pricing['model']} ({pricing['tokens']:,} tokens): ${pricing['cost']:.4f} " +
                      f"(${pricing['cost_per_1k']:.4f}/1K, effective: ${pricing['effective_cost_per_1k']:.4f}/1K)")
                if pricing['savings_vs_direct'] > 0:
                    print(f"      Saves {pricing['savings_vs_direct']:.1f}% vs. direct API access")
        
        # Print reserve account recommendations
        print("\n-- Reserve Account Recommendations --")
        print(f"Optimal entry point: ${recommendations['reserve_accounts']['optimal_entry_point']:,}")
        
        print("\nDiscount structure:")
        for amount, discount in recommendations["reserve_accounts"]["discount_structure"].items():
            breakeven = recommendations["reserve_accounts"]["breakeven_calculations"][amount]["breakeven_queries"]
            print(f"  ${amount:,}: {discount}% discount (breakeven at {breakeven:.0f} queries)")
        
        # Print model recommendations
        print("\n-- Model Recommendations --")
        
        print("\nEfficiency leaders (fewest tokens for equivalent quality):")
        for model, factor in recommendations["model_recommendations"]["efficiency_leaders"]:
            print(f"  • {model}: {factor:.2f}x tokens needed (lower is better)")
        
        print("\nCost-effectiveness leaders:")
        for model, cost in recommendations["model_recommendations"]["cost_effectiveness_leaders"]:
        ...
        for tokens, model in recommendations["model_recommendations"]["best_models_by_volume"].items():
            if model:
                print(f"  • {tokens:,} tokens: {model}")
        
        # Print marketing messaging
        print("\n-- Marketing Messaging --")
        print(f"Value proposition: {recommendations['marketing_messaging']['value_proposition']}")
