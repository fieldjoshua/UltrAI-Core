...
def check_environment():
    ...
    if missing_files:
        print(f"{Colors.RED}Error: The following required files are missing:{Colors.ENDC}")
...
def generate_glossary():
    ...
    try:
        result = subprocess.run([sys.executable, str(generator_file)], 
...
def show_menu():
    ...
    if glossary_file.exists():
        ...
        print(f"{Colors.YELLOW}No parameter glossary found. You need to generate one first.{Colors.ENDC}")
...
def main():
    ...
    parser.add_argument("--edit", "-e", action="store_true", 
                        help="Launch parameter editor directly")
    parser.add_argument("--search", "-s", 
                        help="Search for parameters matching the given term")
    ...
    if args.generate:
        ...
        if not glossary_file.exists():
            print(f"{Colors.YELLOW}Parameter glossary not found. Generating now...{Colors.ENDC}")
        ...
        if choice == 'q':
            ...
            if choice == '1':
                if generate_glossary():
                    print(f"{Colors.GREEN}Parameter glossary generated. You can now browse parameters.{Colors.ENDC}")
                    input("Press Enter to continue...")
            else:
                print(f"{Colors.RED}Invalid option. Please try again.{Colors.ENDC}")
                input("Press Enter to continue...")
