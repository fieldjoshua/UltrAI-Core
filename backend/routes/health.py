...
class DependencyStatus(BaseModel):
    ...
    error: Optional[str] = Field(
        None, description="Error message if dependency is not available"
    )
    installation_cmd: Optional[str] = Field(
        None, description="Command to install the dependency"
    )
...
class FeatureStatus(BaseModel):
    ...
    dependencies: Optional[List[str]] = Field(
        None, description="Dependencies required for this feature"
    )
...
class ServiceStatus(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(
        None, description="Service-specific details"
    )
    using_fallback: Optional[bool] = Field(
        None, description="Whether service is using fallback"
    )
...
class HealthResponse(BaseModel):
    ...
    services: Optional[Dict[str, Dict[str, Any]]] = Field(
        None, description="Service statuses"
    )
...
async def basic_health_check():
    ...
    return {"status": overall_status, "uptime": int(time.time() - start_time)}
...
async def api_health_check(
    ...
    if service:
        ...
        if health_check:
            ...
            return JSONResponse(
                status_code=404, content={"error": f"Service '{service}' not found"}
            )
    ...
    if type:
        try:
            ...
            service_statuses = {name: check.check() for name, check in services.items()}
            ...
            if any(
                status.get("status") == HealthStatus.CRITICAL
                for status in service_statuses.values()
            ):
                response["status"] = HealthStatus.CRITICAL
            elif any(
                status.get("status") == HealthStatus.DEGRADED
                for status in service_statuses.values()
            ):
            ...
            return JSONResponse(
                status_code=400, content={"error": f"Invalid service type: {type}"}
            )
...
async def get_llm_health(
    ...
    if any(
        ...
        status.get("status") == HealthStatus.UNAVAILABLE
        for status in provider_statuses.values()
    ):
