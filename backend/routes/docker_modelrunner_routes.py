...
class ModelRunnerRequest(BaseModel):
    ...
    model: Optional[str] = Field(None, description="The model to use (defaults to configured default)")
...
async def list_models():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Environment: PATH={os.environ.get('PATH', 'N/A')}, DOCKER_HOST={os.environ.get('DOCKER_HOST', 'N/A')}")
            ...
            if process.returncode != 0:
                ...
                raise HTTPException(status_code=500, detail=f"Docker model list command failed: {error}")
            ...
            raise HTTPException(status_code=500, detail=f"Error listing Docker models: {str(e)}")
    except Exception as e:
...
async def generate(request: ModelRunnerRequest):
    ...
    try:
        ...
        try:
            ...
            # For debugging - show environment
            logger.info(f"Running with model: {model}, Environment: PATH={os.environ.get('PATH', 'N/A')}")
            ...
            if process.returncode != 0:
                ...
                raise HTTPException(status_code=500, detail=f"Docker model command failed: {error}")
            
            response = stdout.decode().strip()
            return {"model": model, "response": response}
        except Exception as e:
            logger.error(f"Error executing Docker model command: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Error executing Docker model command: {str(e)}")
