    ...
    while True:
        try:
            ...
            for i in range(count):
                while True:
                    ...
                    if model_name in models:
                        print(f"You've already added {model_name}. Please choose a different model.")
                        continue
                    
                    percentage = get_input(f"Percentage for {model_name} (1-100)", 100 if i == 0 else 0, int)
            ...
            if total_percentage != 100:
                print(f"Total percentage ({total_percentage}%) is not 100%. Normalizing values...")
                for model in models:
...
def get_features():
    ...
    while True:
        try:
            ...
            for i in range(count):
                while True:
                    feature_num = get_input(f"Feature #{i+1} (enter number 1-5)", 1, int)
                    ...
                    if feature_name in features:
                        print(f"You've already added {feature_name}. Please choose a different feature.")
                        continue
    ...
    while True:
        ...
        if choice == 1:
            ...
            for model, data in result["model_costs"].items():
                print(f"  {model}: ${data['cost']:.2f} ({data['percentage']:.1f}% of usage)")
