...
ANALYSIS_TYPES = [
    {
        ...
        "description": "Multiple models analyze your prompt in parallel",
    },
    {
        "id": "depth",
        "name": "In-Depth Analysis",
        "description": "Ultra performs a detailed analysis with all models",
    },
    {
        "id": "comparative",
        "name": "Comparative Analysis",
        "description": "Compare results across different models",
    },
]
...
ALACARTE_OPTIONS = [
    {
        ...
        "description": "Provides more in-depth results",
    },
    {
        "id": "code_generation",
    ...
    },
    {
        "id": "citation",
        "name": "Citation",
        "description": "Include citations for information sources",
    },
    {
        "id": "summary",
        "name": "Executive Summary",
        "description": "Include a concise summary of findings",
    },
]
...
OUTPUT_FORMATS = [
    {"id": "text", "name": "Plain Text", "description": "Results as plain text"},
    {
        "id": "markdown",
        "name": "Markdown",
        "description": "Results formatted with markdown",
    },
    {
        "id": "html",
        "name": "HTML",
        "description": "Results as HTML for rich formatting",
    },
...
async def analyze(request: AnalysisRequest):
    ...
    if request.analysis_type not in valid_analysis_types:
        raise HTTPException(
            status_code=400, detail=f"Invalid analysis type: {request.analysis_type}"
        )
    ...
    if request.output_format not in valid_output_formats:
        raise HTTPException(
            status_code=400, detail=f"Invalid output format: {request.output_format}"
        )
    ...
    for model_id in request.models:
        ...
        response_text = (
            f"Analysis from {model_info['name']} by {model_info['provider']}\n\n"
        )

        # Add analysis type info
        analysis_type_info = next(
            at for at in ANALYSIS_TYPES if at["id"] == request.analysis_type
        )
        ...
        if selected_options:
            ...
            for option_id in selected_options:
                option = next(
                    (o for o in ALACARTE_OPTIONS if o["id"] == option_id), None
                )
                if option:
        ...
        if model_info["type"] == "thinking":
            ...
            response_text += (
                "This is a straightforward processing of your request with key points."
            )
        ...
        if request.output_format == "markdown":
            response_text = (
                f"# Analysis from {model_info['name']}\n\n"
                + response_text.replace("\n\n", "\n\n## ")
            )
        elif request.output_format == "html":
            response_text = (
                f"<h1>Analysis from {model_info['name']}</h1><p>"
                + response_text.replace("\n\n", "</p><h2>")
                + "</h2>"
            )
