...
PRICE_ANOMALY_CRITICAL_THRESHOLD = 1.0  # 100% price change (doubling/halving) triggers critical warning
...
class PricingUpdater:
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if self.changes or self.force_update:
                ...
                logger.info(f"Pricing update completed: {len(self.changes)} changes detected")
                return True
            else:
    ...
    def _scrape_all_providers(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame(pricing_data, columns=[
            "Model", "Input Cost per 1K", "Output Cost per 1K", 
            "Total Cost per 1K", "Context Window", "Is Thinking Model"
        ])
    ...
    def _convert_scraped_data(self, df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            ...
            is_thinking_model = row["Is Thinking Model"] if not pd.isna(row["Is Thinking Model"]) else False
            
            # Determine if this is a "thinking model" based on certain criteria
            if not isinstance(is_thinking_model, bool):
                is_thinking_model = (
                    "gpt-4" in model_name.lower() or 
                    "claude" in model_name.lower() and "opus" in model_name.lower() or
                    "claude" in model_name.lower() and "sonnet" in model_name.lower() or
                    "gemini" in model_name.lower() and "pro" in model_name.lower() and "max" in model_name.lower()
                )
        ...
        for model_id, new_data in self.updated_pricing.items():
            if model_id in self.current_pricing:
                ...
                if old_input > 0 and new_input > 0:
                    ...
                    input_change_direction = "increase" if new_input > old_input else "decrease"
                    
                    # Check for anomalies in input cost
                    self._check_price_anomaly(model_id, "input_cost_per_1k", old_input, new_input, input_change_pct)
                ...
                if old_output > 0 and new_output > 0:
                    ...
                    output_change_direction = "increase" if new_output > old_output else "decrease"
                    
                    # Check for anomalies in output cost
                    self._check_price_anomaly(model_id, "output_cost_per_1k", old_output, new_output, output_change_pct)
                ...
                if old_context > 0 and new_context > 0 and old_context != new_context:
                    ...
                    if context_change_pct > 2.0:  # 200% change in context window is unusual
    ...
    def _check_price_anomaly(self, model_id: str, field: str, old_value: float, new_value: float, change_pct: float) -> None:
    ...
    def _update_models_file(self) -> None:
        ...
        for model_id, pricing in self.updated_pricing.items():
            ...
            if not all(k in pricing for k in ["input_cost_per_1k", "output_cost_per_1k", "total_cost_per_1k"]):
                continue
            ...
            if re.search(pattern, content):
                ...
                logger.warning(f"Model {model_id} not found in models file - would need to add it manually")
    
    def _update_existing_model(self, model_id: str, pricing: Dict[str, Any], content: str) -> None:
        ...
        if match:
            ...
            for model_id, pricing in self.updated_pricing.items():
                ...
                if model_match:
                    ...
                    pricing_block = pricing_block.replace(model_match.group(0), model_block)
                else:
                    # TODO: Add code to insert new model in the pricing block
                    logger.warning(f"Model {model_id} not found in pricing simulator - would need to add it manually")
            ...
            for section in pricing_sections:
                ...
                for row in model_rows:
                    try:
                        ...
                        input_price = cells[1].get_text(strip=True) if len(cells) > 1 else "N/A"
                        output_price = cells[2].get_text(strip=True) if len(cells) > 2 else "N/A"
                        total_price = "N/A"  # Calculate later
                        context = cells[3].get_text(strip=True) if len(cells) > 3 else "N/A"
                        
                        # Determine if it's a thinking model
                        is_thinking = (
                            "gpt-4" in model_name.lower() and not "mini" in model_name.lower() or
                            "o1" in model_name.lower()
                        )
                        
                        pricing_data.append([f"OpenAI {model_name}", input_price, output_price, total_price, context, is_thinking])
                    except Exception as e:
                        logger.warning(f"Error parsing OpenAI row: {e}")
            
            if not pricing_data:
                logger.warning("No OpenAI pricing data found - using current pricing data")
                ...
                if not pricing_data:
                    logger.warning("No current OpenAI pricing data - using hardcoded fallback values")
                    pricing_data = [
                        ["OpenAI GPT-4o", "$0.0025", "$0.0100", "$0.0125", "128K", True],
                        ["OpenAI GPT-4o mini", "$0.00015", "$0.00060", "$0.00075", "128K", False],
                        ["OpenAI GPT-4 Turbo", "$0.01", "$0.03", "$0.04", "128K", True],
                        ["OpenAI GPT-3.5 Turbo", "$0.0005", "$0.0015", "$0.0020", "4K", False],
                        ["OpenAI GPT-o1", "$0.015", "$0.060", "$0.075", "200K", True],
                        ["OpenAI GPT-o3 mini", "$0.00110", "$0.00440", "$0.00550", "200K", False]
                    ]
            ...
            for table in pricing_tables:
                ...
                for row in rows:
                    try:
                        ...
                        # Determine if it's a thinking model
                        is_thinking = (
                            "opus" in model_name.lower() or
                            "sonnet" in model_name.lower() and "3.7" in model_name
                        )
                        
                        pricing_data.append(["Anthropic " + model_name, input_price, output_price, total_price, context, is_thinking])
                    except Exception as e:
                        logger.warning(f"Error parsing Anthropic row: {e}")
            
            if not pricing_data:
                logger.warning("No Anthropic pricing data found - using current pricing data")
                # Use current pricing instead of hardcoded fallbacks
    ...
    def _fetch_google_pricing(self) -> List[List[Any]]:
        ...
        try:
            ...
            if not pricing_data:
                logger.warning("No Google pricing data found - using current pricing data")
    ...
    def _get_current_provider_pricing(self, provider: str) -> List[List[Any]]:
        ...
        if not pricing_data:
            logger.warning(f"No current {provider} pricing data - using hardcoded fallback values")
            if provider.lower() == "openai":
                ...
                pricing_data = [
                    ["Anthropic Claude 3.7 Sonnet", "$0.003", "$0.015", "$0.018", "200K", True],
                    ["Anthropic Claude 3 Opus", "$0.015", "$0.075", "$0.090", "200K", True],
                    ["Anthropic Claude 3.5 Haiku", "$0.0008", "$0.0040", "$0.0048", "200K", False]
                ]
            elif provider.lower() == "google":
                pricing_data = [
                    ["Google Gemini 1.5 Pro", "$0.000075", "$0.000300", "$0.000375", "128K", False],
                    ["Google Gemini 2.5 Pro Max", "$0.003", "$0.015", "$0.018", "1M", True]
                ]
            elif provider.lower() == "cohere":
                pricing_data = [
                    ["Cohere Command R+", "$0.00250", "$0.01000", "$0.0125", "256K", False],
                    ["Cohere Command R", "$0.00015", "$0.00060", "$0.00075", "100K", False]
                ]
            elif provider.lower() == "ai21":
                pricing_data = [
                    ["AI21 Jamba Large", "$0.0020", "$0.0080", "$0.0100", "100K", False],
                    ["AI21 Jamba Mini", "$0.0002", "$0.0004", "$0.0006", "100K", False]
                ]
            elif provider.lower() == "aws":
                pricing_data = [
                    ["AWS Bedrock Titan Text Express", "$0.0008", "$0.0016", "$0.0024", "8K", False],
                    ["AWS Bedrock Titan Text Lite", "$0.00015", "$0.00020", "$0.00035", "4K", False]
                ]
...
def main():
    ...
    parser.add_argument("--alert-email", help="Email to send alerts to (overrides environment variable)")
