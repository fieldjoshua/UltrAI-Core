...
def test_analyze_prompt_happy_path(client):
    ...
    assert "gpt4o" in model_responses or any("gpt" in model for model in model_responses), \
    ...
    assert "total_time_seconds" in performance, "Total time missing from performance metrics"
    assert "model_times" in performance, "Model times missing from performance metrics"
    assert "token_counts" in performance, "Token counts missing from performance metrics"
    
    # Ensure we're not getting placeholder data (common issue)
    assert "Paris is the capital" not in str(ultra_response), \
