...
class RealtimePricingEstimator:
    ...
    def get_real_time_estimate(self, user_prompt, model="gpt-3.5-turbo", features=None):
        ...
        # Get pricing
        pricing = self.calculator.calculate_cost(model, total_tokens, self.tier, features)
        
        # Add token estimates to the pricing info
        pricing.update({
            "prompt_tokens": estimated_tokens,
            "estimated_completion_tokens": completion_tokens,
            "prompt_text_preview": user_prompt[:100] + "..." if len(user_prompt) > 100 else user_prompt
        })
...
def main():
    ...
    while True:
        ...
        if estimate["status"] == "error":
            ...
            print(f"Allowed models for {tier} tier: {', '.join(estimate['allowed_models'])}")
            continue
        ...
        if estimate['discount_amount'] > 0:
            print(f"Volume discount ({estimate['volume_discount_percentage']}%): -${estimate['discount_amount']:.6f}")
        ...
        print(f"Monthly cost (excluding subscription): ${(estimate['total_cost'] * QUERIES_PER_DAY * 30):.2f}")
        print(f"Monthly subscription fee: ${estimate['monthly_fee']:.2f}")
        print(f"Total monthly cost: ${(estimate['total_cost'] * QUERIES_PER_DAY * 30 + estimate['monthly_fee']):.2f}")
