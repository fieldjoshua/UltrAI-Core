...
from backend.utils.validation import is_url_safe, validate_url, url_validator
...
def test_url_validation():
    ...
    for url in disallowed_urls:
        try:
            is_safe = is_url_safe(url)
            print(f"  {url}: {'✅ Allowed (PROBLEM!)' if is_safe else '✅ Blocked (correct)'}")
        except Exception as e:
...
def test_environment_variables():
    ...
    try:
        ...
        print(f"  Without SENTRY_DSN: {'✅ Empty string (correct)' if sentry_dsn == '' else '❌ Not empty (problem)'}")
        
        os.environ["SENTRY_DSN"] = "test-dsn-value"
        sentry_dsn = os.environ.get("SENTRY_DSN", "")
        print(f"  With SENTRY_DSN: {'✅ Value present' if sentry_dsn == 'test-dsn-value' else '❌ Value missing'}")
        
        # Test ENVIRONMENT handling
        print("\nTesting ENVIRONMENT handling:")
        os.environ.pop("ENVIRONMENT", None)
        env = os.environ.get("ENVIRONMENT", "development")
        print(f"  Without ENVIRONMENT: {'✅ Default to development' if env == 'development' else '❌ Wrong default'}")
        
        os.environ["ENVIRONMENT"] = "production"
        env = os.environ.get("ENVIRONMENT", "development")
        print(f"  With ENVIRONMENT=production: {'✅ Value present' if env == 'production' else '❌ Value missing'}")
        ...
        print(f"  example.com allowed: {'✅ Yes (correct)' if is_example_allowed else '❌ No (problem)'}")
        print(f"  test.org allowed: {'✅ Yes (correct)' if is_test_allowed else '❌ No (problem)'}")
        print(f"  api.openai.com allowed: {'✅ No (correct)' if not is_openai_allowed else '❌ Yes (problem)'}")
